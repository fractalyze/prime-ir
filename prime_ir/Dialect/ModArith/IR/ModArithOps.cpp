...
// Helper: get storage bit width and modulus APInt from a ModArithType result.
std::pair<unsigned, APInt> getModArithParams(Type type) {
...
void setLazyRedcRange(Value result, SetIntRangeFn setResultRange) {
  auto [w, p] = getModArithParams(result.getType());
...
void ConstantOp::inferResultRanges(ArrayRef<ConstantIntRanges> /*argRanges*/,
  ...
  if (attr) {
    ...
    setResultRange(getResult(),
                   ConstantIntRanges::fromUnsigned(APInt::getZero(w), p - 1));
  }
}
