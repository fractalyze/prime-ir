...
ParseResult parseEllipticCurveConstant(OpAsmParser &parser,
  ...
  ShortWeierstrassAttr curveAttr = getCurveFromPointLike(pointType);
  Type baseFieldType = curveAttr.getBaseField();

  unsigned numCoords = getNumCoordsFromPointLike(pointType);
...
ConstantOp ConstantOp::materialize(OpBuilder &builder, Attribute value,
  ...
  if (!isa<AffineType>(getElementTypeOrSelf(type)) &&
      !isa<JacobianType>(getElementTypeOrSelf(type)) &&
      !isa<XYZZType>(getElementTypeOrSelf(type))) {
    return nullptr;
  }
