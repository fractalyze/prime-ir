...
ParseResult parseDenseValues(OpAsmParser &parser, SmallVector<APInt> &parsedInts,
...
//   Tensor ext field:   dense<[[[a0,a1],[b0,b1]], ...]> : tensor<Nx!affine_ext2>
ParseResult parseEllipticCurveConstant(OpAsmParser &parser,
  ...
  // 2. Parse type
  Type parsedType;
  if (failed(parser.parseGreater()) || failed(parser.parseColonType(parsedType)))
    return failure();
  ...
  auto coordsAttr =
      reconstructCoordsAttr(parser.getContext(), parsedInts, pointType, numPoints);
