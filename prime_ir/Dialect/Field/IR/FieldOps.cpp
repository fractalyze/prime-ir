  ...
  OpFoldResult foldTensor(DenseIntElementsAttr attr) const override {
    ...
    for (size_t i = 0; i < numElements; i += degree) {
      ...
      for (size_t j = 0; j < degree; ++j) {
        coeffs.push_back(*(inputValues.begin() + i + j));
      }
...
public:
  ...
  OpFoldResult foldTensor(DenseIntElementsAttr lhsAttr,
    ...
    for (size_t i = 0; i < numElements; i += degree) {
      SmallVector<APInt> lhsCoeffs, rhsCoeffs;
      lhsCoeffs.reserve(degree);
      rhsCoeffs.reserve(degree);
      for (size_t j = 0; j < degree; ++j) {
        lhsCoeffs.push_back(*(lhsValues.begin() + i + j));
        rhsCoeffs.push_back(*(rhsValues.begin() + i + j));
      }
