...
struct ConvertConstant : public OpConversionPattern<ConstantOp> {
  ...
  matchAndRewrite(ConstantOp op, OpAdaptor adaptor,
    ...
    for (auto coeff : denseAttr.getValues<APInt>()) {
      ...
      primeCoeffs.push_back(b.create<mod_arith::ConstantOp>(modType, coeffAttr));
    }
...
struct ConvertPowUI : public OpConversionPattern<PowUIOp> {
  ...
  matchAndRewrite(PowUIOp op, OpAdaptor adaptor,
    ...
    Value result = generateBitSerialLoop(
        b, exp, base, init,
        [](ImplicitLocOpBuilder &b, Value v) -> Value {
          return b.create<SquareOp>(v);
        },
        [](ImplicitLocOpBuilder &b, Value acc, Value v) -> Value {
