...
// Montgomery reduction uses only 2 PMULL instructions for 256->128 bit reduction
//===----------------------------------------------------------------------===//
...
Value emitVEXT8(ImplicitLocOpBuilder &b, Value a) {
  auto vecType = VectorType::get(2, b.getI64Type());
  return b
      .create<LLVM::InlineAsmOp>(vecType, ValueRange{a},
                                 "ext $0.16b, $1.16b, $1.16b, #8", "=w,w",
                                 /*has_side_effects=*/false,
                                 /*is_align_stack=*/false,
                                 LLVM::TailCallKind::None, LLVM::AsmDialectAttr{},
                                 /*operand_attrs=*/ArrayAttr())
      .getResult(0);
}
...
// Note: Polyval uses different polynomial than Tower, producing different results!
