...
def pywrap_library(
    ...
    """A macro which does final linking of multiple C++ Python extensions that share common parts.
    ...
    compatibility with previous pybind_extension implementations, and should be strictly discouraged
    in new code.
    ...
    Args:
        ...
        common_lib_def_files_or_filters: Similar to common_lib_version_scripts argument, but is
            ...
            is necessary to deal with 2^16 exported symbols limit for a single .dll on Windows
            platform.
        ...
        pywrap_count: Number of python_extension artifacts in the transitive closure of deps; if
            ...
            thrown. This parameter is necessary for technical reasons. If not provided it will be
    ...
    # by different pywrap libraries but depending on Python symbols).
...
def python_extension(
    ...
    Args:
        ...
        wrap_py_init: Whether to wrap the PyInit_* function, making the extension artifact
            super-thin, contain only one PyInit_{name} function, with the rest of the logic being
...
def _collected_pywrap_infos_impl(ctx):
    ...
    Actual regular pywrap libraries in the transitive closure of {label}:
        {found_pws}
