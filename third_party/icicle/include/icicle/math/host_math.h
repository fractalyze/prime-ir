...
namespace host_math {
  ...
  template <typename T>
  static constexpr HOST_DEVICE_INLINE T add(const T x, const T y)
  {
    return x + y;
  }
  ...
  template <typename T>
  static constexpr HOST_DEVICE_INLINE T addc(const T x, const T y, const T carry)
  {
    return x + y + carry;
  }
  ...
  template <typename T>
  static constexpr HOST_DEVICE_INLINE T add_cc(const T x, const T y, T& carry)
  {
    T result = x + y;
    carry = x > result;
    return result;
  ...
  template <>
  HOST_DEVICE_INLINE uint64_t add_cc(const uint64_t x, const uint64_t y, uint64_t& carry)
  {
  ...
  template <typename T>
  static constexpr HOST_DEVICE_INLINE T addc_cc(const T x, const T y, T& carry)
  {
  ...
  template <>
  HOST_DEVICE_INLINE uint64_t addc_cc(const uint64_t x, const uint64_t y, uint64_t& carry)
  {
  ...
  // return x - y with T operands
  template <typename T>
  static constexpr HOST_DEVICE_INLINE T sub(const T x, const T y)
  {
    return x - y;
  }
  ...
  template <typename T>
  static constexpr HOST_DEVICE_INLINE T subc(const T x, const T y, const T borrow)
  {
    return x - y - borrow;
  }
  ...
  template <typename T>
  static constexpr HOST_DEVICE_INLINE T sub_cc(const T x, const T y, T& borrow)
  {
    T result = x - y;
    borrow = x < result;
    return result;
  ...
  template <typename T>
  static constexpr HOST_DEVICE_INLINE T subc_cc(const T x, const T y, T& borrow)
  {
  ...
  static constexpr HOST_DEVICE_INLINE uint32_t madc_cc(const uint32_t x, const uint32_t y, const uint32_t z, uint32_t& carry)
  {
  ...
  static constexpr HOST_DEVICE_INLINE uint64_t
  ...
  struct carry_chain {
    ...
    constexpr HOST_DEVICE_INLINE carry_chain() : index(0) {}

    template <typename T>
    constexpr HOST_DEVICE_INLINE T add(const T x, const T y, T& carry)
    {
      index++;
    ...
    template <typename T>
    constexpr HOST_DEVICE_INLINE T sub(const T x, const T y, T& carry)
    {
      index++;
      if (index == 1 && OPS_COUNT == 1 && !CARRY_IN && !CARRY_OUT)
  ...
  static constexpr HOST_DEVICE_INLINE uint32_t add_sub_limbs_32(const uint32_t* x, const uint32_t* y, uint32_t* r)
  {
    uint32_t carry = 0;
...
#pragma unroll
  ...
  template <unsigned NLIMBS, bool SUBTRACT, bool CARRY_OUT>
  static constexpr HOST_DEVICE_INLINE uint64_t add_sub_limbs_64(const uint64_t* x, const uint64_t* y, uint64_t* r)
  {
    uint64_t carry = 0;
    carry_chain<NLIMBS, false, CARRY_OUT> chain;
#pragma unroll
  ...
  template <
    ...
    bool CARRY_OUT,
    bool USE_32 = false>
  static constexpr HOST_DEVICE_INLINE uint32_t // 32 is enough for the carry
...
#pragma unroll
  ...
  template <unsigned NLIMBS_A, unsigned NLIMBS_B = NLIMBS_A>
  static HOST_DEVICE_INLINE void multiply_raw_64(const uint64_t* a, const uint64_t* b, uint64_t* r)
  {
#pragma unroll
...
#pragma unroll
  ...
  static HOST_DEVICE_INLINE void lsb_multiply_raw_64(const uint64_t* a, const uint64_t* b, uint64_t* r)
  {
#pragma unroll
...
#pragma unroll
  ...
  template <unsigned NLIMBS>
  static constexpr HOST_DEVICE_INLINE unsigned get_bit(const storage<NLIMBS>& xs, unsigned bit)
  {
    unsigned limb_size = sizeof(xs.limbs[0]) * 8;
    return (xs.limbs[bit / limb_size] >> (bit % limb_size)) & 0x1;
  }

  template <unsigned NLIMBS>
  static constexpr HOST_DEVICE_INLINE bool is_zero(const storage<NLIMBS>& xs)
  {
  ...
  static constexpr HOST_DEVICE_INLINE storage<NLIMBS> left_shift(const storage<NLIMBS>& xs)
  {
    if constexpr (BITS == 0)
      return xs;
    else {
...
#pragma unroll
  ...
  template <unsigned NLIMBS, unsigned BITS>
  static constexpr HOST_DEVICE_INLINE storage<NLIMBS> right_shift(const storage<NLIMBS>& xs)
  {
    if constexpr (BITS == 0)
      return xs;
    else {
...
#pragma unroll
  ...
  template <
    ...
    bool USE_32 = false>
  static constexpr HOST_DEVICE_INLINE void integer_division(
