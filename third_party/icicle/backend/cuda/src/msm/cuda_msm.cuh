...
#include "third_party/icicle/gpu-utils/error_handler.h"
#include "third_party/icicle/cuda_mont.cuh"
#include "third_party/icicle/msm/cuda_msm_config.cuh"
#include "third_party/icicle/error_translation.h"

// clang CUDA doesn't provide host-compatible min/max overloads like nvcc.
// Use two-argument template to handle mixed types (e.g., int vs unsigned).
#ifndef __NVCC__
namespace {
template <typename T, typename U>
__host__ __device__ __forceinline__ auto min(T a, U b) -> decltype(a < b ? a : b) { return a < b ? a : b; }
template <typename T, typename U>
__host__ __device__ __forceinline__ auto max(T a, U b) -> decltype(a > b ? a : b) { return a > b ? a : b; }
} // namespace
#endif
...
#define MAX_C_FOR_PRECOMPUTATION 22
      ...
      cudaStream_t stream)
    {
      ...
      // Always run cleanup synchronously. When called from multi_chunked_msm
      // (is_async=true), the main stream has already waited for all internal
      // stream events, so stream_large_buckets/stream_points are done.
      // Detached threads caused race conditions: they could still be running
      // cudaStreamDestroy when the next MSM call started.
      cleanup();
