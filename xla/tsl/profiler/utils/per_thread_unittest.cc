...
class ThreadFactory {
 public:
  ...
  void Start(int32_t id, ProfilingStage first_use_stage,
             ProfilingStage exit_stage) {
    std::string name = absl::StrCat("thread_", id);
    threads_existing_at_[exit_stage].emplace_back(absl::WrapUnique(
        Env::Default()->StartThread(ThreadOptions(), name, [=]() {
          ThreadMain(id, first_use_stage, exit_stage);
        })));
  }

  void StopAllAt(ProfilingStage exit_stage) {
    threads_existing_at_[exit_stage].clear();
  }
...
TEST(PerThreadRecordingTest, Lifecycles) {
  auto get_ids = [](std::vector<std::shared_ptr<int32_t>>& threads_data) {
    ...
    for (const auto& thread_value : threads_data) {
      threads_values.push_back(*thread_value);
    }
