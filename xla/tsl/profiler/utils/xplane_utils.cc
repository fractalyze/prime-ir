...
const XPlane* FindPlaneWithName(const XSpace& space, std::string_view name) {
...
std::vector<const XPlane*> FindPlanesWithNames(
    const XSpace& space, const std::vector<std::string_view>& names) {
  absl::flat_hash_set<std::string_view> names_set(names.begin(), names.end());
  std::vector<int> indices =
...
XPlane* FindMutablePlaneWithName(XSpace* space, std::string_view name) {
  int i = Find(space->planes(),
               [name](const XPlane* plane) { return plane->name() == name; });
  return (i != -1) ? space->mutable_planes(i) : nullptr;
}

XPlane* FindOrAddMutablePlaneWithName(XSpace* space, std::string_view name) {
...
const XLine* FindLineWithName(const XPlane& plane, std::string_view name) {
