...
XEventMetadata* XPlaneBuilder::GetOrCreateEventMetadata(std::string_view name) {
...
XEventMetadata* XPlaneBuilder::GetEventMetadata(std::string_view name) const {
  auto result = event_metadata_by_name_.find(name);
  if (result == event_metadata_by_name_.end()) return nullptr;
  return result->second;
}

XStatMetadata* XPlaneBuilder::GetStatMetadata(std::string_view name) const {
...
XStatMetadata* XPlaneBuilder::GetOrCreateStatMetadata(std::string_view name) {
