...
XLineBuilder CreateXLine(XPlaneBuilder* plane, std::string_view name,
                         std::string_view display, int64_t id,
...
void CreateXStats(XPlaneBuilder* plane, T* stats_owner,
                  std::string_view stats_name, V stats_value) {
  stats_owner->AddStatValue(*plane->GetOrCreateStatMetadata(stats_name),
                            stats_value);
}

void CheckXLine(const XLine& line, std::string_view name,
                std::string_view display, int64_t start_time_ns,
                int64_t events_size) {
...
void CheckXEvent(const XEvent& event, const XPlane& plane,
                 std::string_view name, std::string_view display,
...
TEST(XPlaneUtilsTest, MergeXPlaneTest) {
  ...
  absl::flat_hash_map<std::string_view, std::string_view> plane_stats;
...
TEST(XplaneUtilsTest, XPlaneGroupingPropagatesStep) {
  ...
  // Check that step line is preserved
  EXPECT_GE(aggregated_xplane.lines_size(), 1);
}
