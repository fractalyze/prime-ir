...
std::vector<std::string_view> SplitNameAndMetadata(
...
std::vector<std::string_view> SplitPairs(std::string_view metadata) {
...
std::vector<std::pair<std::string_view, std::string_view>> ParseMetadata(
    std::string_view metadata) {
  std::vector<std::pair<std::string_view, std::string_view>> key_values;
  for (std::string_view pair : SplitPairs(metadata)) {
    std::vector<std::string_view> parts =
        absl::StrSplit(pair, absl::MaxSplits('=', 1));
    if (parts.size() == 2) {
      std::string_view key = absl::StripAsciiWhitespace(parts[0]);
      std::string_view value = absl::StripAsciiWhitespace(parts[1]);
...
Annotation ParseAnnotation(std::string_view annotation) {
  Annotation result;
  std::vector<std::string_view> parts = SplitNameAndMetadata(annotation);
  if (!parts.empty()) {
...
std::vector<Annotation> ParseAnnotationStack(
    std::string_view annotation_stack) {
  std::vector<Annotation> annotations;
  const std::string kAnnotationDelimiter = "::";
  for (std::string_view annotation : absl::StrSplit(
