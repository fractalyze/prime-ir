...
class XStatsBuilder {
 ...
 private:
  ...
  static void SetStatValue(std::string_view value, XStat* stat) {
    stat->set_str_value(std::string(value));
  }
  ...
  static void SetStatValue(const google::protobuf::MessageLite& proto,
                           XStat* stat) {
  ...
  void CopyStatValue(const XStat& src_stat, const XPlane& src_plane,
    ...
    switch (src_stat.value_case()) {
      ...
      case XStat::kRefValue: {
        ...
        if (ABSL_PREDICT_TRUE(it != stat_metadata_by_id.end())) {
  ...
  const XStatMetadata& GetOrCreateStatMetadata(std::string_view value);
...
class XPlaneBuilder : public XStatsBuilder<XPlane> {
 public:
  ...
  XEventMetadata* GetOrCreateEventMetadata(std::string_view name);
  XEventMetadata* GetOrCreateEventMetadata(std::string&& name);
  XEventMetadata* GetOrCreateEventMetadata(const char* name) {
    return GetOrCreateEventMetadata(std::string_view(name));
  }
  ...
  XEventMetadata* GetEventMetadata(std::string_view name) const;

  // Returns stat metadata with the given name. Returns nullptr if not found.
  XStatMetadata* GetStatMetadata(std::string_view name) const;
  ...
  // WARNING: If calling any of these overloads, do not call the integer one
  // above on the same instance.
  XStatMetadata* GetOrCreateStatMetadata(std::string_view name);
  XStatMetadata* GetOrCreateStatMetadata(std::string&& name);
  XStatMetadata* GetOrCreateStatMetadata(const char* name) {
    return GetOrCreateStatMetadata(std::string_view(name));
  }
...
template <typename T>
std::string_view XStatsBuilder<T>::StrOrRefValue(const XStat& stat) {
