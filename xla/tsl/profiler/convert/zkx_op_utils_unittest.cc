...
TEST(ZkxOpUtilsTest, HloModuleNameWithProgramId) {
  EXPECT_EQ("module(123)", HloModuleNameWithProgramId("module", 123));
}

TEST(ZkxOpUtilsTest, IsHloRematerialization) {
...
TEST(ZkxOpUtilsTest, IsFrameworkRematerialization) {
...
TEST(ZkxOpUtilsTest, IsRematerialization) {
...
TEST(ZkxOpUtilsTest, IsHostOrSparseCoreV0Infeed) {
