...
void BFCAllocator::AddTraceMe(std::string_view traceme_name, const void* ptr) {
...
void BFCAllocator::AddTraceMe(std::string_view traceme_name,
                              const void* chunk_ptr, int64_t req_bytes,
                              int64_t alloc_bytes) {
  profiler::TraceMe::InstantActivity(
...
std::string BFCAllocator::RenderOccupancy() {
  ...
  constexpr size_t kResolution = 100;
  char rendered[kResolution];
  ...
  RenderRegion(rendered, kResolution, total_region_size, 0, nullptr, nullptr,
               total_region_size, '_');
  ...
  for (const auto& region : region_manager_.regions()) {
    ...
    while (h != kInvalidChunkHandle) {
      ...
      if (c->in_use()) {
        ...
        if (wasted > 0) {
          RenderRegion(rendered, kResolution, total_region_size,
                       region_offset + c->requested_size, region.ptr(), c->ptr,
                       wasted, 'x');
        }
        // Then the occupied space
        RenderRegion(rendered, kResolution, total_region_size, region_offset,
  ...
  return std::string(rendered, kResolution);
}
