...
TransposeFusion::WriteResult TransposeFusion::EmitWriteToShMemMlir(
  ...
  for (auto* transpose : shmem_transposes_) {
    auto elem_type = mlir_utils::PrimitiveTypeToMlirType(
        transpose->shape().element_type(), ctx);
  ...
  if (!side_output_inits.empty()) {
    side_output_vector =
        emitters::EmitZkxLoopOp(builder, thread_and_block_ids,
...
void TransposeFusion::EmitReadFromShMemMlir(
  ...
  auto result_tensors = emitters::EmitZkxLoopOp(
...
IndexingMap TransposeFusion::GetIndexing(bool input, const Shape& shape,
