...
class TransposeTest : public HloTestBase {
 public:
  TransposeSpec GetTransposeSpecFromRoot(std::string_view hlo_text) {
...
TEST_F(TransposeTest, Transpose_10) {
  auto spec = GetTransposeSpecFromRoot(R"(ENTRY entry {
    p0 = s32[8, 32] parameter(0)
    ROOT transpose_p0 = s32[32, 8] transpose(p0), dimensions={1, 0}
  })");
...
TEST_F(TransposeTest, Transpose_210) {
  auto spec = GetTransposeSpecFromRoot(R"(ENTRY entry {
    p0 = s32[8, 2, 32] parameter(0)
    ROOT transpose_p0 = s32[32, 2, 8] transpose(p0), dimensions={2, 1, 0}
  })");
...
TEST_F(TransposeTest, Transpose_102) {
  auto spec = GetTransposeSpecFromRoot(R"(ENTRY entry {
    p0 = s32[8, 2, 32] parameter(0)
    ROOT transpose_p0 = s32[2, 8, 32] transpose(p0), dimensions={1, 0, 2}
  })");
...
TEST_F(TransposeTest, Transpose_42130) {
  auto spec = GetTransposeSpecFromRoot(R"(ENTRY entry {
    p0 = s32[8, 2, 32, 7, 6] parameter(0)
    ROOT transpose_p0 = s32[6, 32, 2, 7, 8] transpose(p0),
