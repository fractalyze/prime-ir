...
absl::StatusOr<int64_t> GetHeroIndex(std::string_view name,
...
std::pair<std::string, std::vector<int64_t>> ParseHeroAndIds(
    std::string_view hero_and_ids) {
  std::pair<std::string_view, std::string_view> hero_and_ids_pair =
      absl::StrSplit(hero_and_ids, ':');
  std::vector<int64_t> ids;
  for (std::string_view id : absl::StrSplit(hero_and_ids_pair.second, ',')) {
      ...
      tsl::Flag(
          ...
          [](std::string name_and_ids) {
            ...
            flags.bijection_inputs.push_back(
                zkx::gpu::ParseHeroAndIds(name_and_ids));
            return true;
          },
          "",
