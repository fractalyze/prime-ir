...
#ifndef ZKX_STREAM_EXECUTOR_INTEGRATIONS_TF_ALLOCATOR_ADAPTER_H_
#define ZKX_STREAM_EXECUTOR_INTEGRATIONS_TF_ALLOCATOR_ADAPTER_H_
...
// Adapter class that wraps a tsl::Allocator (like BFCAllocator) to
// DeviceMemoryAllocator.
//
// Assumes that the tsl::Allocator permits asynchronous deallocation:
...
class TfAllocatorAdapter : public DeviceMemoryAllocator {
 public:
  ...
  // The BFC allocator used on GPU allows host-side deallocation
  // before GPU execution takes place. This uses the ordering of the main
  // compute stream to enforce a happens-before relationship between a memory
  // allocation and code that reuses the same memory.
...
// streams as a TfAllocatorAdapter. Assumes that the allocator permits
...
class MultiDeviceAdapter : public DeviceMemoryAllocator {
 public:
  ...
  // The BFC allocator used on GPU allows host-side deallocation
  // before GPU execution takes place.
  // (This attribute has no effect on CPU.)
  bool AllowsAsynchronousDeallocation() const override { return true; }
 ...
 private:
  ...
  absl::flat_hash_map<std::pair<int, void*>, int64_t> buffer_memory_spaces_
      ABSL_GUARDED_BY(mu_);
  // The wrapped tsl::Allocators backing per_device_allocators_
...
#endif  // ZKX_STREAM_EXECUTOR_INTEGRATIONS_TF_ALLOCATOR_ADAPTER_H_
