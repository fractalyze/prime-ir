...
ProfileEvent::ProfileEvent(const XEvent* event, int64_t line_timestamp_ns,
    ...
    : event_(event),
      ...
      xspace_(xspace) {
  CHECK_NE(event_, nullptr);
  CHECK_NE(plane_, nullptr);
  CHECK_NE(xspace_, nullptr);
}
...
// static
ProfileData ProfileData::from_serialized_xspace(
    const nb::bytes& serialized_xspace) {
  return ProfileData(serialized_xspace);
}

// static
ProfileData ProfileData::from_file(const std::string& proto_file_path) {
  std::string serialized_xspace;
  zkx::ThrowIfError(tsl::ReadFileToString(tsl::Env::Default(), proto_file_path,
                                          &serialized_xspace));
  return ProfileData(serialized_xspace.c_str(), serialized_xspace.size());
}

// static
ProfileData ProfileData::from_raw_cpp_ptr(nb::capsule capsule) {
...
ProfileData::ProfileData(const void* serialized_xspace_ptr,
  ...
  CHECK_NE(serialized_xspace_ptr, nullptr);
