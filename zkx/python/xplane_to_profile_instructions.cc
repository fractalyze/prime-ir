...
std::unique_ptr<zkx::HloModule> CreateModuleFromProto(
    const zkx::HloModuleProto& proto) {
  auto config = zkx::HloModule::CreateModuleConfigFromProto(proto, {});
  if (config.ok()) {
    auto module = zkx::HloModule::CreateFromProto(proto, config.value());
    if (module.ok()) {
...
std::optional<std::string> GetHloModuleFingerprint(
    const zkx::HloModuleProto& hlo_module_proto) {
...
void GetXPlaneHloModuleInfo(
  ...
  xplane.ForEachEventMetadata([&](const XEventMetadataVisitor& event_metadata) {
    event_metadata.ForEachStat([&](const XStatVisitor& stat) {
      ...
      // TODO(ZKX): Use tsl::ParseProtoUnlimited when ported.
      if (hlo_proto.ParseFromArray(stat.BytesValue().data(),
                                   stat.BytesValue().size())) {
        const zkx::HloModuleProto& hlo_module_proto = hlo_proto.hlo_module();
