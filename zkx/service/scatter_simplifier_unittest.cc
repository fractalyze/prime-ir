...
TEST_F(ScatterSimplifierTest, InsertsIndexVectorAndWindowDims) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      ...
      operand1 = s32[3,3] parameter(1)
      indices = s32[2] parameter(2)
      update0 = s32[2,3] parameter(3)
      update1 = s32[2,3] parameter(4)
      ROOT scatter = (s32[3,3], s32[3,3]) scatter(operand0, operand1, indices,
                                                  update0, update1),
  ...
  RunAndFilecheckHloRewrite(kModuleStr, ScatterSimplifier(), R"(
      ...
      CHECK: %[[RESHAPED_UPDATES0:.*]] = s32[2,1,3]{2,1,0} reshape(%update0)
      CHECK: %[[RESHAPED_UPDATES1:.*]] = s32[2,1,3]{2,1,0} reshape(%update1)
      CHECK: ROOT %{{.*}} = (s32[3,3]{1,0}, s32[3,3]{1,0}) scatter(
...
TEST_F(ScatterSimplifierTest, CollapsesScatterDims) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    scatter_computation {
      %p0 = s32[] parameter(0)
      ROOT result = s32[] parameter(1)
    }

    ENTRY kernel_entry {
      operand = s32[3,3] parameter(0)
      indices = s32[2,1,2] parameter(1)
      update = s32[2,1,1,3] parameter(2)
      ROOT scatter = s32[3,3] scatter(operand, indices, update),
          to_apply=scatter_computation,
  ...
  RunAndFilecheckHloRewrite(kModuleStr, ScatterSimplifier(), R"(
           ...
           CHECK: %[[RESHAPED_UPDATES:.*]] = s32[2,1,3]{2,1,0} reshape(%update)
           CHECK: scatter(
...
TEST_F(ScatterSimplifierTest, TransformsUpdatesAndOperandUsingScatterDims) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      operand = s32[3,4,5] parameter(0)
      indices = s32[2,2] parameter(1)
      update = s32[2,1,1,3] parameter(2)
      ROOT scatter = s32[3,4,5] scatter(operand, indices, update),
          to_apply=scatter_computation,
  ...
  RunAndFilecheckHloRewrite(kModuleStr, ScatterSimplifier(), R"(
           CHECK: %[[T_OPERAND:.*]] = s32[5,3,4]{2,1,0} transpose(%operand),
      CHECK-SAME:     dimensions={2,0,1}
           CHECK: %[[T_UPDATES:.*]] = s32[2,3,1,1]{3,2,1,0} transpose(%update),
...
TEST_F(ScatterSimplifierTest, MakesScatterDimensionsLeadingInUpdates) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      ...
      update = s32[2,1] parameter(2)
      ROOT scatter = s32[3] scatter(operand, indices, update),
          to_apply=scatter_computation,
  ...
  RunAndFilecheckHloRewrite(kModuleStr, ScatterSimplifier(), R"(
           CHECK: %[[TRANSPOSED_UPDATES:.*]] = s32[1,2]{1,0}
...
TEST_F(ScatterSimplifierTest, ZeroDimScatterIndices) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      ...
      update = s32[3,3] parameter(2)
      ROOT scatter = s32[4,4]{1,0} scatter(operand, indices, update),
...
TEST_F(ScatterSimplifierTest,
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      ...
      indices = s32[1,1] parameter(1)
      update = s32[1,2,2] parameter(2)
      ROOT scatter = s32[3,2] scatter(operand, indices, update),
          to_apply=scatter_computation,
...
TEST_F(ScatterSimplifierTest, VariadicScatterIntoScalar) {
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      operand0 = s32[] parameter(0)
      operand1 = s16[] parameter(1)
      indices = s32[0]{0} parameter(2)
      updates0 = s32[] parameter(3)
      updates1 = s16[] parameter(4)
      ROOT scatter = (s32[], s16[]) scatter(operand0, operand1, indices, updates0, updates1),
          update_window_dims={},
          inserted_window_dims={},
          scatter_dims_to_operand_dims={},
          index_vector_dim=0,
  ...
  RunAndFilecheckHloRewrite(kModuleStr, ScatterSimplifier(), R"(
    CHECK: ENTRY
    CHECK: %[[UPDATES0:.*]] = s32[] parameter(3)
    CHECK: %[[UPDATES1:.*]] = s16[] parameter(4)
    CHECK: ROOT %{{.*}} = (s32[], s16[]) tuple(%[[UPDATES0]], %[[UPDATES1]])
  )");
}
...
TEST_F(SimpleScatterExampleTest, 1x1d) {
  constexpr std::string_view hlo_text = R"(
    HloModule scatter_simplifier
