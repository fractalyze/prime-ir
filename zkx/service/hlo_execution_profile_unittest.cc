...
TEST_F(HloExecutionProfileTest, Basic) {
  ...
  ENTRY entry_computation {
    lhs = s32[30,30]{1,0} parameter(0)
    rhs = s32[30,30]{1,0} parameter(1)
    add = s32[30,30]{1,0} add(lhs, rhs)
    ROOT dot = s32[30,30]{1,0} dot(lhs, add), lhs_contracting_dims={1}, rhs_contracting_dims={0}
  })")
                        .value();
  const HloInstruction* dot_instruction =
      hlo_module->entry_computation()->root_instruction();
  const HloInstruction* add_instruction = dot_instruction->operand(1);
  Shape shape = ShapeUtil::MakeShape(S32, {30, 30});
