...
bool IsCommand(const HloComputation* computation,
...
bool IsConstant(const HloInstruction* hlo) {
  return HloPredicateIsOp<HloOpcode::kConstant>(hlo);
}

bool IsParameter(const HloInstruction* hlo) {
...
bool IsAsyncStartCommand(const HloInstruction* hlo,
                         const CommandBufferConfig& config) {
  ...
  if (HloPredicateIsOp<HloOpcode::kAsyncStart>(hlo)) {
    // ZKX: Removed IsCublasGemm check (ML op not applicable to ZKX).
...
bool IsAsyncDoneCommand(const HloInstruction* hlo,
                        const CommandBufferConfig& config) {
...
bool IsCommand(const HloInstruction*, const CommandBufferConfig&);
...
bool IsCommand(const HloCustomCallInstruction* hlo,
               const CommandBufferConfig& config) {
  // ZKX: Removed cuBLAS/cuDNN/cuBLASLt checks (ML ops not applicable to ZKX).
  // XLA had: IsLegacyCublasMatmul, IsCublasLtMatmul, IsCublasLtMatmulF8,
  //          IsCustomCallTofMHA checks here.
  ...
  // TODO(chokobole): Implement this. Dependency: FFI
  return false;
}

bool IsCommand(const HloInstruction* hlo, const CommandBufferConfig& config) {
  if (auto* fusion = DynCast<HloFusionInstruction>(hlo)) {
    auto gpu_config = fusion->backend_config<GpuBackendConfig>();
    const FusionBackendConfig& backend_config =
        gpu_config->fusion_backend_config();
    // ZKX: Removed kCuDnnFusionKind check (cuDNN fusion not applicable to ZKX).
...
void RemoveTrailingNoOps(HloInstructionSequence& seq) {
...
absl::StatusOr<bool> CommandBufferScheduling::Run(
  ...
  // ZKX: Changed xla::Internal() to absl::InternalError() (ZKX convention).
  if (!module->has_schedule())
    return absl::InternalError("module is not scheduled");

  const DebugOptions& debug_options = module->config().debug_options();

  absl::flat_hash_set<DebugOptions::CommandBufferCmdType> commands;
  for (auto cmd_type : debug_options.zkx_gpu_enable_command_buffer()) {
  ...
  static constexpr auto kRequireConditionals = {DebugOptions::CONDITIONAL,
                                                DebugOptions::WHILE};
  // ZKX: Removed CUBLAS, CUBLASLT, CUDNN from kRequireTracing
  // (ML ops not applicable to ZKX).
  static constexpr auto kRequireTracing = {DebugOptions::CUSTOM_CALL,
  ...
  for (HloComputation* comp : order) {
    ...
    std::vector<HloInstructionSequence> sequences =
        CollectCommandBufferSequences(
            ...
            debug_options.zkx_gpu_graph_min_graph_size());
