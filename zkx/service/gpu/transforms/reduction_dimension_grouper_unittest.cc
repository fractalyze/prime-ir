...
class ReductionDimensionGrouperTest : public HloHardwareIndependentTestBase {
 public:
  void CheckDimensionGrouper(std::string_view hlo,
...
ENTRY main {
  input = s32[100,10,32,3]{3,2,1,0} parameter(0)
  zero = s32[] constant(0)

  ROOT out = s32[100,10]{0,1} reduce(input, zero), dimensions={2,3}, to_apply=add
}
)";

  CheckDimensionGrouper(hlo,
                        R"(
// CHECK:  [[input_0:%[^ ]+]] = s32[100,10,32,3]{3,2,1,0} parameter(0)
// CHECK:  [[bitcast_1:%[^ ]+]] = s32[100,10,96]{2,1,0} bitcast([[input_0]])
// CHECK:  ROOT [[out_1_2:%[^ ]+]] = s32[100,10]{0,1} reduce([[bitcast_1]], [[zero_3:%[^ ]+]]), dimensions={2}, to_apply=[[add_4:%[^ ]+]]
      )");
}
...
ENTRY main {
  ...
  ROOT out = (s32[100,10]{1,0}, u32[100,10]{1,0}) reduce(input, idxs, zero, zero_idx), dimensions={2,3}, to_apply=argmax
}
)";
...
// CHECK:  ROOT [[out_1_4:%[^ ]+]] = (s32[100,10]{1,0}, u32[100,10]{1,0}) reduce([[bitcast_1]], [[bitcast_1_3]], [[zero_5:%[^ ]+]], [[zero_idx_6:%[^ ]+]]), dimensions={2}, to_apply=[[argmax_7:%[^ ]+]]
)");
}
