...
class ReductionDegenerateDimRemoverTest
 ...
 public:
  void CheckDegenerateDimRemover(std::string_view hlo,
...
ENTRY main {
  input = s32[1,3,1,4,1,5,1] parameter(0)
  zero = s32[] constant(0)

  ROOT out = s32[1,1,1,1] reduce(input, zero), dimensions={1,3,5}, to_apply=add
}

)";

  CheckDegenerateDimRemover(hlo, R"(
// CHECK: [[bitcast_0:%[^ ]+]] = s32[3,4,5]{2,1,0} bitcast([[input_1:%[^ ]+]])
// CHECK: [[reduce_2:%[^ ]+]] = s32[] reduce([[bitcast_0]], [[zero_3:%[^ ]+]]), dimensions={0,1,2}, to_apply=[[add_4:%[^ ]+]]
// CHECK: ROOT [[bitcast_1_5:%[^ ]+]] = s32[1,1,1,1]{3,2,1,0} bitcast([[reduce_2]])
  )");
}
...
ENTRY main {
  ...
  ROOT out = (s32[1,1,1,1], u32[1,1,1,1]) reduce(input, idxs, zero, zero_idx), dimensions={1,3,5}, to_apply=argmax
}
...
// CHECK:  [[reduce_4:%[^ ]+]] = (s32[], u32[]) reduce([[bitcast_0]], [[bitcast_1_2]], [[zero_5:%[^ ]+]], [[zero_idx_6:%[^ ]+]]), dimensions={0,1,2}, to_apply=[[argmax_7:%[^ ]+]]
// CHECK-NEXT:  [[get_tuple_element_8:%[^ ]+]] = s32[] get-tuple-element([[reduce_4]]), index=0
// CHECK-NEXT:  [[bitcast_2_9:%[^ ]+]] = s32[1,1,1,1]{3,2,1,0} bitcast([[get_tuple_element_8]])
...
// CHECK-NEXT:  ROOT [[tuple_12:%[^ ]+]] = (s32[1,1,1,1]{3,2,1,0}, u32[1,1,1,1]{3,2,1,0}) tuple([[bitcast_2_9]], [[bitcast_3_11]])
)");
}
...
ENTRY main {
  ...
  ROOT out = s32[3,4,5,1] reduce(input, zero), dimensions={0,2,4}, to_apply=add
}
)";

  CheckDegenerateDimRemover(hlo,
                            R"(
// CHECK: ROOT [[bitcast_0:%[^ ]+]] = s32[3,4,5,1]{3,2,1,0} bitcast([[input_1:%[^ ]+]])
      )");
}
