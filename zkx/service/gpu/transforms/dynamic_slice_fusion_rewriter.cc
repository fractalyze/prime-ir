...
bool IsCustomCall(const HloInstruction* hlo, std::string_view platform_name) {
...
// non-sliced dimensions being a multiple of `kZkxAllocatedBufferAlignBytes`.
bool IsAlignedSlice(const HloInstruction* slice) {
  ...
  for (auto dim : slice_shape.layout().minor_to_major()) {
    if ((strides.value()[dim] % kZkxAllocatedBufferAlignBytes) == 0) {
      return true;
    }
...
absl::Status CreateRootTuple(
  ...
  for (const auto& sliced_user_path : sliced_user_paths) {
