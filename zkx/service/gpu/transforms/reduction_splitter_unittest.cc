...
class ReductionSplitterTest : public HloHardwareIndependentTestBase {
 public:
  using HloHardwareIndependentTestBase::HloHardwareIndependentTestBase;
...
TEST_F(ReductionSplitterTest, SplitReductionAtDimensionTwo) {
  ...
  ENTRY entry_computation {
    param_0 = s16[6,16,512,64]{3,2,1,0} parameter(0)
    transpose.1781 = s16[6,512,16,64]{3,1,2,0} transpose(param_0), dimensions={0,2,1,3}
    convert.6986 = s32[6,512,16,64]{3,1,2,0} convert(transpose.1781)
    bitcast.2136 = s32[6,16,512,64]{3,2,1,0} bitcast(convert.6986)
    constant_11111 = s32[] constant(0)
    ROOT reduce.982 = s32[16,64]{1,0} reduce(bitcast.2136, constant_11111), dimensions={0,2}, to_apply=add_computation
  }
  )")
                    .value();
  ...
  EXPECT_THAT(pre_reduction->shape(), ShapeUtil::MakeShape(S32, {6, 16, 64}));
  EXPECT_THAT(root_reduction->dimensions(), std::vector<int64_t>({0}));
  EXPECT_THAT(root_reduction->shape(), ShapeUtil::MakeShape(S32, {16, 64}));
}
...
TEST_F(ReductionSplitterTest, SplitReductionAtDimensionZero) {
  ...
  ENTRY entry_computation {
    param_0 = s32[1024,16,512,64,128]{4,3,2,1,0} parameter(0)
    constant_11111 = s32[] constant(0)
    ROOT reduce.982 = s32[16,64]{1,0} reduce(param_0, constant_11111), dimensions={2,0,4}, to_apply=add_computation
  }
  )")
                    .value();
  ...
  EXPECT_THAT(pre_reduction->shape(),
              ShapeUtil::MakeShape(S32, {16, 512, 64, 128}));
...
TEST_F(ReductionSplitterTest, DontSplitReductionWithSmallDimensions) {
  ...
  ENTRY entry_computation {
    param_0 = s32[16,8,1024,8]{3,2,1,0} parameter(0)
    constant_11111 = s32[] constant(0)
    ROOT reduce.982 = s32[16,1024]{1,0} reduce(param_0, constant_11111), dimensions={3,1}, to_apply=add_computation
  }
  )")
                    .value();
...
TEST_F(ReductionSplitterTest, DontSplitReductionsWithContiguousDimensions) {
  ...
  ENTRY entry_computation {
    param_0 = s32[128,128,64,128]{3,2,1,0} parameter(0)
    constant_11111 = s32[] constant(0)
    // The dimensions to keep (1 and 2) are contiguous.
    ROOT reduce.982 = s32[128,64]{1,0} reduce(param_0, constant_11111), dimensions={3,0}, to_apply=add_computation
  }
  )")
                    .value();
