...
TEST_F(GpuFusibleTest, IsPhysicallyTransposing_ElementwiseProducer) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ENTRY entry {
      ...
      c0 = s32[] constant(0)
      neg = s32[2,2,2]{2,1,0} negate(p0)
      ROOT reduce = s32[2,2]{1,0} reduce(neg, c0), dimensions={2}, to_apply=scalar_add
    })"))
                    .value();
  SCOPED_TRACE(module->ToString());
  const HloInstruction* neg =
      module->entry_computation()->root_instruction()->operand(0);
  ASSERT_EQ(neg->opcode(), HloOpcode::kNegate);
  EXPECT_FALSE(IsPhysicallyTransposing(*neg));
}

TEST_F(GpuFusibleTest, IsPhysicallyTransposing_MixedLayoutProducer) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    mixed_input_layouts_computation {
      p0.1 = s16[128,1024,32,32]{1,3,2,0} parameter(0)
      p1.1 = s16[128,1024,32,32]{3,2,1,0} parameter(1)
      copy = s16[128,1024,32,32]{1,3,2,0} copy(p1.1)
      c0 = s16[] constant(0)
      broadcast = s16[128,1024,32,32]{1,3,2,0} broadcast(c0), dimensions={}
      greater-than = pred[128,1024,32,32]{1,3,2,0} compare(copy, broadcast), direction=GT
      ROOT root = s16[128,1024,32,32]{1,3,2,0} select(greater-than, p0.1, broadcast)
    }
    fused_reduce {
      p0.2 = s16[128,1024,32,32]{1,3,2,0} parameter(0)
      convert = s32[128,1024,32,32]{1,3,2,0} convert(p0.2)
      c0.2 = s32[] constant(0)
      ROOT reduce = s32[1024]{0} reduce(convert, c0.2), dimensions={0,2,3}, to_apply=scalar_add
    }
    ENTRY entry {
      p0 = s16[128,1024,32,32]{1,3,2,0} parameter(0)
      p1 = s16[128,1024,32,32]{3,2,1,0} parameter(1)
      loop_fusion = s16[128,1024,32,32]{1,3,2,0} fusion(p0, p1), kind=kLoop, calls=mixed_input_layouts_computation
      reduce_fusion = s32[1024]{0} fusion(loop_fusion), kind=kInput, calls=fused_reduce
      ROOT root = (s32[1024]{0}, s16[128,1024,32,32]{1,3,2,0}) tuple(reduce_fusion, loop_fusion)
    })"))
                    .value();
  SCOPED_TRACE(module->ToString());
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    mixed_input_layouts_computation {
      p0.1 = s16[128,1,32,32]{1,3,2,0} parameter(0)
      p1.1 = s16[128,1,32,32]{3,2,1,0} parameter(1)
      bitcast = s16[128,1,32,32]{1,3,2,0} bitcast(p1.1)
      c0 = s16[] constant(0)
      broadcast = s16[128,1,32,32]{1,3,2,0} broadcast(c0), dimensions={}
      greater-than = pred[128,1,32,32]{1,3,2,0} compare(bitcast, broadcast), direction=GT
      ROOT root = s16[128,1,32,32]{1,3,2,0} select(greater-than, p0.1, broadcast)
    }
    fused_reduce {
      p0.2 = s16[128,1,32,32]{1,3,2,0} parameter(0)
      convert = s32[128,1,32,32]{1,3,2,0} convert(p0.2)
      c0.2 = s32[] constant(0)
      ROOT reduce = s32[1]{0} reduce(convert, c0.2), dimensions={0,2,3}, to_apply=scalar_add
    }
    ENTRY entry {
      p0 = s16[128,1,32,32]{1,3,2,0} parameter(0)
      p1 = s16[128,1,32,32]{3,2,1,0} parameter(1)
      loop_fusion = s16[128,1,32,32]{1,3,2,0} fusion(p0, p1), kind=kLoop, calls=mixed_input_layouts_computation
      reduce_fusion = s32[1]{0} fusion(loop_fusion), kind=kInput, calls=fused_reduce
      ROOT root = (s32[1]{0}, s16[128,1,32,32]{1,3,2,0}) tuple(reduce_fusion, loop_fusion)
    })"))
                    .value();
  SCOPED_TRACE(module->ToString());
  const HloInstruction* loop_fusion =
...
TEST_F(GpuFusibleTest, IsPhysicallyTransposing_CopyProducer) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduce {
      p0.1 = s32[128,1024,32,32]{1,3,2,0} parameter(0)
      c0.1 = s32[] constant(0)
      ROOT reduce = s32[1024]{0} reduce(p0.1, c0.1), dimensions={0,2,3}, to_apply=scalar_add
    }
    ENTRY entry {
      p0 = s32[128,1024,32,32]{3,2,1,0} parameter(0)
      copy = s32[128,1024,32,32]{1,3,2,0} copy(p0)
      ROOT reduce_fusion = s32[1024]{0} fusion(copy), kind=kInput, calls=fused_reduce
    })"))
                    .value();
  SCOPED_TRACE(module->ToString());
...
TEST_F(GpuFusibleTest, IsPhysicallyTransposing_PhysicalTranspose) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduce {
      p0.1 = s32[1024,128,32,32]{3,2,1,0} parameter(0)
      c0.1 = s32[] constant(0)
      ROOT reduce = s32[1024]{0} reduce(p0.1, c0.1), dimensions={1,2,3}, to_apply=scalar_add
    }
    ENTRY entry {
      p0 = s32[128,1024,32,32]{3,2,1,0} parameter(0)
      copy = s32[1024,128,32,32]{3,2,1,0} transpose(p0), dimensions={1,0,2,3}
      ROOT reduce_fusion = s32[1024]{0} fusion(copy), kind=kInput, calls=fused_reduce
    })"))
                    .value();
  SCOPED_TRACE(module->ToString());
...
TEST_F(GpuFusibleTest, IsPhysicallyTransposing_LayoutChangingFusionProducer) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    layout_changing_computation {
      p0.1 = s16[128,1024,32,32]{3,2,1,0} parameter(0)
      p1.1 = s16[128,1024,32,32]{3,2,1,0} parameter(1)
      c0 = s16[] constant(0)
      broadcast = s16[128,1024,32,32]{3,2,1,0} broadcast(c0), dimensions={}
      greater-than = pred[128,1024,32,32]{3,2,1,0} compare(p1.1, broadcast), direction=GT
      select = s16[128,1024,32,32]{3,2,1,0} select(greater-than, p0.1, broadcast)
      ROOT root = s16[128,1024,32,32]{1,3,2,0} copy(select)
    }
    fused_reduce {
      p0.2 = s16[128,1024,32,32]{1,3,2,0} parameter(0)
      convert = s32[128,1024,32,32]{1,3,2,0} convert(p0.2)
    ...
    ENTRY entry {
      p0 = s16[128,1024,32,32]{3,2,1,0} parameter(0)
      p1 = s16[128,1024,32,32]{3,2,1,0} parameter(1)
      loop_fusion = s16[128,1024,32,32]{1,3,2,0} fusion(p0, p1), kind=kLoop, calls=layout_changing_computation
      ROOT reduce_fusion = s32[1024]{0} fusion(loop_fusion), kind=kInput, calls=fused_reduce
    })"))
                    .value();
  SCOPED_TRACE(module->ToString());
  const HloInstruction* loop_fusion =
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    broadcasting_computation {
      p0.1 = s32[128,1024,32,32]{1,3,2,0} parameter(0)
      p1.1 = s32[1,128,1,1]{3,2,1,0} parameter(1)
      reshape = s32[128]{0} reshape(p1.1)
      broadcast = s32[128,1024,32,32]{1,3,2,0} broadcast(reshape), dimensions={0}
      ROOT add = s32[128,1024,32,32]{1,3,2,0} add(p0.1, broadcast)
    }
    ENTRY entry {
      p0 = s32[128,1024,32,32]{1,3,2,0} parameter(0)
      p1 = s32[1,128,1,1]{3,2,1,0} parameter(1)
      loop_fusion = s32[128,1024,32,32]{1,3,2,0} fusion(p0, p1), kind=kLoop, calls=broadcasting_computation
      c0.2 = s32[] constant(0)
      ROOT reduce = s32[1024]{0} reduce(loop_fusion, c0.2), dimensions={0,2,3}, to_apply=scalar_add
    })"))
                    .value();
  SCOPED_TRACE(module->ToString());
  const HloInstruction* loop_fusion =
...
TEST_F(GpuFusibleTest, TransposesMinorDimension) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ENTRY entry {
      default_layout = s32[10,20,30,40]{3,2,1,0} parameter(0)
      non_default_layout = s32[10,20,30,40]{1,2,3,0} parameter(1)

      transpose_minor_default = s32[10,20,40,30]{3,2,1,0} transpose(default_layout), dimensions={0,1,3,2}
      no_transpose_minor_default = s32[10,20,40,30]{2,3,1,0} transpose(default_layout), dimensions={0,1,3,2}
      transpose_major_default = s32[10,30,20,40]{3,2,1,0} transpose(default_layout), dimensions={0,2,1,3}

      transpose_minor_non_default = s32[10,30,20,40]{1,2,3,0} transpose(non_default_layout), dimensions={0,2,1,3}
      no_transpose_minor_non_default = s32[10,20,40,30]{1,2,0,3} transpose(non_default_layout), dimensions={0,1,3,2}
      transpose_major_non_default = s32[10,20,40,30]{1,2,3,0} transpose(non_default_layout), dimensions={0,1,3,2}
...
TEST_F(GpuFusibleTest, TransposesMinorDimensionSkipTrivialDimensions) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ENTRY entry {
      default_layout = s32[10,20,1,1]{3,2,1,0} parameter(0)
      non_default_layout = s32[10,20,1,1]{1,2,3,0} parameter(1)

      // Only trivial dimensions are swapped.
      transpose_minor_default = s32[10,20,1,1]{3,2,1,0} transpose(default_layout), dimensions={0,1,3,2}
      // The first non-trivial dimension is still the same in input and output.
      transpose_nontrivial_minor_default = s32[10,1,20,1]{3,2,1,0} transpose(default_layout), dimensions={0,2,1,3}
      no_transpose_minor_default = s32[10,20,1,1]{2,3,1,0} transpose(default_layout), dimensions={0,1,3,2}
      // We swap the most major dimension with a trivial dimension.
      transpose_one_major_default = s32[1,20,10,1]{3,2,1,0} transpose(default_layout), dimensions={2,1,0,3}
      // The first two non-trivial dimensions are swapped.
      transpose_two_major_default = s32[20,10,1,1]{3,2,1,0} transpose(default_layout), dimensions={1,0,2,3}

      transpose_minor_non_default = s32[10,1,20,1]{1,2,3,0} transpose(non_default_layout), dimensions={0,2,1,3}
      no_transpose_minor_non_default = s32[10,20,1,1]{1,2,0,3} transpose(non_default_layout), dimensions={0,1,3,2}
      transpose_major_non_default = s32[10,20,1,1]{1,2,3,0} transpose(non_default_layout), dimensions={0,1,3,2}
...
TEST_F(GpuFusibleTest, CopyTransposesMinorDimension) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ENTRY entry {
      ...
      copy_transpose_minor_default = s32[10,20,30,40]{2,3,1,0} copy(default_layout)
      copy_no_transpose_minor_default = s32[10,20,30,40]{3,2,1,0} copy(default_layout)

      copy_transpose_minor_non_default = s32[10,20,30,40]{2,1,3,0} copy(non_default_layout)
      copy_no_transpose_minor_non_default = s32[10,20,30,40]{1,2,3,0} copy(non_default_layout)
...
TEST_F(GpuFusibleTest, CopyTransposesMinorDimensionSkipTrivialDimensions) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ENTRY entry {
      ...
      copy_transpose_minor_default = s32[10,20,1,1]{2,3,1,0} copy(default_layout)
      copy_no_transpose_minor_default = s32[10,20,1,1]{3,2,1,0} copy(default_layout)

      copy_transpose_minor_non_default = s32[10,20,1,1]{2,0,3,1} copy(non_default_layout)
      copy_no_transpose_minor_non_default = s32[10,20,1,1]{1,2,3,0} copy(non_default_layout)
...
TEST_F(GpuFusibleTest, IsReduceInputFusion_ReductionToVector) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ENTRY entry {
      ...
      ROOT reduce = s32[512]{0} reduce(p1, c0), dimensions={0,2,3}, to_apply=scalar_add
    })"))
                    .value();
  SCOPED_TRACE(module->ToString());
...
TEST_F(GpuFusibleTest, IsReduceInputFusion_ElementalReduction) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ENTRY entry {
      c0 = s32[] parameter(0)
      p1 = s32[8,512,5,16,1,1]{5,4,3,2,1,0} parameter(1)
      ROOT reduce = s32[512,5,1,1]{3,2,1,0} reduce(p1, c0), dimensions={3,0},
        to_apply=scalar_add
    })"))
                    .value();
  SCOPED_TRACE(module->ToString());
...
TEST_F(GpuFusibleTest, IsReduceInputFusion_SingleOutputInputReduceFusion) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduction {
      c0 = s32[] constant(0)
      p1 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      ROOT reduce = s32[128,512]{1,0} reduce(p1, c0), dimensions={2,3}, to_apply=scalar_add
    }
    ENTRY entry {
      p0 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      ROOT fusion = s32[128,512]{1,0} fusion(p0), kind=kInput, calls=fused_reduction
    })"))
                    .value();
  const HloInstruction* reduce =
      module->entry_computation()->root_instruction();
...
TEST_F(GpuFusibleTest, IsReduceInputFusion_SingleOutputLoopReduceFusion) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduction {
      c0 = s32[] constant(0)
      p1 = s32[8,512,5,16,1,1]{5,4,3,2,1,0} parameter(0)
      ROOT reduce = s32[8,5,1,1]{3,2,1,0} reduce(p1, c0), dimensions={1,3}, to_apply=scalar_add
    }
    ENTRY entry {
      p0 = s32[8,512,5,16,1,1]{5,4,3,2,1,0} parameter(0)
      ROOT fusion = s32[8,5,1,1]{3,2,1,0} fusion(p0), kind=kLoop, calls=fused_reduction
    })"))
                    .value();
  const HloInstruction* reduce =
      module->entry_computation()->root_instruction();
...
TEST_F(GpuFusibleTest, IsReduceInputFusion_MultiOutputInputReduceFusion) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduction {
      c0 = s32[] constant(0)
      p1 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      reduce.0 = s32[128,512]{1,0} reduce(p1, c0), dimensions={2,3}, to_apply=scalar_add
      reduce.1 = s32[128,512]{1,0} reduce(p1, c0), dimensions={2,3}, to_apply=scalar_add
      ROOT root = (s32[128,512]{1,0}, s32[128,512]{1,0}) tuple(reduce.0, reduce.1)
    }
    ENTRY entry {
      p0 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      ROOT fusion = (s32[128,512]{1,0}, s32[128,512]{1,0}) fusion(p0), kind=kInput, calls=fused_reduction
    })"))
                    .value();
  const HloInstruction* reduce =
      module->entry_computation()->root_instruction();
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduction {
      c0 = s32[] constant(0)
      p1 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      reduce = s32[128,512]{1,0} reduce(p1, c0), dimensions={2,3}, to_apply=scalar_add
      mul = s32[128,512,28,28]{3,2,1,0} multiply(p1, p1)
      ROOT root = (s32[128,512]{1,0}, s32[128,512,28,28]{3,2,1,0}) tuple(reduce, mul)
    }
    ENTRY entry {
      p0 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      ROOT fusion = (s32[128,512]{1,0}, s32[128,512,28,28]{3,2,1,0}) fusion(p0), kind=kInput, calls=fused_reduction
    })"))
                    .value();
  const HloInstruction* reduce =
      module->entry_computation()->root_instruction();
...
TEST_F(GpuFusibleTest, IsReduceInputFusion_MultiOutputLoopReduceFusion) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduction {
      c0 = s32[] constant(0)
      p1 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      reduce.0 = s32[512,28]{1,0} reduce(p1, c0), dimensions={0,2}, to_apply=scalar_add
      reduce.1 = s32[512,28]{1,0} reduce(p1, c0), dimensions={0,2}, to_apply=scalar_add
      ROOT root = (s32[512,28]{1,0}, s32[512,28]{1,0}) tuple(reduce.0, reduce.1)
    }
    ENTRY entry {
      p0 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      ROOT fusion = (s32[512,28]{1,0}, s32[512,28]{1,0}) fusion(p0), kind=kLoop, calls=fused_reduction
    })"))
                    .value();
  const HloInstruction* reduce =
      module->entry_computation()->root_instruction();
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduction {
      c0 = s32[] constant(0)
      p1 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      reduce = s32[512,28]{1,0} reduce(p1, c0), dimensions={0,2}, to_apply=scalar_add
      mul = s32[128,512,28,28]{3,2,1,0} multiply(p1, p1)
      ROOT root = (s32[512,28]{1,0}, s32[128,512,28,28]{3,2,1,0}) tuple(reduce, mul)
    }
    ENTRY entry {
      p0 = s32[128,512,28,28]{3,2,1,0} parameter(0)
      ROOT fusion = (s32[512,28]{1,0}, s32[128,512,28,28]{3,2,1,0}) fusion(p0), kind=kLoop, calls=fused_reduction
    })"))
                    .value();
  const HloInstruction* reduce =
      module->entry_computation()->root_instruction();
...
TEST_F(GpuFusibleTest, FusionHeroesAreCompatible_TransposeFusionCompatible) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_computation_1 {
      p0.1 = s32[64,32]{1,0} parameter(0)
      neg = s32[64,32]{1,0} negate(p0.1)
      ROOT transpose = s32[32,64]{1,0} transpose(neg), dimensions={1,0}
    }

    fused_computation_2 {
      p0.2 = s32[32,64]{1,0} parameter(0)
      neg = s32[32,64]{1,0} negate(p0.2)
      ROOT add = s32[32,64]{1,0} add(neg, neg)
    }

    ENTRY entry {
      p0 = s32[64,32]{1,0} parameter(0)
      fusion.1 = s32[32,64]{1,0} fusion(p0), kind=kLoop, calls=fused_computation_1
      ROOT fusion.2 = s32[32,64]{1,0} fusion(fusion.1), kind=kLoop, calls=fused_computation_2
    })"))
                    .value();
...
TEST_F(GpuFusibleTest, FusionHeroesAreCompatible_TransposeFusionNotCompatible) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_computation_1 {
      p0.1 = s32[64,32]{1,0} parameter(0)
      neg = s32[64,32]{1,0} negate(p0.1)
      bc = s32[1,64,32]{2,1,0} bitcast(neg)
      transpose = s32[1,32,64]{2,1,0} transpose(bc), dimensions={0,2,1}
      ROOT bc2 = s32[32,64]{1,0} bitcast(transpose)
    }

    fused_computation_2 {
      p0.2 = s32[32,64]{1,0} parameter(0)
      broadcast = s32[32,64,4]{2,1,0} broadcast(p0.2), dimensions={0,1}
      ROOT add = s32[32,64,4]{2,1,0} add(broadcast, broadcast)
    }

    ENTRY entry {
      p0 = s32[64,32]{1,0} parameter(0)
      fusion.1 = s32[32,64]{1,0} fusion(p0), kind=kLoop, calls=fused_computation_1
      ROOT fusion.2 = s32[32,64,4]{2,1,0} fusion(fusion.1), kind=kLoop, calls=fused_computation_2
    })"))
                    .value();
  const HloInstruction* fusion_1 =
      module->entry_computation()->root_instruction();
...
TEST_F(GpuFusibleTest, ShapesCompatibleForMultiOutputFusion_IgnoreElementType) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_computation_1 {
      p0.1 = s32[6400]{0} parameter(0)
      ROOT mul = s32[6400]{0} multiply(p0.1, p0.1)
    ...
    fused_computation_2 {
      p0.2 = s32[6400]{0} parameter(0)
      ROOT convert = s16[6400]{0} convert(p0.2)
    }

    ENTRY entry {
      p0 = s32[6400]{0} parameter(0)
      fusion.1 = s32[6400]{0} fusion(p0), kind=kLoop, calls=fused_computation_1
      fusion.2 = s16[6400]{0} fusion(p0), kind=kLoop, calls=fused_computation_2
      ROOT root = (s32[6400]{0}, s16[6400]{0}) tuple(fusion.1, fusion.2)
    })"))
                    .value();
  const HloInstruction* fusion_1 =
      module->entry_computation()->root_instruction()->operand(0);
...
TEST_F(GpuFusibleTest, ShapesCompatibleForMultiOutputFusion_BitcastCompatible) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ...
    fused_computation_2 {
      p0.2 = s32[6400]{0} parameter(0)
      bitcast = s32[1,6400]{1,0} bitcast(p0.2)
      ROOT convert = s16[1,6400]{1,0} convert(bitcast)
    }

    ENTRY entry {
      p0 = s32[6400]{0} parameter(0)
      fusion.1 = s32[6400]{0} fusion(p0), kind=kLoop, calls=fused_computation_1
      fusion.2 = s16[1,6400]{1,0} fusion(p0), kind=kLoop, calls=fused_computation_2
      ROOT root = (s32[6400]{0}, s16[1,6400]{1,0}) tuple(fusion.1, fusion.2)
    })"))
                    .value();
  const HloInstruction* fusion_1 =
      module->entry_computation()->root_instruction()->operand(0);
...
TEST_F(GpuFusibleTest, ShapesCompatibleForMultiOutputFusion_Reduce) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ...
    ENTRY entry {
      ...
      reduce = s32[] reduce(p0, const.2), dimensions={0}, to_apply=scalar_add
      ROOT root = (s32[6400]{0}, s32[]) tuple(fusion.1, reduce)
    })"))
                    .value();
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_computation_1 {
      ...
      ROOT tuple = (s32[8,1,5,16,1,1]{5,4,3,2,1,0}, s32[8,1,5,16,1,1]{5,4,3,2,1,0}) tuple(mul, neg)
    }

    fused_computation_2 {
      p0.2 = s32[8,1,5,16,1,1]{5,4,3,2,1,0} parameter(0)
      const.2 = s32[] constant(0)
      broadcast = s32[8,1,5,16,1,1]{5,4,3,2,1,0} broadcast(const.2), dimensions={}
      ROOT add = s32[8,1,5,16,1,1]{5,4,3,2,1,0} add(p0.2, broadcast)
    }

    ENTRY entry {
      p0 = s32[8,1,5,16,1,1]{5,4,3,2,1,0} parameter(0)
      fusion.1 = (s32[8,1,5,16,1,1]{5,4,3,2,1,0}, s32[8,1,5,16,1,1]{5,4,3,2,1,0}) fusion(p0), kind=kLoop, calls=fused_computation_1
      ...
      ROOT root = (s32[8,1,5,16,1,1]{5,4,3,2,1,0}, s32[8,1,5,16,1,1]{5,4,3,2,1,0}, s32[8,1,5,16,1,1]{5,4,3,2,1,0}) tuple(gte0, gte1, fusion.2)
    })"))
                    .value();
  const HloInstruction* fusion_1 =
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ...
    fused_computation_2 {
      ...
      const.2 = s32[] constant(0)
      broadcast = s32[8,1,5,16,1,1]{5,4,3,2,1,0} broadcast(const.2), dimensions={}
      add = s32[8,1,5,16,1,1]{5,4,3,2,1,0} add(p0.2, broadcast)
      ROOT convert = s16[8,1,5,16,1,1]{5,4,3,2,1,0} convert(add)
    }

    ENTRY entry {
      p0 = s32[8,1,5,16,1,1]{5,4,3,2,1,0} parameter(0)
      fusion.1 = (s32[8,1,5,16,1,1]{5,4,3,2,1,0}, s32[8,1,5,16,1,1]{5,4,3,2,1,0}) fusion(p0), kind=kLoop, calls=fused_computation_1
      fusion.2 = s16[8,1,5,16,1,1]{5,4,3,2,1,0} fusion(p0), kind=kLoop, calls=fused_computation_2
      gte0 = s32[8,1,5,16,1,1]{5,4,3,2,1,0} get-tuple-element(fusion.1), index=0
      gte1 = s32[8,1,5,16,1,1]{5,4,3,2,1,0} get-tuple-element(fusion.1), index=1
      ROOT root = (s32[8,1,5,16,1,1]{5,4,3,2,1,0}, s32[8,1,5,16,1,1]{5,4,3,2,1,0}, s16[8,1,5,16,1,1]{5,4,3,2,1,0}) tuple(gte0, gte1, fusion.2)
    })"))
                    .value();
  const HloInstruction* fusion_1 =
      module->entry_computation()->root_instruction()->operand(0)->operand(0);
...
TEST_F(GpuFusibleTest, ShapesCompatibleForMultiOutputFusion_UnfusedOps) {
  ...
  const HloInstruction* neg =
      module->entry_computation()->root_instruction()->operand(1);
  EXPECT_TRUE(ShapesCompatibleForMultiOutputFusion(*reduce, *neg));
}
...
TEST_F(GpuFusibleTest, ShapesCompatibleForMultiOutputFusion_DifferentLayouts) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ENTRY reduce {
      p0 = s32[2,2,2]{2,1,0} parameter(0)
      p1 = s32[2,2,2]{0,1,2} parameter(1)
      c0 = s32[] constant(0)
      neg = s32[2,2,2]{2,1,0} negate(p0)
      reduce = s32[2,2]{0,1} reduce(p1, c0), dimensions={2}, to_apply=scalar_add
      ROOT root = (s32[2,2]{0,1}, s32[2,2,2]{2,1,0}) tuple(reduce, neg)
    })"))
                    .value();
  const HloInstruction* reduce =
      module->entry_computation()->root_instruction()->operand(0);
  const HloInstruction* neg =
      module->entry_computation()->root_instruction()->operand(1);
  EXPECT_FALSE(ShapesCompatibleForMultiOutputFusion(*reduce, *neg));
}
...
TEST_F(
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_021_transpose {
      param_0 = s32[20,20,20]{2,1,0} parameter(0)
      transpose = s32[20,20,20]{2,1,0} transpose(param_0), dimensions={0,2,1}
      ROOT bitcast = s32[8000]{0} bitcast(transpose)
    }

    fused_220_transpose {
      param_0 = s32[20,20,20]{2,1,0} parameter(0)
      transpose = s32[20,20,20]{2,1,0} transpose(param_0), dimensions={2,1,0}
      ROOT bitcast = s32[8000]{0} bitcast(transpose)
    }

    ENTRY reduce {
      p0 = s32[20,20,20]{2,1,0} parameter(0)
      fusion = s32[8000]{0} fusion(p0), kind=kInput, calls=fused_021_transpose
      fusion.1 = s32[8000]{0} fusion(p0), kind=kInput, calls=fused_220_transpose
      ROOT root = (s32[8000]{0}, s32[8000]{0}) tuple(fusion, fusion.1)
    })"))
                    .value();
  const HloInstruction* fusion_1 =
      module->entry_computation()->root_instruction()->operand(0);
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_1230_transpose {
      param_0 = s32[1,20,20]{2,1,0} parameter(0)
      bitcast.1 = s32[20,2,2,5]{3,2,1,0} bitcast(param_0)
      transpose = s32[2,2,5,20]{3,2,1,0} transpose(bitcast.1), dimensions={1,2,3,0}
      ROOT bitcast.2 = s32[400]{0} bitcast(transpose)
    }

    fused_021_transpose {
      param_0 = s32[1,20,20]{2,1,0} parameter(0)
      transpose = s32[1,20,20]{2,1,0} transpose(param_0), dimensions={0,2,1}
      ROOT bitcast = s32[400]{0} bitcast(transpose)
    }

    ENTRY reduce {
      p0 = s32[1,20,20]{2,1,0} parameter(0)
      fusion = s32[400]{0} fusion(p0), kind=kInput, calls=fused_1230_transpose
      fusion.1 = s32[400]{0} fusion(p0), kind=kInput, calls=fused_021_transpose
      ROOT root = (s32[400]{0}, s32[400]{0}) tuple(fusion, fusion.1)
    })"))
                    .value();
  const HloInstruction* fusion_1 =
      module->entry_computation()->root_instruction()->operand(0);
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_select {
      ...
      c0 = s32[] constant(0)
      broadcast = s32[2,2,2]{2,1,0} broadcast(s32[] c0), dimensions={}
      greater-than = pred[2,2,2]{2,1,0} compare(s32[2,2,2]{2,1,0} p1.1, s32[2,2,2]{2,1,0} broadcast), direction=GT
      p0.1 = s32[2,2,2]{2,1,0} parameter(0)
      ROOT select = s32[2,2,2]{2,1,0} select(pred[2,2,2]{2,1,0} greater-than, s32[2,2,2]{2,1,0} p0.1, s32[2,2,2]{2,1,0} broadcast)
    }

    fused_reduce {
      p0.2 = s32[2,2,2]{2,1,0} parameter(0)
      c1 = s32[] constant(0)
      r1 = s32[2,2]{1,0} reduce(p0.2, c1), dimensions={2}, to_apply=scalar_add
      mul = s32[2,2,2]{2,1,0} multiply(p0.2, p0.2)
      r2 = s32[2,2]{1,0} reduce(mul, c1), dimensions={2}, to_apply=scalar_add
      ROOT tuple = (s32[2,2]{1,0}, s32[2,2]{1,0}) tuple(r1, r2)
    }

    ENTRY reduce {
      p0 = s32[2,2,2]{2,1,0} parameter(0)
      p1 = s32[2,2,2]{2,1,0} parameter(1)
      select = s32[2,2,2]{2,1,0} fusion(p0, p1), kind=kLoop, calls=fused_select
      fusion = (s32[2,2]{1,0}, s32[2,2]{1,0}) fusion(select), kind=kInput, calls=fused_reduce
      gte0 = s32[2,2]{1,0} get-tuple-element(fusion), index=0
      gte1 = s32[2,2]{1,0} get-tuple-element(fusion), index=1
      ROOT root = (s32[2,2]{1,0}, s32[2,2]{1,0}, s32[2,2,2]{2,1,0}) tuple(gte1, gte1, select)
    })"))
                    .value();
  const HloInstruction* fusion_1 =
      module->entry_computation()->root_instruction()->operand(0)->operand(0);
...
TEST_F(GpuFusibleTest, ShapesCompatibleForMultiOutputFusion_ReduceFusions) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduce_1 {
      p0.1 = s32[2,2,2]{2,1,0} parameter(0)
      c0 = s32[] constant(0)
      ROOT reduce = s32[2,2]{1,0} reduce(s32[2,2,2]{2,1,0} p0.1, s32[] c0), dimensions={0}, to_apply=scalar_add
    }

    fused_reduce_2 {
      p0.2 = s32[2,2,2]{2,1,0} parameter(0)
      mul = s32[2,2,2]{2,1,0} multiply(s32[2,2,2]{2,1,0} p0.2, s32[2,2,2]{2,1,0} p0.2)
      c1 = s32[] constant(0)
      ROOT reduce = s32[2,2]{1,0} reduce(s32[2,2,2]{2,1,0} mul, s32[] c1), dimensions={0}, to_apply=scalar_add
    }

    ENTRY reduce {
      p0 = s32[2,2,2]{2,1,0} parameter(0)
      p1 = s32[2,2,2]{2,1,0} parameter(1)
      reduce_1 = s32[2,2]{1,0} fusion(p0), kind=kLoop, calls=fused_reduce_1
      reduce_2 = s32[2,2]{1,0} fusion(p1), kind=kLoop, calls=fused_reduce_2
      ROOT root = (s32[2,2]{1,0}, s32[2,2]{1,0}) tuple(reduce_1, reduce_2)
    })"))
                    .value();
  const HloInstruction* fusion_1 =
      module->entry_computation()->root_instruction()->operand(0);
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduce_1 {
      ...
      c0 = s32[] constant(0)
      ROOT reduce = s32[32,32]{1,0} reduce(s32[32,32,32]{2,1,0} p0.1, s32[] c0),
        dimensions={0}, to_apply=scalar_add
    }

    fused_reduce_2 {
      p0.2 = s32[32,32,32]{2,1,0} parameter(0)
      mul = s32[32,32,32]{2,1,0} multiply(s32[32,32,32]{2,1,0} p0.2,
        s32[32,32,32]{2,1,0} p0.2)
      c1 = s32[] constant(0)
      ROOT reduce = s32[32,32]{1,0} reduce(s32[32,32,32]{2,1,0} mul, s32[] c1),
    ...
    ENTRY reduce {
      ...
      reduce_1 = s32[32,32]{1,0} fusion(p0), kind=kLoop, calls=fused_reduce_1
      reduce_2 = s32[32,32]{1,0} fusion(p1), kind=kLoop, calls=fused_reduce_2
      ROOT root = (s32[32,32]{1,0}, s32[32,32]{1,0}) tuple(reduce_1, reduce_2)
    })"))
                    .value();
  const HloInstruction* fusion_1 =
      module->entry_computation()->root_instruction()->operand(0);
...
TEST_F(GpuFusibleTest,
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ...
    fused_reduce {
      ...
      mul = s32[32,32,32]{2,1,0} multiply(s32[32,32,32]{2,1,0} p0.2,
        s32[32,32,32]{2,1,0} p0.2)
      broadcast = s32[32,32,32,32]{3,2,1,0} broadcast(mul), dimensions={3,2,1}
      c1 = s32[] constant(0)
      // Note that reduce is not a reduction-to-vector.
      ROOT reduce = s32[32,32]{1,0} reduce(s32[32,32,32,32]{3,2,1,0} broadcast,
        s32[] c1), dimensions={1,3}, to_apply=scalar_add
    }

    ENTRY reduce {
      p0 = s32[32,32,32]{2,1,0} parameter(0)
      p1 = s32[32,32,32]{2,1,0} parameter(1)
      element_wise = s32[32,32,32]{2,1,0} fusion(p0, p1), kind=kLoop,
        calls=fused_element_wise
      fusion = s32[32,32]{1,0} fusion(element_wise),
        kind=kLoop, calls=fused_reduce
      ROOT root = (s32[32,32]{1,0}, s32[32,32,32]{2,1,0})
  ...
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    ...
    ENTRY reduce {
      ...
      fusion = (s32[32,32]{1,0}, s32[32,32]{1,0}) fusion(select), kind=kInput,
        calls=fused_reduce
      ROOT root = ((s32[32,32]{1,0}, s32[32,32]{1,0}), s32[32,32,32]{2,1,0}) tuple(fusion, select)
    })"))
                    .value();
  const HloInstruction* root = module->entry_computation()->root_instruction();
  const HloInstruction* consumer = root->operand(0);
...
TEST_F(GpuFusibleTest, ProducerConsumerFusionReduceUnfriendlyLoopFusion) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    mixed_input_layouts_computation {
      p0.1 = s16[128,1024,32,32]{1,3,2,0} parameter(0)
      p1.1 = s16[128,1024,33,33]{3,2,1,0} parameter(1)
      copy = s16[128,1024,33,33]{1,3,2,0} copy(p1.1)
      slice = s16[128,1024,32,32]{1,3,2,0} slice(copy), slice={[0:128],[0:1024],[0:32],[0:32]}
      c0 = s16[] constant(0)
      broadcast = s16[128,1024,32,32]{1,3,2,0} broadcast(c0), dimensions={}
    ...
    ENTRY reduce {
      p0 = s16[128,1024,32,32]{1,3,2,0} parameter(0)
      p1 = s16[128,1024,33,33]{3,2,1,0} parameter(1)
      loop_fusion = s16[128,1024,32,32]{1,3,2,0} fusion(p0, p1), kind=kLoop, calls=mixed_input_layouts_computation
      reduce_fusion = s32[1024]{0} fusion(loop_fusion), kind=kInput, calls=fused_reduce
      ROOT root = (s32[1024]{0}, s16[128,1024,32,32]{1,3,2,0}) tuple(reduce_fusion, loop_fusion)
    })"))
...
ENTRY computation {
    p = s32[1,5000,6000]{2,1,0} parameter(0)
    c = s32[1,6000,5000]{2,1,0} transpose(p), dimensions={0,2,1}
    ROOT r = s32[300,20,5000]{2,1,0} reshape(c)
}
)")
                    .value();
  const HloInstruction* root = module->entry_computation()->root_instruction();
...
// TODO(batzor): Implement this test. Dependency: custom_call_target.
...
TEST_F(GpuFusibleTest, GetFusibleComputations) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    fused_reduce {
      p0 = s32[128,1024] parameter(0)
      c0 = s32[] constant(0)
      ROOT reduce = s32[128]{0} reduce(p0, c0), dimensions={1}, to_apply=scalar_add
    }
    body_a {
      p0 = s32[128,1024] parameter(0)
      ROOT reduce_fusion = s32[128] fusion(p0), kind=kInput, calls=fused_reduce
    }
    body_b {
      p0 = s32[128,1024] parameter(0)
      c0 = s32[] constant(0)
      ROOT bc = s32[128] broadcast(c0), dimensions={}
    }
    body_c {
      p0 = s32[128,1024] parameter(0)
      c0 = s32[] constant(0)
    ...
    ENTRY main {
      p0 = s32[] parameter(0)
      p1 = s32[128,1024] parameter(1)
      called = s32[128] call(p1), to_apply=body_c,
        frontend_attributes={_zkx_stream_annotation="1"}
      ROOT conditional = s32[128] conditional(p0, p1, p1),
...
TEST_F(GpuFusibleTest, GetSharedMemoryUsage) {
  auto module = ParseAndReturnVerifiedModule(absl::StrCat(kModulePrefix, R"(
    wrapped_transpose {
      p0 = s32[128,1024,2]{2,1,0} parameter(0)
      ROOT transpose = s32[1024,128,2]{2,1,0} transpose(p0), dimensions={1,0,2}
    }
    ENTRY main {
      p = s32[128,1024,2] parameter(0)
      ROOT res = s32[1024,128,2]{2,1,0} fusion(p), kind=kInput, calls=wrapped_transpose
    })"))
                    .value();
