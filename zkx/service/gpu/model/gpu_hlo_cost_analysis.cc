...
absl::Status GpuHloCostAnalysis::HandleAllReduce(
  ...
  // TODO(TJ): Support multi-node case, we need to know how many nodes there
  // are.
  ...
  if (num_ranks > 1) {
    ...
    // bus bandwidth to get the effective bandwidth of the algorithm. The
    // scaling ratio differs based on what algorithm NCCL chooses to use. This
    // is the scaling factor for ring since NCCL will only use ring for
    // single-node, need to add support for tree algo in multi-node case.
    scaling_ratio = static_cast<float>(num_ranks) / num_intra_steps;
  }
