...
// TODO(batzor): Variadic ReduceWindow emission is not yet supported (single
// output only). This detection is correct, but downstream must handle it.
...
bool IsStreamAnnotatedComputation(const HloInstruction* instr) {
  return instr->opcode() == HloOpcode::kCall &&
         instr->frontend_attributes().map().contains(kZkxStreamAnnotationAttr);
}
