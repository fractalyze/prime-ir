...
Service::Service(const ServiceOptions& options,
  ...
  if (execute_backend_) {
    ...
    LOG(INFO) << absl::StreamFormat(
        "ZKX service %p initialized for platform %s (this does not guarantee "
        "that ZKX will be used). Devices:",
    ...
    for (int i = 0; i < execute_backend_->device_count(); ++i) {
      ...
      LOG(INFO) << absl::StreamFormat("  StreamExecutor device (%d): %s, %s", i,
                                      description.name(),
                                      description.platform_version());
    }
  } else {
    VLOG(1) << "ZKX compile-only service constructed";
  }
}
...
absl::StatusOr<std::unique_ptr<HloModuleConfig>> Service::CreateModuleConfig(
  ...
  return zkx::CreateModuleConfig(program_shape, argument_shapes,
...
Service::BuildExecutables(
  ...
  VLOG(1) << absl::StreamFormat("BuildExecutable on service %p", this);
...
Service::BuildAotResults(
    ...
    Backend* backend, std::vector<std::vector<se::StreamExecutor*>> executors,
    const Compiler::CompileOptions& options, bool run_backend_only) {
  VLOG(1) << absl::StreamFormat("BuildAotResults on service %p", this);
...
Service::ExecuteParallelAndRegisterResult(
  ...
  for (int64_t i = 0, end = executables.size(); i < end; i++) {
    ...
    for (int64_t replica = 0; replica < n; ++replica) {
      ...
      if (replica == 0 &&
          executables[i]->module_config().debug_options().zkx_hlo_profile() &&
  ...
  for (int64_t i = 0, end = streams.size(); i < end; ++i) {
    ...
    if (!block_status.ok()) {
      return absl::InternalError(
          absl::StrFormat("failed to complete execution for stream %d: %s", i,
...
Service::ExecuteGraphParallel(
    absl::Span<const ZkxComputationInstance> computations) {
  ...
  int num_requested_devices =
      std::accumulate(computations.begin(), computations.end(), 0,
                      [](int a, const ZkxComputationInstance& r) -> int {
  ...
  for (int64_t i = 0; i < computations.size(); ++i) {
    const ZkxComputationInstance& computation = computations[i];
...
absl::StatusOr<std::unique_ptr<Executable>> Service::BuildExecutable(
  ...
  tsl::profiler::ScopedAnnotation annotation{[&] {
    ...
    return absl::StrFormat("ZkxCompile:#module=%s#", module_proto.name());
  }};
...
absl::StatusOr<ExecutionHandle> Service::Compile(
    const ZkxComputation& computation, absl::Span<const Shape> argument_shapes,
    const ExecutionOptions& execution_options) {
  VLOG(1) << "running compile request";

  if (!computation.proto().has_host_program_shape()) {
    return absl::InvalidArgumentError("program shape may not be empty");
  }
...
absl::StatusOr<std::unique_ptr<GlobalData>> Service::Execute(
  ...
  if (num_module_args != arguments.size()) {
    return absl::InvalidArgumentError(
        absl::StrFormat("The executable expects %lld arguments, but sees %lld.",
...
absl::StatusOr<Literal> Service::TransferToClient(
  ...
  if (shape_with_layout) {
    ...
    if (!LayoutUtil::HasLayout(return_shape)) {
      return absl::InvalidArgumentError(
          "shape_with_layout must have layout if present.");
    }
...
absl::StatusOr<Literal> Service::ComputeConstantGraph(
    const ZkxComputation& computation, const Layout* output_layout) {
  if (!computation.proto().has_host_program_shape()) {
    return absl::InvalidArgumentError("program shape may not be empty");
  }
  ...
  // TODO(chokobole): Uncomment this. Dependency: DynamicPadder
  ...
  evaluator.set_custom_call_handler(
      [](const HloInstruction* custom_call,
        ...
        return absl::UnimplementedError(absl::StrFormat(
            "Custom call %s is not supported: %s",
            custom_call->custom_call_target(), custom_call->ToString()));
      });
...
// static
void GlobalData::Release(std::vector<std::unique_ptr<GlobalData>> instances) {
