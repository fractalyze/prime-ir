...
// pass.
// NOTE: ZKX does not currently have an unstacking pass, but we keep this
...
absl::StatusOr<bool> UnrollInternal(HloInstruction* while_op,
  ...
  for (int64_t i = config.init; i < config.trip_count + config.init; ++i) {
    CHECK(OverflowSafeAdd(i, static_cast<int64_t>(1)).has_value());
...
absl::StatusOr<bool> WhileLoopUnroller::Run(
  ...
  ZKX_VLOG_LINES(3, "WhileLoopUnroller::Run(), before:\n" + module->ToString());
  bool changed = false;
  // Make sure all the necessary passes are executed before unrolling in order
  // to unroll every possible loop.
  ...
  ZKX_VLOG_LINES(3, "WhileLoopUnroller::Run(), after:\n" + module->ToString());
  return changed;
}
