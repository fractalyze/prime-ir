...
#include "zkx/metric_table_report.h"
#include "zkx/util.h"
...
std::string HumanReadableProfileBuilder::ToString() const {
  ...
  auto print_op = [&](const OpInfo& op, bool is_total = false) {
    ...
    StrAppendFormat(
        &s,
        "%15d cycles (%s) :: %12.1f usec %22s :: %18s :: %14s :: "
        "%16s :: %s\n",
  ...
  print_op({is_entry_computation_ ? "[total] [entry]" : "[total]", "[total]",
            /*category=*/"", total_cycles_, total_flops, total_bytes,
            static_cast<float>(optimal_seconds_sum)},
           /*is_total=*/true);
