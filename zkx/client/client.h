...
#ifndef ZKX_CLIENT_CLIENT_H_
#define ZKX_CLIENT_CLIENT_H_
...
// ZKX service's client object -- wraps the service with convenience and
...
class Client {
 public:
  ...
  using ZkxComputationInstance = zkx::ZkxComputationInstance;
  ...
  // to call the Execute(const ZkxComputation&) overload.  If you're going to
  ...
  // Executes a list ZkxComputationInstances and returns global data produced
  ...
  //
  // This functionality can be useful when translating a computation into ZKX
  // where something that looked dynamic is required by ZKX to be specified as a
  // constant. E.g. the source computation (outside of ZKX) may include a
  ...
  absl::StatusOr<ZkxComputation> LoadSnapshot(const HloSnapshot& module);
