...
// The "client library" instantiates a local (in-process) ZKX service for
// use by this process, and connects to it with a singleton ZKX local
// client. ClientLibrary::GetOrCreateLocalClient will spawn a local service,
// and return a client that's connected to it and ready to run ZKX
// computations.
#ifndef ZKX_CLIENT_CLIENT_LIBRARY_H_
#define ZKX_CLIENT_CLIENT_LIBRARY_H_
...
#include "zkx/client/compile_only_client.h"
...
class ClientLibrary {
 public:
  ...
  // to issue ZKX commands on. Arguments:
  //
  //   platform : The platform the underlying ZKX service should target. If
  ...
  static LocalService* GetZkxService(se::Platform* platform);
...
#endif  // ZKX_CLIENT_CLIENT_LIBRARY_H_
