...
CompileOnlyClient::CompileAheadOfTime(
    const absl::Span<const AotZkxComputationInstance> computations,
    const AotCompilationOptions& options,
    std::unique_ptr<AotCompilationMetadata>* metadata) {
  std::vector<CompileOnlyService::AotZkxComputationInstance> service_instances;
  service_instances.reserve(computations.size());
  for (const AotZkxComputationInstance& instance : computations) {
    service_instances.emplace_back();
    CompileOnlyService::AotZkxComputationInstance& service_instance =
