...
absl::StatusOr<Literal> Client::ExecuteAndTransfer(
    const ZkxComputation& computation, absl::Span<GlobalData* const> arguments,
...
absl::StatusOr<Literal> Client::ComputeConstant(
    const ZkxComputation& computation, const Layout* output_layout) const {
  return stub_->ComputeConstantGraph(computation, output_layout);
}

absl::StatusOr<ZkxComputation> Client::LoadSnapshot(const HloSnapshot& module) {
  TF_RET_CHECK(module.has_hlo() && module.hlo().has_hlo_module());
  return ZkxComputation(module.hlo().hlo_module());
}
...
absl::StatusOr<std::unique_ptr<GlobalData>> Client::Execute(
  ...
  std::vector<ZkxComputationInstance> computation_instances = {
      ZkxComputationInstance{
          computation,
...
Client::ExecuteParallel(absl::Span<const ZkxComputationInstance> computations) {
...
absl::StatusOr<std::vector<DeviceHandle>> Client::GetDeviceHandles(
  ...
  if (device_count < 1) {
    return absl::InvalidArgumentError("device_count must be greater than 0");
  }
