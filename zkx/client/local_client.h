...
#ifndef ZKX_CLIENT_LOCAL_CLIENT_H_
#define ZKX_CLIENT_LOCAL_CLIENT_H_
...
class LocalExecutable {
 public:
  ...
  // Verifies that a device is compatible with the executable's build device.
 ...
 private:
  ...
  // The ordinal of the device which this executable was compiled for.
...
// A ZKX Client specialization for use when the client and service run in
// the same process.
class LocalClient : public Client {
 public:
  ...
  CompileAheadOfTime(const ZkxComputation& computation,
                     absl::Span<const Shape* const> argument_layouts,
                     const ExecutableBuildOptions& options);
  ...
  absl::StatusOr<GlobalDataHandle> TransferToLocalServer(
      const BorrowingLiteral& literal, int device_ordinal);
  ...
  // Transfer and return a value from the outfeed of the given device.
  ...
  // Returns the number of devices on the system of the service platform type.
  ...
  // service to execute computations.
...
#endif  // ZKX_CLIENT_LOCAL_CLIENT_H_
