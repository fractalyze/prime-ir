...
// static
absl::StatusOr<LocalClient*> ClientLibrary::GetOrCreateLocalClient(
...
// static
LocalClient* ClientLibrary::LocalClientOrDie() {
...
// static
LocalService* ClientLibrary::GetZkxService(se::Platform* platform) {
  ClientLibrary& client_library = Singleton();
  absl::MutexLock lock(&client_library.service_mutex_);
  auto it = client_library.local_instances_.find(platform->id());
...
// static
absl::StatusOr<CompileOnlyClient*> ClientLibrary::GetOrCreateCompileOnlyClient(
...
// static
void ClientLibrary::DestroyLocalInstances() {
  ClientLibrary& client_library = Singleton();
  absl::MutexLock lock(&client_library.service_mutex_);
