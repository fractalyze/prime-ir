...
#ifndef ZKX_FFI_EXECUTION_CONTEXT_H_
#define ZKX_FFI_EXECUTION_CONTEXT_H_
...
// handlers in the scope of a single ZKX execution. Execution context allows to
// pass arbitrary user data to FFI handlers via the side channel that does not
// require modifying HLO modules.
//
// From ZKX FFI perspective user data is an opaque pointer that can be forwarded
...
// unique between separate calls to ZKX execute.
class ExecutionContext {
 ...
 private:
  ...
  // stays valid during the ZKX execution and correctly destroyed afterwards.
  class UserData {
   public:
...
#endif  // ZKX_FFI_EXECUTION_CONTEXT_H_
