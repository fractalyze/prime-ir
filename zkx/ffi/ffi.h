...
#ifndef ZKX_FFI_FFI_H_
#define ZKX_FFI_FFI_H_

#ifdef ZKX_FFI_API_FFI_H_
...
class AnyBuffer {
 public:
  ...
  explicit AnyBuffer(const ZKX_FFI_Buffer* absl_nonnull buf) : buf_(buf) {
    DCHECK(buf_ != nullptr) << "ZKX_FFI_Buffer must be non-null";
  }
...
class Buffer {
 public:
  ...
  explicit Buffer(const ZKX_FFI_Buffer* absl_nonnull buf) : buf_(buf) {
    DCHECK(buf_ != nullptr) << "ZKX_FFI_Buffer must be non-null";
  }
...
struct ArgDecoding<AnyBuffer> {
  ...
  static std::optional<AnyBuffer> Decode(ZKX_FFI_ArgType type, void* arg,
    ...
    if (ABSL_PREDICT_FALSE(type != ZKX_FFI_ArgType_BUFFER)) {
 ...
 public:
  ...
  absl::StatusOr<T> get(size_t index) const {
    ...
    if (ABSL_PREDICT_FALSE(!value.has_value())) {
      return absl::InternalError(diagnostic.Result());
    }
...
struct RetDecoding<AnyBuffer> {
  ...
  static std::optional<Result<AnyBuffer>> Decode(ZKX_FFI_RetType type,
                                                 void* arg,
                                                 DiagnosticEngine& diagnostic) {
    if (ABSL_PREDICT_FALSE(type != ZKX_FFI_RetType_BUFFER)) {
...
struct RetDecoding<Buffer<dtype, rank>> {
  ...
  static std::optional<Result<Buffer<dtype, rank>>> Decode(
      ZKX_FFI_RetType type, void* arg, DiagnosticEngine& diagnostic) {
    if (ABSL_PREDICT_FALSE(type != ZKX_FFI_RetType_BUFFER)) {
      return diagnostic.Emit("Wrong result type: expected ")
             << ZKX_FFI_RetType_BUFFER << " but got " << type;
    }
...
#define ZKX_FFI_REGISTER_ARRRAY_ATTR_DECODING(T, TYPE)                   \
  template <>                                                            \
  struct AttrDecoding<absl::Span<const T>> {                             \
    ...
    static std::optional<Type> Decode(ZKX_FFI_AttrType type, void* attr, \
                                      DiagnosticEngine& diagnostic) {    \
      if (ABSL_PREDICT_FALSE(type != ZKX_FFI_AttrType_ARRAY)) {          \
...
ZKX_FFI_REGISTER_ARRRAY_ATTR_DECODING(int8_t, ZKX_FFI_DataType_S8);
ZKX_FFI_REGISTER_ARRRAY_ATTR_DECODING(int16_t, ZKX_FFI_DataType_S16);
ZKX_FFI_REGISTER_ARRRAY_ATTR_DECODING(int32_t, ZKX_FFI_DataType_S32);
ZKX_FFI_REGISTER_ARRRAY_ATTR_DECODING(int64_t, ZKX_FFI_DataType_S64);

#undef ZKX_FFI_REGISTER_ARRRAY_ATTR_DECODING
...
struct AttrDecoding<Pointer<T>> {
  ...
  static std::optional<Type> Decode(ZKX_FFI_AttrType type, void* attr,
    ...
    if (ABSL_PREDICT_FALSE(type != ZKX_FFI_AttrType_SCALAR ||
...
struct CtxDecoding<Stream> {
  ...
  static std::optional<Type> Decode(const ZKX_FFI_Api* api,
                                    ...
                                    DiagnosticEngine& diagnostic) {
    void* ptr = api->internal_api->ZKX_FFI_INTERNAL_Stream_Get(ctx);
...
struct CtxDecoding<DeviceOrdinal> {
  ...
  static std::optional<Type> Decode(const ZKX_FFI_Api* api,
                                    ZKX_FFI_ExecutionContext* ctx,
                                    DiagnosticEngine&) {
    return api->internal_api->ZKX_FFI_INTERNAL_DeviceOrdinal_Get(ctx);
  }
};
...
struct CtxDecoding<CalledComputation> {
  ...
  static std::optional<Type> Decode(const ZKX_FFI_Api* api,
                                    ZKX_FFI_ExecutionContext* ctx,
                                    DiagnosticEngine&) {
    void* ptr = api->internal_api->ZKX_FFI_INTERNAL_CalledComputation_Get(ctx);
    return reinterpret_cast<Type>(ptr);
  }
};
...
struct CtxDecoding<RunId> {
  ...
  static std::optional<Type> Decode(const ZKX_FFI_Api* api,
                                    ZKX_FFI_ExecutionContext* ctx,
                                    DiagnosticEngine& diagnostic) {
    return RunId{api->internal_api->ZKX_FFI_INTERNAL_RunId_Get(ctx)};
  }
};
...
struct CtxDecoding<UserData<T>> {
  ...
  static std::optional<Type> Decode(const ZKX_FFI_Api* api,
    ...
    auto* execution_context = reinterpret_cast<const ExecutionContext*>(
        api->internal_api->ZKX_FFI_INTERNAL_ExecutionContext_Get(ctx));
...
struct CtxDecoding<State<T>> {
  ...
  static std::optional<Type> Decode(const ZKX_FFI_Api* api,
    ...
    auto* execution_state = reinterpret_cast<const ExecutionState*>(
        api->internal_api->ZKX_FFI_INTERNAL_ExecutionState_Get(ctx));
...
struct ResultEncoding<stage, absl::Status> {
  static ZKX_FFI_Error* Encode(const ZKX_FFI_Api* api,
    ...
    return api->internal_api->ZKX_FFI_INTERNAL_Error_Forward(&status);
  }
};
...
struct ResultEncoding<stage, tsl::AsyncValueRef<tsl::Chain>> {
  static ZKX_FFI_Future* Encode(const ZKX_FFI_Api* api,
                                ZKX_FFI_ExecutionContext* ctx,
                                tsl::AsyncValueRef<tsl::Chain> async_value) {
    return api->internal_api->ZKX_FFI_INTERNAL_Future_Forward(
        async_value.release());
  }
};
