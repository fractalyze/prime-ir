...
#ifndef ZKX_FFI_API_API_H_
#define ZKX_FFI_API_API_H_
...
// ZKX FFI call frames and invoking corresponding C++ functions. This must have
// no dependencies outside of the C++ standard library.
//
// There are two extensions to this base library:
//
//   (1) zkx/ffi/api/ffi.h for defining "external" FFI handlers loaded from
//       dynamic libraries potentially built with different toolchains and/or
//       a different ZKX commit. It is a header-only library without any
//       dependencies.
//
//   (2) zkx/ffi/ffi.h for defining "internal" FFI handlers that must be
//       statically linked into the binary and must be built from the same
//       commit using the same toolchain, as it provides access to ZKX
...
#include "zkx/ffi/api/c_api.h"

#ifdef __has_builtin
#define ZKX_FFI_HAS_BUILTIN(x) __has_builtin(x)
#else
#define ZKX_FFI_HAS_BUILTIN(x) 0
#endif

#if __has_attribute(always_inline)
#define ZKX_FFI_ATTRIBUTE_ALWAYS_INLINE __attribute__((always_inline))
#elif defined(_MSC_VER)
#define ZKX_FFI_ATTRIBUTE_ALWAYS_INLINE __forceinline
#else
#define ZKX_FFI_ATTRIBUTE_ALWAYS_INLINE
#endif

#if __has_attribute(noinline)
#define ZKX_FFI_ATTRIBUTE_NEVER_INLINE __attribute__((noinline))
#elif defined(_MSC_VER)
#define ZKX_FFI_ATTRIBUTE_NEVER_INLINE __declspec(noinline)
#else
#define ZKX_FFI_ATTRIBUTE_NEVER_INLINE
#endif

#if ZKX_FFI_HAS_BUILTIN(__builtin_expect)
#define ZKX_FFI_PREDICT_FALSE(x) (__builtin_expect(false || (x), false))
#define ZKX_FFI_PREDICT_TRUE(x) (__builtin_expect(false || (x), true))
#else
#define ZKX_FFI_PREDICT_FALSE(x) (x)
#define ZKX_FFI_PREDICT_TRUE(x) (x)
#endif
...
inline std::ostream& operator<<(std::ostream& os,
                                const ZKX_FFI_DataType dtype) {
  switch (dtype) {
    case ZKX_FFI_DataType_INVALID:
      return os << "INVALID";
    case ZKX_FFI_DataType_PRED:
      return os << "PRED";
    case ZKX_FFI_DataType_S8:
      return os << "S8";
    case ZKX_FFI_DataType_S16:
      return os << "S16";
    case ZKX_FFI_DataType_S32:
      return os << "S32";
    case ZKX_FFI_DataType_S64:
      return os << "S64";
    case ZKX_FFI_DataType_U8:
      return os << "U8";
    case ZKX_FFI_DataType_U16:
      return os << "U16";
    case ZKX_FFI_DataType_U32:
      return os << "U32";
    case ZKX_FFI_DataType_U64:
      return os << "U64";
    case ZKX_FFI_DataType_TOKEN:
      return os << "TOKEN";
#define ZK_DTYPES_CASE(unused, unused2, enum, unused3) \
  case ZKX_FFI_DataType_##enum:                        \
    return os << #enum;
...
inline std::ostream& operator<<(std::ostream& os, const ZKX_FFI_AttrType type) {
  switch (type) {
    case ZKX_FFI_AttrType_ARRAY:
      return os << "array";
    case ZKX_FFI_AttrType_DICTIONARY:
      return os << "dictionary";
    case ZKX_FFI_AttrType_SCALAR:
      return os << "scalar";
    case ZKX_FFI_AttrType_STRING:
      return os << "string";
  }
}

inline std::ostream& operator<<(std::ostream& os,
                                const ZKX_FFI_ExecutionStage stage) {
  switch (stage) {
    case ZKX_FFI_ExecutionStage_INSTANTIATE:
      return os << "instantiate";
    case ZKX_FFI_ExecutionStage_PREPARE:
      return os << "prepare";
    case ZKX_FFI_ExecutionStage_INITIALIZE:
      return os << "initialize";
    case ZKX_FFI_ExecutionStage_EXECUTE:
      return os << "execute";
  }
}
...
enum class ExecutionStage : uint8_t {
  kInstantiate = ZKX_FFI_ExecutionStage_INSTANTIATE,
  kPrepare = ZKX_FFI_ExecutionStage_PREPARE,
  kInitialize = ZKX_FFI_ExecutionStage_INITIALIZE,
  kExecute = ZKX_FFI_ExecutionStage_EXECUTE,
};

enum class Traits : uint32_t {
  kCmdBufferCompatible = ZKX_FFI_HANDLER_TRAITS_COMMAND_BUFFER_COMPATIBLE,
};
...
//===----------------------------------------------------------------------===//
// ZKX FFI virtual base for implementing FFI handlers
//===----------------------------------------------------------------------===//
...
class Ffi {
 public:
  ...
  virtual ~Ffi() = default;
  virtual ZKX_FFI_Error* Call(ZKX_FFI_CallFrame* call_frame) const = 0;

  // Registers FFI handler bundle with an ZKX runtime under the given name on a
  // given platform.
  static ZKX_FFI_Error* RegisterStaticHandler(
      const ZKX_FFI_Api* api, std::string_view name, std::string_view platform,
      ZKX_FFI_Handler_Bundle bundle, ZKX_FFI_Handler_Traits traits = 0);

  // Registers FFI execute handler with an ZKX runtime under the given name on a
  // given platform.
  static ZKX_FFI_Error* RegisterStaticHandler(
      const ZKX_FFI_Api* api, std::string_view name, std::string_view platform,
      ZKX_FFI_Handler* execute, ZKX_FFI_Handler_Traits traits = 0) {
    return RegisterStaticHandler(
        api, name, platform,
        ZKX_FFI_Handler_Bundle{nullptr, nullptr, nullptr, execute}, traits);
  }
  ...
  // `type_id`.
  static ZKX_FFI_Error* RegisterTypeId(const ZKX_FFI_Api* api,
 ...
 protected:
  ...
  static ZKX_FFI_Error* Success();

  static ZKX_FFI_Error* MakeError(const ZKX_FFI_Api* api,
                                  ZKX_FFI_Error_Code errc, std::string message);

  static ZKX_FFI_Error* InvalidArgument(const ZKX_FFI_Api* api,
                                        std::string message);

  static ZKX_FFI_Error* CheckStructSize(const ZKX_FFI_Api* api,
                                        std::string_view struct_name,
                                        size_t expected, size_t actual);

  static ZKX_FFI_Error* StructSizeIsGreaterOrEqual(const ZKX_FFI_Api* api,
                                                   std::string_view struct_name,
                                                   size_t expected,
                                                   size_t actual);
};

inline ZKX_FFI_Error* Ffi::RegisterStaticHandler(
    const ZKX_FFI_Api* api, std::string_view name, std::string_view platform,
    ZKX_FFI_Handler_Bundle bundle, ZKX_FFI_Handler_Traits traits) {
  ZKX_FFI_Handler_Register_Args args;
  args.struct_size = ZKX_FFI_Handler_Register_Args_STRUCT_SIZE;
  args.extension_start = nullptr;
  args.name = ZKX_FFI_ByteSpan{name.data(), name.size()};
  args.platform = ZKX_FFI_ByteSpan{platform.data(), platform.size()};
  args.bundle = bundle;
  args.traits = traits;
  return api->ZKX_FFI_Handler_Register(&args);
}

inline ZKX_FFI_Error* Ffi::RegisterTypeId(const ZKX_FFI_Api* api,
                                          std::string_view name,
                                          ZKX_FFI_TypeId* type_id) {
  ZKX_FFI_TypeId_Register_Args args;
  args.struct_size = ZKX_FFI_TypeId_Register_Args_STRUCT_SIZE;
  args.extension_start = nullptr;
  args.name = ZKX_FFI_ByteSpan{name.data(), name.size()};
  args.type_id = type_id;
  return api->ZKX_FFI_TypeId_Register(&args);
}
...
inline ZKX_FFI_Error* Ffi::Success() { return nullptr; }

inline ZKX_FFI_Error* Ffi::MakeError(const ZKX_FFI_Api* api,
                                     ZKX_FFI_Error_Code errc,
                                     std::string message) {
  ZKX_FFI_Error_Create_Args args;
  args.struct_size = ZKX_FFI_Error_Create_Args_STRUCT_SIZE;
  args.extension_start = nullptr;
  args.errc = errc;
  args.message = message.c_str();
  return api->ZKX_FFI_Error_Create(&args);
}

inline ZKX_FFI_Error* Ffi::InvalidArgument(const ZKX_FFI_Api* api,
                                           std::string message) {
  return MakeError(api, ZKX_FFI_Error_Code_INVALID_ARGUMENT,
                   std::move(message));
}

inline ZKX_FFI_Error* Ffi::CheckStructSize(const ZKX_FFI_Api* api,
                                           std::string_view struct_name,
                                           size_t expected, size_t actual) {
  if (expected != actual) {
...
inline ZKX_FFI_Error* Ffi::StructSizeIsGreaterOrEqual(
    const ZKX_FFI_Api* api, std::string_view struct_name, size_t expected,
    size_t actual) {
  if (actual < expected) {
    return InvalidArgument(
...
// variadic template. In ZKX FFI we use destination passing style APIs and don't
...
template <typename T>
ZKX_FFI_DataType NativeTypeToCApiDataType() {
...
// ZKX FFI binding for an argument.
//
...
// ZKX FFI binding for a returned result.
//
...
// ZKX FFI binding for a named attribute.
//
...
// ZKX FFI binding for dictionary attributes: automatic parsing of all
...
// ZKX FFI binding for values passed via context.
//
...
// ZKX FFI arguments decoding must be defined by specializing this template.
//
// Example: decoding for the `MyType` arguments
//
//   template <>
//   struct ArgDecoding<MyType> {
//     static std::optional<MyType> Decode(ZKX_FFI_ArgType type, void* arg);
//   };
//
...
// ZKX FFI results decoding must be defined by specializing this template.
//
// Example: decoding for the `MyType` results
//
//   template <>
//   struct RetDecoding<MyType> {
//     static std::optional<MyType> Decode(ZKX_FFI_RetType type, void* ret);
//   };
//
// If argument can't be decoded it should return the empty optional.
template <typename T>
...
// ZKX FFI attribute decoding must be defined by specializing this template.
//
...
//    static std::optional<MyType> Decode(ZKX_FFI_AttrType type, void* attr,
//                                        DiagnosticEngine&);
//   }
//
template <typename T>
...
// ZKX FFI execution context decoding must be defined by specializing this
// template.
//
...
// ZKX FFI result encoding (conversion from a returned status-like type to FFI
...
//     ZKX_FFI_Error* Encode(const ZKX_FFI_Api* api,
//                           ZKX_FFI_ExecutionContext* ctx,
//                           absl::Status status) {...}
//   };
//
...
//
// Asynchronous FFI handlers can return encoded result as an `ZKX_FFI_Future*`
// or as an `std::variant` of `ZKX_FFI_Error*` and `ZKX_FFI_Future*`, where an
...
//
// Example: encoding `zkx::ffi::Future` result
//
//   template<ExecutionStage stage>
//   struct ResultEncoding<state, zkx::ffi::Future> {
//     std::variant<ZKX_FFI_Error*, ZKX_FFI_Future*> Encode(
//       const ZKX_FFI_Api* api, ZKX_FFI_ExecutionContext* ctx,
//       zkx::ffi::Future future) {...}
//   };
//
...
struct DecodingContext {
  const ZKX_FFI_CallFrame* call_frame;
...
template <typename T>
struct Decode {
  ZKX_FFI_ATTRIBUTE_ALWAYS_INLINE
...
struct Decode<AttrTag<T>> {
  ...
  static std::optional<R> call(DecodingOffsets& offsets, DecodingContext& ctx,
    ...
    ZKX_FFI_AttrType attr_type = ctx.call_frame->attrs.types[idx];
    ZKX_FFI_ByteSpan* attr_name = ctx.call_frame->attrs.names[idx];
...
class RemainingArgsBase {
 public:
  RemainingArgsBase(const ZKX_FFI_Args* args, size_t offset)
 ...
 protected:
  const ZKX_FFI_Args* args() const { return args_; }
  size_t offset() const { return offset_; }

 private:
  const ZKX_FFI_Args* args_;
  size_t offset_;
};
...
class RemainingRetsBase {
 public:
  RemainingRetsBase(const ZKX_FFI_Rets* rets, size_t offset)
 ...
 protected:
  const ZKX_FFI_Rets* rets() const { return rets_; }
  size_t offset() const { return offset_; }

 private:
  const ZKX_FFI_Rets* rets_;  // not owned
  size_t offset_;
};
...
class DictionaryBase {
 public:
  explicit DictionaryBase(const ZKX_FFI_Attrs* attrs) : attrs_(attrs) {}
 ...
 protected:
  ...
  std::optional<T> get(std::string_view name,
    ...
    if (ZKX_FFI_PREDICT_FALSE(idx >= attrs_->size)) {
      return diagnostic.Emit("Unexpected attribute: ") << name;
    }

    ZKX_FFI_AttrType attr_type = attrs_->types[idx];
 ...
 private:
  size_t Find(std::string_view name) const {
    ZKX_FFI_ByteSpan** begin = attrs_->names;
    ZKX_FFI_ByteSpan** end = begin + attrs_->size;

    auto name_eq = [&](ZKX_FFI_ByteSpan* attr) {
...
struct internal::Decode<internal::AttrsTag<T>> {
  static std::optional<T> call(DecodingOffsets& offsets, DecodingContext& ctx,
    ...
    return AttrDecoding<T>::Decode(
        ZKX_FFI_AttrType_DICTIONARY,
        const_cast<ZKX_FFI_Attrs*>(&ctx.call_frame->attrs), diagnostic);
  }
};
...
class Handler : public Ffi {
 ...
 public:
  ...
  ZKX_FFI_Error* Call(ZKX_FFI_CallFrame* call_frame) const override {
    // Sanity checking call frame struct size.
    if (ZKX_FFI_Error* err = CheckStructSize(
            call_frame->api, "ZKX_FFI_CallFrame", ZKX_FFI_CallFrame_STRUCT_SIZE,
    ...
    if (call_frame->extension_start != nullptr &&
        call_frame->extension_start->type == ZKX_FFI_Extension_Metadata) {
      return PopulateMetadata(call_frame->api,
                              reinterpret_cast<ZKX_FFI_Metadata_Extension*>(
                                  call_frame->extension_start));
    }

    // Check that handler is called during correct execution stage.
    if (ZKX_FFI_PREDICT_FALSE(call_frame->stage !=
                              static_cast<ZKX_FFI_ExecutionStage>(stage))) {
    ...
    if constexpr (internal::HasRemainingArgsTag<Ts...>::value) {
      if (ZKX_FFI_PREDICT_FALSE(call_frame->args.size < kNumArgs)) {
        return InvalidArgument(
            call_frame->api,
    ...
    if constexpr (internal::HasRemainingRetsTag<Ts...>::value) {
      if (ZKX_FFI_PREDICT_FALSE(call_frame->rets.size < kNumRets)) {
        return InvalidArgument(
            call_frame->api,
    ...
    if (ZKX_FFI_PREDICT_FALSE(kNumDictAttrs == 0 &&
 ...
 private:
  ZKX_FFI_Error* PopulateMetadata(const ZKX_FFI_Api* api,
                                  ZKX_FFI_Metadata_Extension* extension) const {
    if (ZKX_FFI_Error* err =
            StructSizeIsGreaterOrEqual(api, "ZKX_FFI_Metadata_Extension",
                                       ZKX_FFI_Metadata_Extension_STRUCT_SIZE,
                                       extension->extension_base.struct_size)) {
      return err;
    }

    if (ZKX_FFI_Error* err = StructSizeIsGreaterOrEqual(
            api, "ZKX_FFI_Metadata", ZKX_FFI_Metadata_STRUCT_SIZE,
            extension->metadata->struct_size)) {
      return err;
    }

    extension->metadata->api_version = ZKX_FFI_Api_Version{
    ...
    ZKX_FFI_Handler_Traits traits = 0;
    for (const auto& trait : traits_) {
      traits |= static_cast<ZKX_FFI_Handler_Traits>(trait);
    }
  ...
  template <size_t... Is>
  ZKX_FFI_ATTRIBUTE_ALWAYS_INLINE ZKX_FFI_Error* Call(
      ZKX_FFI_CallFrame* call_frame, std::index_sequence<Is...>) const {
    ...
    if constexpr (sizeof...(Ts) > 0) {
      ...
      if (ZKX_FFI_PREDICT_FALSE(!all_decoded)) {
    ...
    //   (1) Synchronous handlers that return result encoded as ZKX_FFI_Error*
    //   (2) Asynchronous handlers that return result encoded as ZKX_FFI_Future*
    //   (3) Handlers that can return either (1) or (2)
    static constexpr bool kIsEncodedError =
        std::is_same_v<decltype(encoded), ZKX_FFI_Error*>;
    static constexpr bool kIsEncodedFuture =
        std::is_same_v<decltype(encoded), ZKX_FFI_Future*>;
    static constexpr bool kIsEncodedErrorOrFuture =
        std::is_same_v<decltype(encoded),
                       std::variant<ZKX_FFI_Error*, ZKX_FFI_Future*>>;
  ...
  ZKX_FFI_Error* FailedDecodeError(const ZKX_FFI_CallFrame* call_frame,
...
#define ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(T, TYPE)                \
  template <>                                                         \
  struct AttrDecoding<T> {                                            \
    using Type = T;                                                   \
    static std::optional<T> Decode(ZKX_FFI_AttrType type, void* attr, \
                                   DiagnosticEngine& diagnostic) {    \
      if (ZKX_FFI_PREDICT_FALSE(type != ZKX_FFI_AttrType_SCALAR)) {   \
        return diagnostic.Emit("Wrong attribute type: expected ")     \
               << ZKX_FFI_AttrType_SCALAR << " but got " << type;     \
      }                                                               \
                                                                      \
      auto* scalar = reinterpret_cast<ZKX_FFI_Scalar*>(attr);         \
      if (ZKX_FFI_PREDICT_FALSE(scalar->dtype != TYPE)) {             \
...
ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(bool, ZKX_FFI_DataType_PRED);
ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(int8_t, ZKX_FFI_DataType_S8);
ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(int16_t, ZKX_FFI_DataType_S16);
ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(int32_t, ZKX_FFI_DataType_S32);
ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(int64_t, ZKX_FFI_DataType_S64);
ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(uint8_t, ZKX_FFI_DataType_U8);
ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(uint16_t, ZKX_FFI_DataType_U16);
ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(uint32_t, ZKX_FFI_DataType_U32);
ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(uint64_t, ZKX_FFI_DataType_U64);
#define ZK_DTYPES_CASE(cpp_type, unused, enum, unused2) \
  ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING(cpp_type, ZKX_FFI_DataType_##enum);
ZK_DTYPES_PUBLIC_TYPE_LIST(ZK_DTYPES_CASE)
#undef ZK_DTYPES_CASE

#undef ZKX_FFI_REGISTER_SCALAR_ATTR_DECODING
...
struct DecodeDictionaryAttr {
  ...
  ZKX_FFI_ATTRIBUTE_ALWAYS_INLINE
  static std::optional<T> Decode(const ZKX_FFI_Attrs* attrs,
  ...
  template <size_t... Is>
  ZKX_FFI_ATTRIBUTE_ALWAYS_INLINE static std::optional<T> Decode(
      const ZKX_FFI_Attrs* attrs, std::array<std::string_view, kSize> names,
      std::index_sequence<Is...>, DiagnosticEngine& diagnostic) {
    if (ZKX_FFI_PREDICT_FALSE(kSize != attrs->size)) {
    ...
    // pre-compute mapping from `names` to index in the `ZKX_FFI_Attrs`
    ...
    if (ZKX_FFI_PREDICT_FALSE(!all_decoded)) return std::nullopt;
...
//
#define ZKX_FFI_REGISTER_STRUCT_ATTR_DECODING(T, ...)                         \
  namespace zkx::ffi {                                                        \
  template <>                                                                 \
  ...
  struct AttrDecoding<T> {                                                    \
    ...
    static std::optional<T> Decode(ZKX_FFI_AttrType type, void* attr,         \
                                   DiagnosticEngine& diagnostic) {            \
      if (ZKX_FFI_PREDICT_FALSE(type != ZKX_FFI_AttrType_DICTIONARY)) {       \
        diagnostic.Emit("Wrong attribute type: expected ")                    \
            << ZKX_FFI_AttrType_DICTIONARY << " but got " << type;            \
        return std::nullopt;                                                  \
      }                                                                       \
                                                                              \
      auto decoder = ::zkx::ffi::internal::DictionaryDecoder<T>(__VA_ARGS__); \
      return decltype(decoder)::Decode(                                       \
          reinterpret_cast<const ZKX_FFI_Attrs*>(attr),                       \
          internal::StructMemberNames(__VA_ARGS__), diagnostic);              \
    }                                                                         \
  };                                                                          \
  } /* namespace zkx::ffi */                                                  \
  static_assert(std::is_class_v<::zkx::ffi::AttrsBinding<T>>);                \
  static_assert(std::is_class_v<::zkx::ffi::AttrDecoding<T>>)
...
#define ZKX_FFI_REGISTER_ENUM_ATTR_DECODING(T)                                \
  namespace zkx::ffi {                                                        \
  template <>                                                                 \
  struct AttrDecoding<T> {                                                    \
    using Type = T;                                                           \
    ...
    static_assert(std::is_enum<Type>::value, "Expected enum class");          \
                                                                              \
    static std::optional<Type> Decode(ZKX_FFI_AttrType attr_type, void* attr, \
                                      DiagnosticEngine& diagnostic) {         \
      if (ZKX_FFI_PREDICT_FALSE(attr_type != ZKX_FFI_AttrType_SCALAR)) {      \
        return diagnostic.Emit("Wrong attribute type: expected ")             \
               << ZKX_FFI_AttrType_SCALAR << " but got " << attr_type;        \
      }                                                                       \
                                                                              \
      auto* scalar = reinterpret_cast<ZKX_FFI_Scalar*>(attr);                 \
      auto expected_dtype =                                                   \
          ::zkx::ffi::internal::NativeTypeToCApiDataType<U>();                \
      if (ZKX_FFI_PREDICT_FALSE(scalar->dtype != expected_dtype)) {           \
        return diagnostic.Emit("Wrong scalar data type: expected ")           \
...
#define ZKX_FFI_ATTRIBUTE_UNUSED __attribute__((unused))
#else  // Non-GCC equivalents
#define ZKX_FFI_ATTRIBUTE_UNUSED
#endif

// Use captureless lambda to function pointer conversion to create a static
// ZKX_FFI_Handler function pointer variable.

// Use explicit binding specification to create a handler.
#define ZKX_FFI_DEFINE_HANDLER_EXPLICIT(fn, impl, binding)                    \
  static constexpr ZKX_FFI_Handler* fn = +[](ZKX_FFI_CallFrame* call_frame) { \
    static auto* handler = binding.To(impl).release();                        \
    return handler->Call(call_frame);                                         \
  }

#define ZKX_FFI_DEFINE_HANDLER_EXPLICIT_WITH_TRAITS(fn, impl, binding, traits) \
  static constexpr ZKX_FFI_Handler* fn = +[](ZKX_FFI_CallFrame* call_frame) {  \
...
#define ZKX_FFI_DEFINE_HANDLER_AUTO(fn, impl)                                 \
  static constexpr ZKX_FFI_Handler* fn = +[](ZKX_FFI_CallFrame* call_frame) { \
    static auto* handler = ::zkx::ffi::Ffi::BindTo(impl).release();           \
    return handler->Call(call_frame);                                         \
  }

#define ZKX_FFI_DEFINE_HANDLER_X(x, fn, impl, binding, traits, FUNC, ...) FUNC

// Define ZKX FFI handler as a static function pointer variable, which allows
...
#define ZKX_FFI_DEFINE_HANDLER(fn, impl, ...)                             \
  ZKX_FFI_DEFINE_HANDLER_X(                                               \
      , fn, impl, ##__VA_ARGS__,                                          \
      ZKX_FFI_DEFINE_HANDLER_EXPLICIT_WITH_TRAITS(fn, impl, __VA_ARGS__), \
      ZKX_FFI_DEFINE_HANDLER_EXPLICIT(fn, impl, __VA_ARGS__),             \
      ZKX_FFI_DEFINE_HANDLER_AUTO(fn, impl))
...
// FFI handlers.
#define ZKX_FFI_REGISTER_HANDLER(API, NAME, PLATFORM, FUNC, ...)    \
  ZKX_FFI_REGISTER_HANDLER_(API, NAME, PLATFORM, FUNC, __COUNTER__, \
                            ##__VA_ARGS__)
#define ZKX_FFI_REGISTER_HANDLER_(API, NAME, PLATFORM, FUNC, N, ...) \
  ZKX_FFI_REGISTER_HANDLER__(API, NAME, PLATFORM, FUNC, N, ##__VA_ARGS__)
#define ZKX_FFI_REGISTER_HANDLER__(API, NAME, PLATFORM, FUNC, N, ...)       \
  ZKX_FFI_ATTRIBUTE_UNUSED static const ZKX_FFI_Error*                      \
      ZKX_ffi_static_handler_##N##_registered_ = [] {                       \
        return ::zkx::ffi::Ffi::RegisterStaticHandler(API, NAME, PLATFORM,  \
                                                      FUNC, ##__VA_ARGS__); \
      }()

// Following two APIs are intended for users who want to export ZKX FFI handler
// from a shared library as a C function symbol.

// Declares C function that implements FFI handler.
#define ZKX_FFI_DECLARE_HANDLER_SYMBOL(fn) \
  extern "C" ZKX_FFI_Error* fn(ZKX_FFI_CallFrame* call_frame)

// Defines C function that implements FFI handler.
#define ZKX_FFI_DEFINE_HANDLER_SYMBOL(fn, impl, ...)                           \
  extern "C" ZKX_FFI_Error* fn(ZKX_FFI_CallFrame* call_frame) {                \
    ZKX_FFI_DEFINE_HANDLER(handler, impl, ##__VA_ARGS__);                      \
    return (*handler)(call_frame);                                             \
  }                                                                            \
                                                                               \
  static_assert(                                                               \
      std::is_invocable_r_v<ZKX_FFI_Error*, decltype(fn), ZKX_FFI_CallFrame*>, \
      "FFI handler must return ZKX_FFI_Error* and accept ZKX_FFI_CallFrame*")

}  // namespace zkx::ffi

#endif  // ZKX_FFI_API_API_H_
