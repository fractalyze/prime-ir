...
#include "zkx/ffi/attribute_map.h"
...
absl::StatusOr<CallFrameBuilder::Attribute> ConvertBoolAttr(
    std::string_view name, mlir::BoolAttr boolean) {
  return static_cast<bool>(boolean.getValue());
}

absl::StatusOr<CallFrameBuilder::Attribute> ConvertStringAttr(
    std::string_view name, mlir::StringAttr str) {
  return str.getValue().str();
}

absl::StatusOr<CallFrameBuilder::Attribute> ConvertIntegerAttr(
...
absl::StatusOr<CallFrameBuilder::Attribute> ConvertArrayAttr(
...
template <typename T>
std::vector<T> CopyDenseElementsToVec(mlir::DenseIntOrFPElementsAttr arr) {
  auto it = arr.getValues<T>();
  return std::vector<T>(it.begin(), it.end());
}

absl::StatusOr<CallFrameBuilder::Attribute> ConvertDenseElementsAttr(
...
absl::StatusOr<CallFrameBuilder::Attribute> ConvertDictionaryAttr(
