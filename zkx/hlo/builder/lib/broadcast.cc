...
absl::StatusOr<ZkxOp> BroadcastTo(ZkxOp input,
  ...
  for (auto output_it = output_dims.rbegin(); output_it != output_dims.rend();
    ...
    if (input_it != input_dims.rend()) {
      ...
      if (*output_it == *input_it || *input_it == 1) {
        ...
        // [O]. We must do this in two phases since ZKX broadcasting does not
        // support tiling.
  ...
  ZkxOp output = BroadcastInDim(input, broadcast_shape, broadcast_dims);
