...
#include "zkx/hlo/ir/hlo_instruction_utils.h"
...
std::optional<int64_t> GetConstantValue(const HloInstruction* inst) {
  ...
  return primitive_util::PrimitiveTypeSwitch<std::optional<int64_t>>(
      [&](auto primitive_type_constant) -> std::optional<int64_t> {
        ...
        // BigInt, field types, and EC points are not representable as int64_t.
        return std::nullopt;
      },
...
bool AlgebraicSimplifierVisitor::IsNonNegative(
  ...
  switch (hlo->opcode()) {
    ...
    case HloOpcode::kConstant: {
      // Unsigned integers, field types, and EC points are always non-negative.
      PrimitiveType type = hlo->shape().element_type();
      if (primitive_util::IsUnsignedIntegralType(type) ||
          primitive_util::IsFieldType(type) ||
          primitive_util::IsEcPointType(type)) {
        return true;
      }
      if (std::optional<int64_t> value = GetConstantValue(hlo)) {
        return *value >= 0;
      }
      return false;
    }
...
absl::Status AlgebraicSimplifierVisitor::HandleAnd(
  ...
  if (ShapeUtil::HasPrimitiveType(lhs->shape(), PRED) &&
      ShapeUtil::HasPrimitiveType(rhs->shape(), PRED)) {
    // A && True => A
...
namespace {
HloInstruction* BuildTupleConstant(HloComputation* computation,
  ...
  if (ShapeUtil::ElementIsSigned(divide->shape())) {
    if constexpr (std::is_convertible_v<T, int64_t>) {
      ...
      if (b_value > 0 && absl::has_single_bit(static_cast<uint64_t>(b_value))) {
        ...
        auto* abs_dividend =
            divide->AddInstruction(HloInstruction::CreateTernary(
        ...
        auto* quotient = divide->AddInstruction(HloInstruction::CreateBinary(
            ...
            MakeScalarLike(abs_dividend,
                           zkx::base::Log2Floor<uint64_t>(b_value))));
        ...
        return HloInstruction::CreateTernary(
            divide->shape(), HloOpcode::kSelect, dividend_is_negative,
            neqated_quotient, quotient);
      }
    }  // constexpr if (is_convertible_v<T, int64_t>)
  } else {
    if constexpr (std::is_convertible_v<T, uint64_t>) {
      uint64_t b_value =
          static_cast<uint64_t>(c->literal().GetFirstElement<T>());
      if (absl::has_single_bit(b_value)) {
        return HloInstruction::CreateBinary(
            divide->shape(), HloOpcode::kShiftRightLogical, a,
            MakeScalarLike(a, zkx::base::Log2Floor(b_value)));
      }
    }  // constexpr if (is_convertible_v<T, uint64_t>)
  }
...
AlgebraicSimplifierVisitor::RemoveDegenerateDimensionFromDot(
  ...
  TF_ASSIGN_OR_RETURN(auto new_dot,
                      MakeDotHlo(new_lhs, new_rhs, new_dnums, std::nullopt,
...
//
//  bcast = s32[1,2,3,4] broadcast(s32[2,4] x), dimensions={1,3}
//  trans = s32[2,3,1,4] transpose(s32[1,2,3,4] bcast), dimensions={1,2,0,3}
//
// We want to transform this into
//
//  bcast' = s32[2,3,1,4] broadcast(s32[2,4] x), dimensions={0,3}
...
absl::StatusOr<HloInstruction*> AlgebraicSimplifierVisitor::OptimizeDotOfGather(
  ...
  auto* memoized_inst = dot->AddInstruction(HloInstruction::CreateDot(
...
AlgebraicSimplifierVisitor::OptimizeDotOfReorderContractingDims(
  ...
  //   t2 = s32[2, 6] reshape(t1)
  //   t3 = s32[6, 2] constant(...)
  //   dot = s32[2, 2] dot(t2, t3) lhs_contracting_dims={1},
  ...
  //
  //   t0 = s32[2, 2, 3] parameter(0)
  //   t2' = s32[2, 6] reshape(t0)
  //   t3' = s32[6, 2] ops-to-be-filled ...
  //   dot = s32[2, 2] dot(t2', t3') lhs_contracting_dims={1},
  //                                 rhs_contracting_dims={0}
  //
  ...
  HloInstruction* new_dot = dot->AddInstruction(
      HloInstruction::CreateDot(dot->shape(), lhs, rhs, dnums));
  return new_dot;
}
...
AlgebraicSimplifierVisitor::AssociativeReorderNestedDot(HloDotInstruction* dot,
  ...
  if (old_flops / static_cast<double>(new_flops) >
    ...
    TF_ASSIGN_OR_RETURN(new_inner, MakeDotHlo(new_inner_lhs, new_inner_rhs,
                                              new_inner_dnums, std::nullopt));
    ...
    TF_ASSIGN_OR_RETURN(new_outer, MakeDotHlo(new_outer_lhs, new_outer_rhs,
                                              new_outer_dnums, std::nullopt));
...
// Describes the passthrough dimensions for a gather or scatter operand.
// TODO(batzor): Move to `hlo_sharding_util` once sharding support is available.
...
GatherScatterDims GetGatherScatterOperandPassthroughDims(
  ...
  for (int64_t i = 0; i < operand_shape.rank(); ++i) {
    ...
    if (slice_sizes[i] != operand_shape.dimensions(i)) {
      continue;
    }
...
absl::Status AlgebraicSimplifierVisitor::HandleGather(HloInstruction* gather) {
  ...
  const GatherScatterDims operand_passthrough_dims =
      GetGatherOperandPassthroughDims(*gather, gather->gather_slice_sizes());
...
absl::Status AlgebraicSimplifierVisitor::HandleMultiply(
  ...
  // 0 * RHS => 0
  if (IsAll(lhs, 0) && ReplaceInstructionIfCompatible(multiply, lhs)) {
    return absl::OkStatus();
  }
  // LHS * 0 => 0
  if (IsAll(rhs, 0) && ReplaceInstructionIfCompatible(multiply, rhs)) {
    return absl::OkStatus();
  }
...
absl::Status AlgebraicSimplifierVisitor::HandleCompare(
  ...
  if (primitive_util::IsUnsignedIntegralType(
          Cast<HloCompareInstruction>(compare)->primitive_type())) {
    // X u<  0 -> false
  ...
  // to `ones()`. Consequently, the mask will be converted from s32 to bool,
  ...
  // %sel0 = s32 select(%cmp0, ones, zeros)
  ...
  if (ShapeUtil::ElementIsSigned(remainder->shape())) {
    ...
    if constexpr (std::is_convertible_v<T, uint64_t>) {
      uint64_t b_value =
          static_cast<uint64_t>(c->literal().GetFirstElement<T>());
      if (absl::has_single_bit(b_value)) {
        HloInstruction* mask_amount = remainder->AddInstruction(
...
absl::Status AlgebraicSimplifierVisitor::HandleReshape(
  ...
  // clang-format off
  // TODO(batzor): Uncomment this. Dependency: kRng
...
absl::Status AlgebraicSimplifierVisitor::HandleReverse(
  ...
  if (!options_.is_layout_sensitive()) {
    ...
    if (inner->IsElementwise() && inner->operand_count() == 2) {
      if (inner_opcode == HloOpcode::kCompare) {
        return absl::OkStatus();
      }
      HloInstruction* hlo;
...
absl::Status AlgebraicSimplifierVisitor::HandleSlice(HloInstruction* slice) {
  ...
  if (Match(slice, m::Slice(m::Pad(&pad, m::Op(&pad_operand), m::Op())))) {
    ...
    // Does this slice slice out padding only.
    ...
    for (int64_t i = 0; i < slice->shape().rank(); ++i) {
      ...
      if (start != low || stride != 1) {
        slice_undoes_pad = false;
      }

      if (start < low || limit > edge || stride != 1) {
  ...
  if (!options_.is_layout_sensitive() &&
    ...
    TF_ASSIGN_OR_RETURN(HloInstruction * new_dot,
                        MakeDotHlo(new_lhs, new_rhs, dnums, std::nullopt,
                                   new_sparsity, new_meta));
...
namespace {
bool MatchArgMinMax(const HloInstruction* hlo, bool is_max) {
...
//                       tuple
//
bool MatchArgMax(const HloInstruction* hlo) {
  // Match on variadic Reduce ArgMax.
  if (hlo->opcode() != HloOpcode::kReduce || hlo->operand_count() != 4 ||
      !hlo->shape().IsTuple() ||
      hlo->operand(1)->opcode() != HloOpcode::kIota ||
      !IsAll(hlo->operand(2),
             LiteralUtil::MinValue(hlo->operand(2)->shape().element_type())) ||
      !IsScalarConstantZero(hlo->operand(3))) {
    return false;
  }
...
bool ReductionComputationsEquivalent(const HloComputation& a,
...
absl::Status AlgebraicSimplifierVisitor::HandleReduce(HloInstruction* hlo) {
  ...
  if (options_.raise_slice_and_reduce_through_dot()) {
    ...
    if (Match(arg, m::Dot(m::Op(&a), m::Op(&b))) &&
      ...
      TF_ASSIGN_OR_RETURN(
          HloInstruction * new_dot,
          MakeDotHlo(reduce_a, reduce_b, new_dot_dnums, std::nullopt));
  ...
  if (options_.supports_non_canonical_dots() &&
    ...
    TF_ASSIGN_OR_RETURN(auto new_dot,
                        MakeDotHlo(lhs, rhs, new_dnums, std::nullopt,
  ...
  // Match on Reduce Max with init value MinValue.
  if (reduce->operand_count() == 2 &&
      IsAll(init_value,
            LiteralUtil::MinValue(init_value->shape().element_type())) &&
      Match(reduce->to_apply()->root_instruction(),
  ...
  // Match on Reduce Min with init value MaxValue.
  if (reduce->operand_count() == 2 &&
      IsAll(init_value,
            LiteralUtil::MaxValue(init_value->shape().element_type())) &&
      Match(reduce->to_apply()->root_instruction(),
...
absl::Status AlgebraicSimplifierVisitor::HandleSelect(HloInstruction* select) {
  ...
  if (ShapeUtil::HasPrimitiveType(select->shape(), PRED)) {
  ...
  if (options_.supports_non_canonical_dots() &&
    ...
    TF_ASSIGN_OR_RETURN(bool did_transform, [&]() -> absl::StatusOr<bool> {
      ...
      *new_dnums.mutable_rhs_contracting_dimensions() =
      ...
      HloInstruction* new_dot =
          MakeDotHlo(rhs, lhs, new_dnums, std::nullopt).value();
