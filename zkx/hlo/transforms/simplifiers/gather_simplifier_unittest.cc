...
TEST_F(GatherSimplifierTest, TransformsStartIndices) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      ...
      ROOT gather = s32[42,43,7,8] gather(operand, indices),
          offset_dims={2,3},
  ...
  RunAndFilecheckHloRewrite(kModuleStr, GatherSimplifier(), R"(
         ...
         CHECK: %[[GATHER:.*]] = s32[1806,7,8]{{.*}} gather(
    ...
    CHECK-SAME:     slice_sizes={7,8}
         CHECK: ROOT %{{.*}} = s32[42,43,7,8]{3,2,1,0} reshape(%[[GATHER]])
  )");
}
...
TEST_F(GatherSimplifierTest, RemovesCollapsedSliceDims) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      ...
      ROOT gather = s32[42] gather(operand, indices),
          offset_dims={},
  ...
  RunAndFilecheckHloRewrite(kModuleStr, GatherSimplifier(), R"(
           CHECK: %[[GATHER:.*]] = s32[42,1,1]{2,1,0} gather(%operand, %indices)
      CHECK-SAME:     offset_dims={1,2},
      CHECK-SAME:     collapsed_slice_dims={},
           CHECK: ROOT %{{.*}} = s32[42]{0} reshape(%[[GATHER]])
  )");
}
...
TEST_F(GatherSimplifierTest, MakesStartIndexMapIdentity) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      operand = s32[33,34,35] parameter(0)
      indices = s32[42,3] parameter(1)
      ROOT gather = s32[42,1,2,3] gather(operand, indices),
          offset_dims={1,2,3},
          collapsed_slice_dims={},
  ...
  RunAndFilecheckHloRewrite(kModuleStr, GatherSimplifier(), R"(
  %operand = s32[33,34,35]{2,1,0} parameter(0)
           CHECK: %[[OPERAND:.*]] = s32[35,33,34]{2,1,0} transpose(%operand)
           CHECK: %[[GATHER:.*]] = s32[42,3,1,2]{{.*}} gather(%[[OPERAND]],
      CHECK-SAME:    start_index_map={0,1,2},
           CHECK: ROOT {{.*}} = s32[42,1,2,3]{{.*}} transpose(%[[GATHER]])
  )");
}
...
TEST_F(GatherSimplifierTest, CollapsesSomeDims) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      operand = s32[33,34,35] parameter(0)
      indices = s32[42,1] parameter(1)
      ROOT gather = s32[7,42] gather(operand, indices),
          offset_dims={0},
  ...
  RunAndFilecheckHloRewrite(kModuleStr, GatherSimplifier(), R"(
           CHECK: %[[GATHER:.*]] = s32[42,1,7,1]{3,2,1,0} gather(
           CHECK: %[[COLLAPSED:.*]] = s32[42,7]{1,0} reshape(%[[GATHER]])
           CHECK: ROOT {{.*}} = s32[7,42]{1,0} transpose(%[[COLLAPSED]]),
...
TEST_F(GatherSimplifierTest, ZeroDimStartIndices) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      operand = s32[8,16] parameter(0)
      indices = s32[2] parameter(1)

      ROOT gather = s32[8,16] gather(s32[8,16] operand, s32[2] indices),
          offset_dims={0,1},
          collapsed_slice_dims={},
          start_index_map={0,1},
          index_vector_dim=0,
...
TEST_F(GatherSimplifierTest, ZeroSizeSlice) {
  ...
  constexpr std::string_view kModuleStr = R"(
    ...
    ENTRY kernel_entry {
      operand = s32[0,2] parameter(0)
      indices = s32[3] parameter(1)

      ROOT gather = s32[3,2] gather(s32[0,2] operand, s32[3]{0} indices),
          offset_dims={1},
  ...
  RunAndFilecheckHloRewrite(kModuleStr, GatherSimplifier(), R"(
      ...
      CHECK: ROOT {{.*}} = s32[3,2]{1,0} broadcast(%[[ZERO]]), dimensions={}
  )");
}
