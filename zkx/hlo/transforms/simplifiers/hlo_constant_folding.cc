...
// shouldn't fold.
//
// NOTE(junbeomlee): Removed kRng check — ZKX has no kRng opcode.
//
...
bool IsOrContainsIllegalInstr(const HloInstruction* instr) {
...
absl::StatusOr<bool> HloConstantFolding::Run(
  ...
  // NOTE(junbeomlee): Removed set_use_fast_path — ZKX evaluator has no Eigen
  // fast path for ZK math.
  ...
  for (auto* computation :
    ...
    for (auto* instruction : computation->MakeInstructionPostOrder()) {
      ...
      // NOTE(junbeomlee): Removed kFft skip — ZKX has no FFT ops.
      ...
      SlowOperationAlarm slow_alarm(slow_timeout, [instruction, slow_timeout] {
        ...
        std::string_view explanation_msg =
            ndebug
                ...
                : "ZKX was built without compiler optimizations, which can be "
