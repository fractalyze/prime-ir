...
#include "zkx/hlo/builder/lib/broadcast.h"
...
absl::StatusOr<HloInstruction*> BitcastDtypesExpander::ExpandInstruction(
  ...
  if (!computation) {
    ...
    if (input_bit_width > output_bit_width) {
      ...
      ZkxOp iota = Iota(&b, input_shape, input_shape.dimensions_size() - 1);
      ZkxOp iota_m = Mul(ScalarLike(input, output_bit_width), iota);
    ...
    TF_ASSIGN_OR_RETURN(ZkxComputation zkx_computation, b.Build());
    TF_ASSIGN_OR_RETURN(
        computation, ZkxComputationToHloComputation(zkx_computation, module));
  }
