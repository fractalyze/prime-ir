...
LocalDeviceState::LocalDeviceState(se::StreamExecutor* executor,
  ...
  // Setting ZKX_PJRT_GPU_ALLOW_DELETE_BEFORE_FULFILL to false will:
  ...
  absl::Status status =
      tsl::ReadBoolFromEnvVar("ZKX_PJRT_GPU_ALLOW_DELETE_BEFORE_FULFILL", true,
                              &allow_delete_before_fulfill_);
  if (!status.ok()) {
    LOG(ERROR) << "Failed to read ZKX_PJRT_GPU_ALLOW_DELETE_BEFORE_FULFILL: "
               << status;
  }
  ...
  execute_thread_ =
      std::make_unique<WorkerThread>(tsl::Env::Default(), "py_zkx_execute");
  callback_thread_ =
      std::make_unique<WorkerThread>(tsl::Env::Default(), "py_zkx_callback");
  cleanup_thread_ =
      std::make_unique<WorkerThread>(tsl::Env::Default(), "py_zkx_cleanup");
}
...
absl::Status LocalDeviceState::SynchronizeAllActivity() {
  ...
  if (!ok) {
    status.Update(absl::UnknownError("SynchronizeAllActivity failed."));
  }
  return status;
}
