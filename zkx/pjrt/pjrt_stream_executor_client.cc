...
#include "zkx/pjrt/pjrt_stream_executor_client.h"
...
PjRtStreamExecutorMemorySpace::PjRtStreamExecutorMemorySpace(
    int id, PjRtDevice* device, std::string_view kind, int kind_id)
...
std::string_view PjRtStreamExecutorDevice::platform_name() const {
...
PjRtStreamExecutorDevice::GetLocalDeviceState() const {
  ...
  return absl::InvalidArgumentError(
      absl::StrFormat("Device %s is not a local device.", DebugString()));
}
...
absl::StatusOr<DeviceAssignment> DevicesToDeviceAssignment(
  ...
  DeviceAssignment assignment(devices.size(), devices[0].size());
  for (int replica = 0; replica < devices.size(); ++replica) {
    ...
    for (int partition = 0; partition < devices[replica].size(); ++partition) {
      ...
      assignment(replica, partition) = devices[replica][partition]->id();
    }
  }
  return assignment;
}
...
AllocateDestinationBuffer(
  ...
  if (on_host_shape.IsTuple() && on_host_shape.tuple_shapes_size() == 0) {
    return absl::InvalidArgumentError(
        "Can't make a buffer from an empty tuple");
  }
  ...
  if (memory_space != default_memory_space && !is_pinned_host_memory) {
    return absl::InvalidArgumentError(
        "Buffer allocation: invalid memory space");
  }
...
PjRtStreamExecutorClient::BufferFromHostBufferInternal(
  ...
  auto transfer_h2d =
      [local_client = client(), local_device, data, size, type, packed_size,
       ...
       host_buffer_semantics, transpose{std::move(transpose)}]() {
        ...
        CHECK_OK(local_device->ThenExecuteCallback(
            local_device->host_to_device_stream(),
            [staging_buffer{std::move(staging_buffer)},
             on_done_with_host_buffer{
...
PjRtStreamExecutorClient::MakeCrossHostReceiveBuffersForGather(
  ...
  if (shapes.size() != gather_details.size()) {
    return absl::InvalidArgumentError(
        absl::StrFormat("gather_details parameter has length %lld but shapes "
...
absl::StatusOr<PjRtDevice*> PjRtStreamExecutorClient::LookupAddressableDevice(
    zkx::PjRtLocalDeviceId local_device_id) const {
            ...
            [events_to_wait_for_in_a_different_thread =
             ...
             local_device_state, device_buffer, block_stream]() mutable {
              ...
              if (block_stream != nullptr) {
                CHECK_OK(local_device_state->ThenExecuteCallback(
...
void PjRtStreamExecutorBuffer::Delete() {
  ...
  CHECK_OK(Release(/*wait_for_operations_to_complete=*/false).status());
}
...
PjRtFuture<> PjRtStreamExecutorBuffer::ToLiteral(MutableLiteralBase* literal) {
  ...
  if (IsEmptyTuple()) {
    return PjRtFuture<>(
        absl::InvalidArgumentError("ToLiteral called on empty tuple"));
  }
...
PjRtStreamExecutorBuffer::CopyToMemorySpace(PjRtMemorySpace* dst_memory_space) {
  ...
  return absl::UnimplementedError("CopyToMemorySpace is not supported");
}
...
absl::StatusOr<std::unique_ptr<TupleHandle>> MakeTupleHelper(
  ...
  if (tupled_parameter_shape.tuple_shapes_size() != py_buffers.size()) {
    return absl::InvalidArgumentError(absl::StrFormat(
        "Executable expected %lld parameters but got %lld",
        tupled_parameter_shape.tuple_shapes_size(), py_buffers.size()));
  }
...
std::string_view PjRtStreamExecutorLoadedExecutable::name() const {
...
PjRtStreamExecutorLoadedExecutable::MakeExecutionInputsAndWaitForEvents(
  ...
  if (parameter_is_tupled_arguments_ && !options.arguments_are_tupled) {
    ...
    if (argument_handles.size() != executable_parameter_shapes.size()) {
      return absl::InvalidArgumentError(absl::StrFormat(
          "Executable expected %lld arguments but got %lld",
          executable_parameter_shapes.size(), argument_handles.size()));
    }
        ...
        [references{std::make_tuple(executables_[executable_idx],
         ...
         device_ordinal]() {
          for (const auto& ptr : donated_ptrs) {
            CHECK_OK(allocator->Deallocate(device_ordinal, ptr));
          }
        });
  } else {
    ...
    if (device_state->allocation_model() == LocalDeviceState::kSynchronous) {
      ...
      compute_callbacks.push_back(
          [root_buffer, allocator{client_->allocator()}, device_ordinal]() {
            CHECK_OK(allocator->Deallocate(device_ordinal, root_buffer));
          });
    }
  } else {
    TF_ASSIGN_OR_RETURN(
...
absl::Status GetFirstInputError(
    absl::Span<PjRtBuffer* const> argument_handles) {
...
PjRtStreamExecutorLoadedExecutable::ExecuteHelper(
  ...
  // TODO(chokobole): Uncomment this. Dependency: metrics
...
PjRtStreamExecutorLoadedExecutable::GetHloModules() const {
  ...
  for (const auto& local_exec : executables()) {
    if (!local_exec->executable()->has_module()) {
      return absl::InvalidArgumentError(
          "Executable does not have HLO modules.");
    }
...
absl::StatusOr<std::string_view> MemoryKindFromSimpleShape(
    const Shape& shape, std::string_view default_memory_kind) {
  if (!shape.has_layout()) {
  ...
  switch (shape.layout().memory_space()) {
    ...
    default:
      return absl::InvalidArgumentError(
          absl::StrFormat("Unexpected memory space %d in output layout",
                          shape.layout().memory_space()));
  }
}

absl::StatusOr<std::vector<std::string_view>> MemoryKindsFromShape(
    const Shape& shape, std::string_view default_memory_kind) {
  if (!shape.IsTuple()) {
    TF_ASSIGN_OR_RETURN(std::string_view memory_kind,
  ...
  for (const auto& element_shape : shape.tuple_shapes()) {
    TF_ASSIGN_OR_RETURN(
        std::string_view element_memory_kind,
...
PjRtStreamExecutorLoadedExecutable::GetOutputMemoryKinds() const {
  ...
  for (const auto& shape : shapes) {
    TF_ASSIGN_OR_RETURN(
        std::vector<std::string_view> memory_kind,
...
PjRtStreamExecutorClient::CompileInternal(
  ...
  if (ex_options.has_debug_options() &&
      ex_options.debug_options().zkx_gpu_dump_hlo_unoptimized_snapshots()) {
...
PjRtStreamExecutorClient::Compile(mlir::ModuleOp module,
  ...
  TF_ASSIGN_OR_RETURN(auto arg_layouts_and_pointers,
                      LayoutModesToZkx(
                          zkx_computation, arg_layout_modes, out_layout_modes,
...
absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>>
PjRtStreamExecutorClient::Compile(const ZkxComputation& computation,
                                  CompileOptions options) {
  std::vector<const Shape*> argument_layout_pointers;
...
PjRtStreamExecutorClient::DeserializeExecutable(
    std::string_view serialized, std::optional<CompileOptions> options) {
  ExecutableAndOptionsProto proto;
