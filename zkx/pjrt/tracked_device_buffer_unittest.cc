...
class TestDevice : public PjRtDevice {
 public:
  ...
  absl::Status TransferToInfeed(const LiteralSlice& literal) override {
    return absl::UnimplementedError("Unimplemented for TestDeivce.");
  }
...
TEST(TrackedDeviceBufferTest, AsShapedBuffer) {
  ...
  Shape a_shape = ShapeUtil::MakeShape(S32, {3, 101, 4});
...
TEST(TrackedDeviceBufferTest, FromScopedShapedBuffer) {
  ...
  Literal literal = LiteralUtil::MakeTupleOwned(
      LiteralUtil::CreateFullWithDescendingLayout<int32_t>({10, 3, 7}, 33),
      LiteralUtil::One(S64));
