...
// compatibility window of at least 12 weeks.
constexpr int kMinPjRtMinor = 29;

namespace {

// The bool indicates whether this plugin has been initialized.
absl::flat_hash_map<std::string, std::pair<const PJRT_Api*, bool>>* pjrt_apis =
    nullptr;
...
absl::StatusOr<const PJRT_Api*> PjrtApi(std::string_view device_type) {
...
absl::Status SetPjrtApi(std::string_view device_type, const PJRT_Api* api) {
  if (pjrt_apis == nullptr) {
...
absl::StatusOr<const PJRT_Api*> LoadPjrtPlugin(std::string_view device_type,
...
absl::StatusOr<bool> IsPjrtPluginInitialized(std::string_view device_type) {
  if (pjrt_apis == nullptr) {
    return absl::FailedPreconditionError("PJRT_Api is not initialized.");
  }
  std::string canonicalize_device_type = CanonicalizeDeviceType(device_type);
...
absl::Status InitializePjrtPlugin(std::string_view device_type) {
  std::string canonicalize_device_type = CanonicalizeDeviceType(device_type);
  auto iter = pjrt_apis->find(canonicalize_device_type);
  if (iter == pjrt_apis->end()) {
    return absl::NotFoundError(absl::StrCat(
