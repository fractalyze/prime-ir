...
#include "zkx/stream_executor/integrations/device_mem_allocator.h"
...
absl::StatusOr<LocalClient*> GetGpuZkxClient(
  ...
  if (platform->VisibleDeviceCount() <= 0) {
    return absl::FailedPreconditionError("No visible GPU devices.");
  }
...
absl::StatusOr<std::unique_ptr<tsl::BFCAllocator>> CreateBFCAllocator(
  ...
  if (!executor->DeviceMemoryUsage(&free_memory, &total_memory)) {
    return absl::UnavailableError(absl::StrCat(
        "Failed to query available memory from device ", device_ordinal));
  }
