...
#include "zkx/pjrt/gpu/gpu_metrics.h"
#include "zkx/pjrt/gpu/nccl_id_store.h"
#include "zkx/pjrt/host_memory_spaces.h"
#include "zkx/pjrt/stream_executor_executable.h"
...
std::optional<stream_executor::GpuTargetConfigProto> GetTargetConfigForDevices(
  ...
  for (const PjRtDevice* device : devices) {
    ...
    if (local_device_state != nullptr) {
      return Compiler::TargetConfig(local_device_state->executor()).ToProto();
    }
  }
  return std::nullopt;
}
...
StreamExecutorGpuClient::StreamExecutorGpuClient(
    ...
    : PjRtStreamExecutorClient(
      ...
      topology_(StreamExecutorGpuTopologyDescription(
...
std::string_view StreamExecutorGpuClient::platform_version() const {
...
StreamExecutorGpuClient::CreateBuffersForAsyncHostToDevice(
  ...
  return ::zkx::AsyncHostToDeviceTransferManager::Create(
...
PjRtFuture<> StreamExecutorGpuClient::CopyRawSubBufferToHost(
  ...
  auto async_copy = [this, promise, offset, transfer_size, stream, local_device,
    ...
    if (offset < 0 || offset > device_memory.size() ||
      ...
      promise.Set(absl::InvalidArgumentError(
          absl::StrFormat("Copy raw buffer called on buffer size %lld with "
...
StreamExecutorGpuClient::Compile(const ZkxComputation& computation,
...
absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>>
StreamExecutorGpuClient::LoadSerialized(std::string_view serialized,
...
StreamExecutorGpuClient::Load(std::unique_ptr<PjRtExecutable> executable) {
  ...
  for (std::unique_ptr<AotCompilationResult>& aot_executable :
...
BuildLocalDeviceStates(LocalClient* zkx_client) {
  ...
  for (se::StreamExecutor* executor :
    ...
    addressable_devices.emplace(
        ...
        std::make_unique<LocalDeviceState>(
            executor, zkx_client, LocalDeviceState::kComputeSynchronized,
...
GetStreamExecutorGpuDeviceAllocator(
  ...
  if (debug_options.zkx_gpu_temp_buffer_use_separate_color()) {
...
void NameDeviceAndLauncherThread(const LocalTopologyProto& node,
  ...
  // is created.
  launcher_thread->Schedule([name = absl::StrCat("ZkxLauncher", suffix)] {
...
absl::StatusOr<DeviceTopologyPair> BuildDistributedDevices(
  ...
  for (const auto& ordinal_and_device : local_device_states) {
    ...
    TF_ASSIGN_OR_RETURN(
        std::unique_ptr<se::DeviceDescription> desc,
...
std::string MakeComputeCapabilityString(const se::DeviceDescription* desc) {
  ...
  if (std::holds_alternative<se::CudaComputeCapability>(cc)) {
    ...
    return std::string(rocmcc.gfx_version());
  } else {
    return "unknown";
  }
}
...
StreamExecutorGpuDevice::StreamExecutorGpuDevice(
  ...
  description().SetAttributes(
      {{"coords", PjRtDeviceAttribute(v_coords)},
       {"device_vendor", device_vendor_},
       {"slice_index", static_cast<int64_t>(slice_index)},
       {"compute_capability", PjRtDeviceAttribute(compute_capability)},
...
std::string_view StreamExecutorGpuDevice::device_vendor() const {
...
absl::StatusOr<std::unique_ptr<PjRtClient>> GetStreamExecutorGpuClient(
  ...
  auto pjrt_platform_name = RocmName();
#elif TENSORFLOW_USE_SYCL
  auto pjrt_platform_name = SyclName();
#else   // TENSORFLOW_USE_ROCM
  auto pjrt_platform_name = CudaName();
#endif  // TENSORFLOW_USE_ROCM

  TF_ASSIGN_OR_RETURN(
      LocalClient * zkx_client,
      GetGpuZkxClient(options.platform_name, options.allowed_devices));
  std::map<int, std::unique_ptr<LocalDeviceState>> local_device_states;
  TF_ASSIGN_OR_RETURN(local_device_states, BuildLocalDeviceStates(zkx_client));
  if (local_device_states.empty()) {
    return absl::InvalidArgumentError("No GPU devices found");
  }
  EnablePeerAccess(zkx_client->backend().stream_executors());
  TF_ASSIGN_OR_RETURN(auto allocator,
                      GetStreamExecutorGpuDeviceAllocator(
                          zkx_client->platform(), options.allocator_config,
                          local_device_states));
  TF_ASSIGN_OR_RETURN(
  ...
  static const bool zkx_gpu_require_exclusive_lock =
      GetDebugOptionsFromFlags().zkx_gpu_require_exclusive_lock();
  if (zkx_gpu_require_exclusive_lock) {
  ...
  return std::unique_ptr<PjRtClient>(std::make_unique<StreamExecutorGpuClient>(
      pjrt_platform_name, zkx_client, std::move(device_topology_pair.first),
