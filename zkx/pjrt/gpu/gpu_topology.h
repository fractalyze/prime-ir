...
#ifndef ZKX_PJRT_GPU_GPU_TOPOLOGY_H_
#define ZKX_PJRT_GPU_GPU_TOPOLOGY_H_
...
class GpuTopology {
 public:
  explicit GpuTopology(const std::vector<int>& gpu_device_ids,
                       std::string_view platform_version, int32_t num_slices,
                       int32_t num_hosts_per_slice,
                       int32_t num_devices_per_host)
      : device_ids_(gpu_device_ids),
  ...
  bool operator==(const GpuTopology& other) const {
    return device_ids_ == other.device_ids_ &&
 ...
 private:
  const std::vector<int> device_ids_;
...
#endif  // ZKX_PJRT_GPU_GPU_TOPOLOGY_H_
