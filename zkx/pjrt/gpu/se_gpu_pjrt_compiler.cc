...
absl::StatusOr<Compiler*> GetCompilerForDefaultGpuPlatform() {
  TF_ASSIGN_OR_RETURN(se::Platform * platform,
                      PlatformUtil::GetPlatform("gpu"));
  return Compiler::GetForPlatform(platform);
}

absl::StatusOr<Compiler*> GetCompilerForPlatform(
    std::optional<se::Platform::Id> platform_id) {
  if (!platform_id.has_value()) {
    return GetCompilerForDefaultGpuPlatform();
  }

  TF_ASSIGN_OR_RETURN(se::Platform * platform,
                      se::PlatformManager::PlatformWithId(platform_id.value()));
  return Compiler::GetForPlatform(platform);
}
...
StreamExecutorGpuCompiler::Compile(CompileOptions options,
  ...
  if (!options.target_config) {
    ...
    const auto& gpu_topology =
        tsl::down_cast<const StreamExecutorGpuTopologyDescription&>(topology);
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      gpu_compiler->CompileAheadOfTime(std::move(unique_module_group),
                                       aot_options));
  std::vector<std::vector<std::string_view>> output_memory_kinds(1);
...
StreamExecutorGpuCompiler::Compile(CompileOptions options,
  ...
  TF_RETURN_IF_ERROR(MlirToZkxComputation(
      ...
      /*return_tuple=*/false,
      /*use_shardy=*/false));
  return Compile(std::move(input_options), zkx_computation, topology, client);
}
