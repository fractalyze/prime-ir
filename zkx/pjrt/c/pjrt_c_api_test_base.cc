...
#include "zkx/pjrt/c/pjrt_c_api_wrapper_impl.h"  // IWYU pragma: keep
...
std::string PjrtCApiTestBase::BuildSingleDeviceCompileOptionStr() {
  zkx::ExecutableBuildOptions build_options;
  ...
  absl::StatusOr<zkx::CompileOptionsProto> options_proto = options.ToProto();
...
PjrtCApiTestBase::CreateBufferFromHostBufferArgs(
    ...
    const zkx::PjRtClient::HostBufferSemantics host_buffer_semantics,
    PJRT_Device* device) {
...
PjrtCApiTestBase::create_buffer(PJRT_Device* device) {
  ...
  return create_buffer_from_data(int32_data, shape, device);
}

std::pair<std::unique_ptr<PJRT_Buffer, ::pjrt::PJRT_BufferDeleter>,
          zkx::PjRtFuture<>>
PjrtCApiTestBase::create_buffer_from_data(
    const std::vector<int32_t>& int32_data, const zkx::Shape& shape,
    PJRT_Device* device) {
  PJRT_Client_BufferFromHostBuffer_Args args = CreateBufferFromHostBufferArgs(
      int32_data, shape,
      zkx::PjRtClient::HostBufferSemantics::kImmutableOnlyDuringCall, device);
  ...
  zkx::PjRtFuture<> buffer_ready_event =
...
PjrtCApiTestBase::create_transfer_manager(const zkx::Shape& host_shape) {
