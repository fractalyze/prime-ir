...
std::string ProgramFormatErrorMsg(std::string_view program_format) {
...
PJRT_Device* GetCDevice(const PJRT_Client* client,
                        const zkx::PjRtDevice* device) {
...
PJRT_Memory* GetCMemory(const PJRT_Client* client,
                        const zkx::PjRtMemorySpace* memory) {
...
absl::Status PopulateExecutableCostAnalysis(PJRT_Executable* executable) {
  ...
  for (const auto& property : properties) {
    ...
    const zkx::PjRtValueType& property_value = property.second;
...
absl::Status PopulateExecutableOutputElementTypes(PJRT_Executable* executable) {
  ...
  std::vector<zkx::PrimitiveType>& inner_output_types = output_types[0];
...
absl::Status PopulateExecutableOutputDimensions(PJRT_Executable* executable) {
  TF_ASSIGN_OR_RETURN(auto output_dims,
                      executable->get()->GetOutputDimensions());
  if (output_dims.empty()) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Can't get output dimensions, the list is empty for executable ",
        executable->get()->name(), "."));
  }
  ...
  std::vector<zkx::DimensionVector>& inner_output_dims = output_dims[0];
...
absl::Status PopulateExecutableOutputMemoryKinds(PJRT_Executable* executable) {
  TF_ASSIGN_OR_RETURN(
      std::vector<std::vector<std::string_view>> output_memories,
      executable->get()->GetOutputMemoryKinds());
  if (output_memories.empty()) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Can't get output memory kinds, the list is empty for executable ",
        executable->get()->name(), "."));
  }
  ...
  std::vector<std::string_view>& inner_output_memories = output_memories[0];
  ...
  for (std::string_view memory : inner_output_memories) {
...
class CApiKeyValueStore : public zkx::KeyValueStoreInterface {
 public:
...
std::shared_ptr<zkx::KeyValueStoreInterface> ToCppKeyValueStore(
...
PJRT_Error* PJRT_Plugin_Attributes_Zkx(PJRT_Plugin_Attributes_Args* args) {
  PJRT_RETURN_IF_ERROR(ActualStructSizeIsGreaterOrEqual(
      "PJRT_Plugin_Attributes_Args", PJRT_Plugin_Attributes_Args_STRUCT_SIZE,
      args->struct_size));
  const std::vector<PJRT_NamedValue>& attributes =
      pjrt::GetZkxPluginCAttributes();
...
PJRT_Error* PJRT_Client_PlatformName(PJRT_Client_PlatformName_Args* args) {
  ...
  std::string_view platform_name = args->client->client->platform_name();
...
PJRT_Error* PJRT_Client_PlatformVersion(
  ...
  std::string_view platform_version = args->client->client->platform_version();
...
PJRT_Error* PJRT_Client_LookupDevice(PJRT_Client_LookupDevice_Args* args) {
  ...
  PJRT_ASSIGN_OR_RETURN(
      ...
      args->client->client->LookupDevice(zkx::PjRtGlobalDeviceId(args->id)));
...
PJRT_Error* PJRT_Client_LookupAddressableDevice(
  ...
  PJRT_ASSIGN_OR_RETURN(zkx::PjRtDevice * addressable_device,
                        args->client->client->LookupAddressableDevice(
                            zkx::PjRtLocalDeviceId(args->local_hardware_id)));
...
PJRT_Error* PJRT_LoadedExecutable_Fingerprint(
  ...
  return new PJRT_Error{absl::UnimplementedError(
      "PJRT_LoadedExecutable_Fingerprint is deprecated, use "
...
PJRT_Error* PJRT_Client_CreateBuffersForAsyncHostToDevice(
  ...
  PJRT_RETURN_IF_ERROR(ActualStructSizeIsGreaterOrEqual(
      ...
      args->struct_size));
  std::vector<std::optional<zkx::Layout>> device_layouts;
  absl::InlinedVector<zkx::PjRtClient::ShapeSpec, 4> shape_specs;
  ...
  std::optional<absl::Span<const std::optional<zkx::Layout>>>
      arg_device_layouts;
  if (args->num_device_layouts == 0) {
    ...
    for (int i = 0; i < args->num_device_layouts; ++i) {
      std::optional<zkx::Layout> optional_layout;
  ...
  PJRT_ASSIGN_OR_RETURN(
      std::unique_ptr<zkx::PjRtClient::AsyncHostToDeviceTransferManager>
          transfer_manager,
...
// `zkx::PjRtDevice *` (`cpp_device`). If a match is found, that PJRT_Device*
// is returned. Otherwise, returns nullptr.
PJRT_Device* FindDeviceWrapper(zkx::PjRtDevice* cpp_device,
...
PJRT_Memory* PJRT_Client_FindMemoryWrapper(zkx::PjRtMemorySpace* cpp_memory,
                                           PJRT_Client* client) {
...
void PopulatePjrtExecutableAddressableDevices(
  ...
  absl::Span<zkx::PjRtDevice* const> cpp_devices =
  ...
  for (int i = 0; i < num_addressable_devices; ++i) {
    zkx::PjRtDevice* cpp_device = cpp_devices[i];
...
PJRT_Error* PJRT_AsyncHostToDeviceTransferManager_TransferData(
  ...
  PJRT_RETURN_IF_ERROR(ActualStructSizeIsGreaterOrEqual(
      ...
      args->struct_size));
  zkx::PjRtFuture<>::Promise promise = zkx::PjRtFuture<>::CreatePromise();
  ...
  args->done_with_h2d_transfer =
      new PJRT_Event{zkx::PjRtFuture<>(std::move(promise))};
  return nullptr;
}
...
PJRT_Error* PJRT_AsyncHostToDeviceTransferManager_RetrieveBuffer(
  ...
  PJRT_RETURN_IF_ERROR(ActualStructSizeIsGreaterOrEqual(
      ...
      args->struct_size));
  std::unique_ptr<zkx::PjRtBuffer> buffer_out =
...
PJRT_Error* PJRT_AsyncHostToDeviceTransferManager_SetBufferError(
  ...
  PJRT_RETURN_IF_ERROR(ActualStructSizeIsGreaterOrEqual(
      ...
      args->struct_size));
  auto error_message =
      std::string_view(args->error_message, args->error_message_size);
...
absl::StatusOr<zkx::CompileOptions> ParseCompileOptions(
    std::string_view options_str) {
  zkx::CompileOptionsProto options_proto;
  ...
  return zkx::CompileOptions::FromProto(options_proto);
}

using ProgramVariant =
    std::variant<mlir::OwningOpRef<mlir::ModuleOp>, zkx::ZkxComputation>;
absl::StatusOr<
    std::variant<mlir::OwningOpRef<mlir::ModuleOp>, zkx::ZkxComputation>>
ParsePjrtProgram(std::optional<mlir::MLIRContext>& context,
                 const PJRT_Program* program) {
  auto format_str = std::string_view(program->format, program->format_size);
  auto module_str = std::string_view(program->code, program->code_size);
...
const zkx::ZkxComputation& UnpackPjrtProgram(
    const zkx::ZkxComputation& computation) {
  return computation;
}
...
PJRT_Error* PJRT_Client_Compile(PJRT_Client_Compile_Args* args) {
  ...
  // ZKX: Removed - profiler TraceMeConsumer
  ...
  PJRT_ASSIGN_OR_RETURN(std::unique_ptr<zkx::PjRtLoadedExecutable> executable,
                        std::visit(
...
void PopulateDeviceAssignment(int* const device_assignment_buffer,
...
PJRT_Error* PJRT_Client_BufferFromHostBuffer(
  ...
  std::optional<zkx::Layout> layout = std::nullopt;
  ...
  std::unique_ptr<zkx::PjRtBuffer> buffer;
  ...
  if (has_layout_and_memory) {
    ...
    PJRT_ASSIGN_OR_RETURN(zkx::PjRtMemorySpace * memory_space,
...
PJRT_Error* PJRT_Client_CreateViewOfDeviceBuffer(
  ...
  PJRT_ASSIGN_OR_RETURN(zkx::Shape shape,
                        pjrt::BuildZkxShapeFromC(args->element_type, args->dims,
  ...
  zkx::PjRtMemorySpace* memory_space = nullptr;
...
PJRT_Error* PJRT_DeviceDescription_MemoryDescriptions(
  ...
  absl::Span<const zkx::PjRtMemorySpaceDescription* const> memory_spaces =
      args->device_description->device_description->memory_spaces();

  // We pass each zkx::PjRtMemorySpaceDescriptions to the caller through an
  // opaque pointer.
  args->memory_descriptions =
      reinterpret_cast<const PJRT_MemoryDescription* const*>(
          memory_spaces.data());

  absl::StatusOr<const zkx::PjRtMemorySpaceDescription*> default_memory =
...
PJRT_Error* PJRT_Executable_Name(PJRT_Executable_Name_Args* args) {
  PJRT_RETURN_IF_ERROR(ActualStructSizeIsGreaterOrEqual(
      ...
      args->struct_size));
  std::string_view executable_name = args->executable->get()->name();
...
PJRT_Error* PJRT_Executable_NumOutputs(PJRT_Executable_NumOutputs_Args* args) {
  ...
  PJRT_ASSIGN_OR_RETURN(std::vector<zkx::Shape> output_shapes,
                        args->executable->get()->GetOutputShapes());
  if (output_shapes.empty()) {
    return new PJRT_Error{absl::InvalidArgumentError(
        absl::StrCat("Can't get number of executable outputs, output "
                     "shapes is empty for executable ",
  ...
  const zkx::Shape& shape = output_shapes[0];
  if (shape.IsTuple()) {
...
absl::Status VerifyOptimizedProgramArgs(
...
absl::StatusOr<std::shared_ptr<zkx::HloModule>> GetOptimizedProgramModule(
    const PJRT_Executable_OptimizedProgram_Args* args) {
  TF_ASSIGN_OR_RETURN(std::vector<std::shared_ptr<zkx::HloModule>> hlo_modules,
                      args->executable->get()->GetHloModules());
  if (hlo_modules.empty()) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Can't get the optimized program for executable `",
        args->executable->get()->name(), "`: HLO modules is empty."));
  }
  if (hlo_modules.size() > 1) {
    return absl::UnimplementedError(
        absl::StrCat("Can't get the optimized program for executable `",
                     args->executable->get()->name(),
                     "`: MPMD execution is not supported by PJRT C API"));
  }
...
PJRT_Error* PJRT_Executable_OptimizedProgram(
  ...
  PJRT_ASSIGN_OR_RETURN(std::shared_ptr<zkx::HloModule> hlo_module,
                        GetOptimizedProgramModule(args));
  zkx::HloModuleProtoWithConfig proto = hlo_module->ToProtoWithConfig();
  if (program->code == nullptr) {
    program->code_size = proto.ByteSizeLong();
    if (program->code_size >= 2ull * 1024 * 1024 * 1024) {
      return new PJRT_Error{absl::ResourceExhaustedError(absl::StrCat(
          __func__,
    ...
    if (program->code_size < proto.ByteSizeLong()) {
      return new PJRT_Error{
          absl::InvalidArgumentError(
              absl::StrCat("`program->code_size` ", program->code_size,
                           " < required bytes ", proto.ByteSizeLong())),
      };
    }
    bool succeeded = proto.SerializeToArray(program->code, program->code_size);
    if (!succeeded) {
      return new PJRT_Error{absl::ResourceExhaustedError(
          absl::StrCat(__func__,
                       ": HLO program serialization exceeds max "
                       "supported protobuff size of 2 GiB."))};
    }
    return nullptr;
  }
...
zkx::SendCallback CSendCallbackToCpp(const PJRT_SendCallbackInfo& c_callback) {
  return zkx::SendCallback{
      ...
      [user_arg = c_callback.user_arg, callback = c_callback.send_callback](
          const zkx::PjRtTransferMetadata& unused_metadata,
          zkx::PjRtChunk input, size_t total_size_in_bytes,
...
// arguments.
void CSendCallbackListsToCpp(
    PJRT_SendCallbackInfo** c_lists, size_t outer_size, size_t inner_size,
    std::vector<std::vector<zkx::SendCallback>>& cpp_lists) {
  cpp_lists.reserve(outer_size);
  for (int i = 0; i < outer_size; ++i) {
    std::vector<zkx::SendCallback>& cpp_list = cpp_lists.emplace_back();
...
zkx::RecvCallback CRecvCallbackToCpp(const PJRT_RecvCallbackInfo& c_callback) {
  return zkx::RecvCallback{
      c_callback.channel_id,
      // Transfer metadata is unused because PJRT C API doesn't support
      // use_major_to_minor_data_layout_for_callbacks = false
      [user_arg = c_callback.user_arg, callback = c_callback.recv_callback](
          const zkx::PjRtTransferMetadata& unused_metadata,
          std::unique_ptr<zkx::CopyToDeviceStream> stream) {
...
void CRecvCallbackListsToCpp(
    PJRT_RecvCallbackInfo** c_lists, size_t outer_size, size_t inner_size,
    std::vector<std::vector<zkx::RecvCallback>>& cpp_lists) {
  cpp_lists.reserve(outer_size);
  for (int i = 0; i < outer_size; ++i) {
...
std::vector<std::vector<zkx::PjRtBuffer*>> Convert2DCBuffersToCppBuffers(
    PJRT_Buffer* const* const* c_lists, size_t outer_size, size_t inner_size) {
  std::vector<std::vector<zkx::PjRtBuffer*>> cpp_lists;
  cpp_lists.reserve(outer_size);
  for (int i = 0; i < outer_size; ++i) {
    auto& cpp_list = cpp_lists.emplace_back();
    cpp_list.reserve(inner_size);
...
PJRT_Error* PJRT_LoadedExecutable_Execute(
  ...
  std::vector<std::vector<zkx::PjRtBuffer*>> cpp_argument_lists =
  ...
  auto cpp_send_callbacks =
      std::make_shared<std::vector<std::vector<zkx::SendCallback>>>();
  ...
  auto cpp_recv_callbacks =
      std::make_shared<std::vector<std::vector<zkx::RecvCallback>>>();
  ...
  if (args->execute_device == nullptr) {
    std::vector<std::vector<std::unique_ptr<zkx::PjRtBuffer>>> cpp_buffer_lists;
    if (args->device_complete_events != nullptr ||
        !cpp_send_callbacks->empty() || !cpp_recv_callbacks->empty()) {
      std::optional<std::vector<zkx::PjRtFuture<>>> returned_futures;
    ...
    std::vector<std::unique_ptr<zkx::PjRtBuffer>> cpp_buffer_list;
    std::optional<zkx::PjRtFuture<>> returned_future;
    bool fill_future = args->device_complete_events != nullptr;
    PJRT_ASSIGN_OR_RETURN(zkx::CompileOptions compile_options,
...
PJRT_Error* PJRT_Executable_DeserializeAndLoad(
  ...
  std::string_view serialized(args->serialized_executable,
...
PJRT_Error* PJRT_Buffer_GetMemoryLayout(
      ...
      args->buffer->layout_data;
  {
    ...
    if (!layout_data.has_value()) {
      ...
      std::shared_ptr<const zkx::PjRtLayout> pjrt_layout =
          args->buffer->buffer->layout();
      const zkx::Layout& zkx_layout = pjrt_layout->zkx_layout();

      PJRT_ASSIGN_OR_RETURN(BufferMemoryLayoutData data,
                            ConvertToBufferMemoryLayoutData(zkx_layout));
...
PJRT_Error* PJRT_Buffer_Memory(PJRT_Buffer_Memory_Args* args) {
  ...
  if (args->memory == nullptr) {
    return new PJRT_Error{absl::UnimplementedError(
        absl::StrCat("PJRT_Buffer_Memory not implemented for platform '",
...
PJRT_Error* PJRT_Buffer_CopyRawToHost(PJRT_Buffer_CopyRawToHost_Args* args) {
  ...
  zkx::PjRtFuture<> wrapped_promise = args->buffer->buffer->CopyRawToHost(
...
PJRT_Error* PJRT_Buffer_CopyToDevice(PJRT_Buffer_CopyToDevice_Args* args) {
  ...
  PJRT_ASSIGN_OR_RETURN(std::unique_ptr<zkx::PjRtBuffer> dst_buffer,
...
PJRT_Error* PJRT_Buffer_CopyToMemory(PJRT_Buffer_CopyToMemory_Args* args) {
  ...
  PJRT_ASSIGN_OR_RETURN(
      std::unique_ptr<zkx::PjRtBuffer> dst_buffer,
...
PJRT_Error* PJRT_Buffer_ToHostBuffer(PJRT_Buffer_ToHostBuffer_Args* args) {
  ...
  zkx::Shape host_shape = zkx::ShapeUtil::DeviceShapeToHostShape(device_shape);
  ...
  size_t host_buffer_size = zkx::ShapeUtil::ByteSizeOfElements(host_shape);
  ...
  if (args->dst_size < host_buffer_size) {
    return new PJRT_Error{absl::InvalidArgumentError(
        absl::StrCat("`dst_size` must be >= ", host_buffer_size, ", got ",
                     args->dst_size, "."))};
  }

  auto literal = std::make_unique<zkx::MutableBorrowingLiteral>(
      static_cast<char*>(args->dst), host_shape);
  zkx::PjRtFuture<> future = args->src->buffer->ToLiteral(literal.get());
...
PJRT_Error* PJRT_Buffer_ReadyEvent(PJRT_Buffer_ReadyEvent_Args* args) {
  PJRT_RETURN_IF_ERROR(ActualStructSizeIsGreaterOrEqual(
      ...
      args->struct_size));
  zkx::PjRtFuture<> wrapped_promise = args->buffer->buffer->GetReadyFuture();
  args->event = new PJRT_Event{std::move(wrapped_promise)};
  return nullptr;
}
...
PJRT_Error* PJRT_Buffer_IncreaseExternalReferenceCount(
  ...
  PJRT_RETURN_IF_ERROR(ActualStructSizeIsGreaterOrEqual(
      ...
      args->struct_size));
  PJRT_ASSIGN_OR_RETURN(
      std::unique_ptr<zkx::PjRtBuffer::ExternalReference> external_reference,
...
PJRT_Error* PJRT_TopologyDescription_PlatformName(
  ...
  PJRT_RETURN_IF_ERROR(ActualStructSizeIsGreaterOrEqual(
      ...
      args->struct_size));
  std::string_view platform_name = args->topology->topology->platform_name();
  args->platform_name = platform_name.data();
  args->platform_name_size = platform_name.size();
  return nullptr;
}
...
PJRT_Error* PJRT_Compile(PJRT_Compile_Args* args) {
  ...
  PJRT_ASSIGN_OR_RETURN(auto module_or_hlo,
                        ParsePjrtProgram(context, args->program));
  PJRT_ASSIGN_OR_RETURN(std::unique_ptr<zkx::PjRtExecutable> executable,
                        std::visit(
                            [&](auto& program) {
                              return PjRtCompile(
...
PJRT_Error* PJRT_Layouts_PJRT_Client_GetDefaultLayout(
  ...
  PJRT_ASSIGN_OR_RETURN(zkx::Layout zkx_layout,
                        args->client->client->GetDefaultLayout(
                            pjrt::ConvertFromPjRtBufferType(args->type),
                            {args->dims, args->num_dims}));
  auto pjrt_zkx_layout = std::make_shared<zkx::PjRtLayout>(zkx_layout);
  args->layout = new PJRT_Layouts_MemoryLayout{std::move(pjrt_zkx_layout)};
  return nullptr;
}
...
std::vector<PJRT_NamedValue> PopulatePjrtAttributes(
    const absl::flat_hash_map<std::string, zkx::PjRtDeviceAttribute>&
        attributes) {
...
void PopulatePjrtClientDevices(PJRT_Client* c_client) {
  absl::Span<zkx::PjRtDevice* const> cpp_devices = c_client->client->devices();
  ...
  for (zkx::PjRtDevice* device : cpp_devices) {
...
void PopulatePjrtClientMemories(PJRT_Client* c_client) {
  absl::Span<zkx::PjRtMemorySpace* const> memory_spaces =
  ...
  for (zkx::PjRtMemorySpace* memory_space : memory_spaces) {
...
void AttachDevicesAndMemories(PJRT_Client* c_client) {
  for (PJRT_Device* c_device : c_client->devices) {
    ...
    absl::Span<zkx::PjRtMemorySpace* const> cpp_memories =
        c_device->device->memory_spaces();
    c_device->addressable_memories.reserve(cpp_memories.size());
    for (zkx::PjRtMemorySpace* memory_space : cpp_memories) {
  ...
  for (PJRT_Memory* c_memory : c_client->addressable_memories) {
    ...
    for (zkx::PjRtDevice* cpp_device : cpp_devices) {
      c_memory->devices.push_back(GetCDevice(c_client, cpp_device));
    }
  }
}

absl::StatusOr<std::unique_ptr<PJRT_TopologyDescription>>
GetStatusOrTopologyDescription(const zkx::PjRtClient& cpp_client) {
  absl::StatusOr<const zkx::PjRtTopologyDescription*> status_or_cpp_topo =
...
PJRT_Client* CreateWrapperClient(std::unique_ptr<zkx::PjRtClient> cpp_client) {
...
PJRT_ExecuteContext* CreateWrapperExecuteContext(
    std::unique_ptr<zkx::ExecuteContext> cpp_execute_context) {
...
PJRT_TopologyDescription* CreateWrapperDeviceTopology(
    const zkx::PjRtTopologyDescription* cpp_topology) {
...
PJRT_TopologyDescription* CreateWrapperDeviceTopology(
    std::unique_ptr<zkx::PjRtTopologyDescription> cpp_topology) {
