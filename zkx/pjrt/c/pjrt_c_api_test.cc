...
// TODO(chokobole): Uncomment this. Dependency: mhlo.copy
// constexpr std::string_view module_add_one =
//     R"(module {
// func.func @main(%arg0: tensor<s32>) -> tensor<s32> {
//   %0 = "mhlo.copy"(%arg0) : (tensor<s32>) -> tensor<s32>
//   %1 = mhlo.constant dense<1.000000e+00> : tensor<s32>
//   %2 = mhlo.add %0, %1 : tensor<s32>
//   return %2 : tensor<s32>
// }})";
constexpr std::string_view module_add_one =
    R"(module {
...
constexpr std::string_view kHloString =
    R"(
HloModule TupleCreate_module:
ENTRY %TupleCreate.v4 (v1: s32[], v2: s32[3], v3: s32[2,3]) -> (s32[], s32[3], s32[2,3]) {
  %v1 = s32[] parameter(0)
  %v2 = s32[3]{0} parameter(1)
  %v3 = s32[2,3]{1,0} parameter(2)
  ROOT %tuple = (s32[], s32[3]{0}, s32[2,3]{1,0}) tuple(s32[] %v1, s32[3]{0} %v2, s32[2,3]{1,0} %v3)
}
)";
...
TEST_F(PjrtCApiTest, BufferTransferImmutableUntilTransferCompletes) {
  zkx::Shape shape = zkx::ShapeUtil::MakeShapeWithType<int32_t>({4});
  std::vector<int32_t> int32_data(4);
  std::iota(int32_data.begin(), int32_data.end(), 41);

  PJRT_Client_BufferFromHostBuffer_Args args = CreateBufferFromHostBufferArgs(
      int32_data, shape,
      zkx::PjRtClient::HostBufferSemantics::kImmutableUntilTransferCompletes);
...
TEST_F(PjrtCApiTest, CompileZkxComputation) {
  PJRT_Client_Compile_Args args = PJRT_Client_Compile_Args{
      .struct_size = PJRT_Client_Compile_Args_STRUCT_SIZE,
      .extension_start = nullptr,
      .client = client_,
  };
  zkx::DeviceAssignment device_assignment(1, 1);
  device_assignment(0, 0) = 0;
  zkx::DeviceAssignmentProto proto;
  ...
  absl::StatusOr<std::unique_ptr<zkx::HloModule>> hlo_module =
      zkx::ParseAndReturnUnverifiedModule(kHloString);
...
TEST_F(PjrtCApiTest, PluginAttributes) {
  ...
  EXPECT_TRUE(names.find("zkx_version") != names.end());
...
TEST_F(PjrtCApiTest, DeviceDescriptionAndMemoryDescriptionss) {
  ...
  std::vector<zkx::PjRtMemorySpaceDescription> memory_descriptions =
...
TEST_F(PjrtCApiBufferTest, ToHostBufferNoHostLayout) {
  ...
  args.dst_size = zkx::ShapeUtil::ByteSizeOfElements(host_shape);
  args.event = nullptr;
...
FieldOffsetsAndSizesForVersion(int major_version, int minor_version) {
  ...
  auto add_field = [&version_offsets_and_sizes](std::string_view field_name,
                                                size_t field_size) {
    size_t field_start;
