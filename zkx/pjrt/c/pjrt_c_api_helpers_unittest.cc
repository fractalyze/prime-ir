...
// ZKX: Changed - removed float from the roundtrip test since float types are
// removed in ZKX.
TEST(PjRtCApiHelperTest, ConvertValidPjRtValueType) {
  std::vector<int64_t> int64_list = {static_cast<int64_t>(1),
                                     static_cast<int64_t>(2)};
  absl::flat_hash_map<std::string, zkx::PjRtValueType> original_cpp_map = {
      {"string", "v1"},
...
TEST(PjRtCApiHelperTest, ValidOptionNameAndPjRtValueTypeIndex) {
  ...
  absl::flat_hash_map<std::string, zkx::PjRtValueType> valid_map = {
...
TEST(PjRtCApiHelperTest, InvalidOptionName) {
  ...
  absl::flat_hash_map<std::string, zkx::PjRtValueType> invalid_map = {
      {"invalid", "v1"}};
...
TEST(PjRtCApiHelperTest, ConvertToCLayoutFromLayoutNoTiles) {
  ...
  zkx::Layout layout(minor_to_major);
...
TEST(PjRtCApiHelperTest, ConvertToCLayoutFromLayoutWithTiles) {
  ...
  layout.mutable_tiles()->push_back(zkx::Tile(tile_dims_1));
  layout.mutable_tiles()->push_back(zkx::Tile(tile_dims_2));
...
TEST(PjRtCApiHelperTest, ConvertFromCLayoutToLayout) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(zkx::Layout layout, ConvertToLayout(c_layout.tiled));
...
TEST(PjRtCApiHelperTest, GetZkxPluginCAttributes) {
  auto result = GetZkxPluginCAttributes();
  ...
  EXPECT_TRUE(map.find("zkx_version") != map.end());
