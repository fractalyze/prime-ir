...
#include "zkx/pjrt/c/pjrt_c_api_gpu.h"
...
#include "zkx/client/client_library.h"
...
#include "zkx/pjrt/c/pjrt_c_api_test.h"
#include "zkx/pjrt/c/pjrt_c_api_test_base.h"
#include "zkx/pjrt/c/pjrt_c_api_triton_extension.h"
...
TEST_F(PjrtCApiGpuTest, CreateViewOfDeviceBuffer) {
  ...
  zkx::Shape shape = zkx::ShapeUtil::MakeShape(zkx::S32, {4});
  ...
  zkx::Shape host_shape = zkx::ShapeUtil::MakeShape(zkx::S32, {4});
  auto literal = std::make_shared<zkx::Literal>(host_shape);
  to_host_args.host_layout = nullptr;
  to_host_args.dst = literal->untyped_data();
  to_host_args.dst_size = zkx::ShapeUtil::ByteSizeOfElements(host_shape);
  ...
  zkx::PjRtFuture<> transfer_to_host =
      ::pjrt::ConvertCEventToCppFuture(to_host_args.event, api_);
  CHECK_OK(transfer_to_host.Await());
  ASSERT_EQ(literal->data<int32_t>().size(), 4);
  std::vector<int32_t> int32_data(4);
  std::iota(int32_data.begin(), int32_data.end(), 41.0f);
  EXPECT_TRUE(zkx::LiteralTestUtil::Equal(
      zkx::LiteralUtil::CreateR1<int32_t>(int32_data), *literal));
}
class PjrtCApiGpuTransferManagerTest : public PjrtCApiGpuTest {
 public:
  ...
  void CreateTransferManager(const zkx::Shape& host_shape) {
...
TEST_F(PjrtCApiGpuTest, CreateAndDestroyExecuteContext) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      ...
      create_arg.context->execute_context->ffi_context().Lookup(
          zkx::ffi::TypeIdRegistry::TypeId(42)));
...
TEST_F(PjrtCApiGpuTransferManagerTest, SetBufferError) {
  zkx::Shape host_shape =
      zkx::ShapeUtil::MakeShape(zkx::S32, /*dimensions=*/{8});
  std::vector<int32_t> int32_data = {1, 2, 3, 4, 5, 6, 7, 8};
...
TEST_F(PjrtCApiGpuTransferManagerTest, TransferRawDataToBufferIsSuccessful) {
  zkx::Shape host_shape =
      zkx::ShapeUtil::MakeShape(zkx::U32, /*dimensions=*/{8});
  ...
  TF_ASSERT_OK_AND_ASSIGN(zkx::Shape result_shape,
  ...
  TF_ASSERT_OK_AND_ASSIGN(std::shared_ptr<zkx::Literal> literal,
...
TEST(PjrtCApiGpuKVStoreTest, CreateClientWithKVCallback) {
  ...
  for (int i = 0; i < num_nodes; i++) {
    threads.emplace_back([api, i, num_nodes,
                          ...
                          kv_store = kv_store] {
      absl::flat_hash_map<std::string, zkx::PjRtValueType> options = {
...
TEST(PjrtCApiGpuAllocatorTest, ValidOptionsParsing) {
  ...
  for (const std::string& allocator_option : allocator_options) {
    ...
    absl::flat_hash_map<std::string, zkx::PjRtValueType> options = {
        ...
        {"visible_devices", zkx::PjRtValueType(std::vector<int64_t>{0, 1})},
    };
...
TEST(PJRTGpuDeviceTopologyTest, CreateGpuTopology) {
  ...
  EXPECT_TRUE((pjrt_topology->topology->platform_id() == zkx::CudaId() &&
               pjrt_topology->topology->platform_name() == zkx::CudaName()) ||
              (pjrt_topology->topology->platform_id() == zkx::RocmId() &&
               pjrt_topology->topology->platform_name() == zkx::RocmName()));
...
TEST(PjrtCApiGpuExtensionTest, CustomCallUntyped) {
  ...
  void* custom_call = zkx::CustomCallTargetRegistry::Global()->Lookup(
...
TEST(PjrtCApiGpuExtensionTest, CustomCallTyped) {
  static constexpr auto* noop = +[] { return zkx::ffi::Error::Success(); };
  ZKX_FFI_DEFINE_HANDLER(kNoop, noop, zkx::ffi::Ffi::Bind());
  ...
  CHECK_EQ(error, nullptr);
  auto registration =
      zkx::ffi::FindHandler(function_name, stream_executor::GpuPlatformName())
          .value();
  EXPECT_EQ(reinterpret_cast<void*>(registration.bundle.execute), kNoop);
}

constexpr std::string_view kAddOneTTIR = R"(
module {
  tt.func public @add_one(%arg0: !tt.ptr<i32, 1> {tt.divisibility = 32 : i32}, %arg1: !tt.ptr<i32, 1> {tt.divisibility = 32 : i32}, %arg2: !tt.ptr<i32, 1> {tt.divisibility = 32 : i32}, %arg3: !tt.ptr<i32, 1> {tt.divisibility = 32 : i32}) {
    %0 = tt.get_program_id x : i32
    %1 = tt.load %arg0 {cache = 1 : i32, evict = 1 : i32, isVolatile = false} : !tt.ptr<i32>
    %2 = tt.load %arg1 {cache = 1 : i32, evict = 1 : i32, isVolatile = false} : !tt.ptr<i32>
    %cst = arith.constant 1.000000e+00 : i32
    %3 = arith.addf %1, %cst : i32
    %4 = tt.load %arg2 {cache = 1 : i32, evict = 1 : i32, isVolatile = false} : !tt.ptr<i32>
    tt.store %arg2, %3 {cache = 1 : i32, evict = 1 : i32} : !tt.ptr<i32>
    %5 = tt.load %arg3 {cache = 1 : i32, evict = 1 : i32, isVolatile = false} : !tt.ptr<i32>
    tt.store %arg3, %2 {cache = 1 : i32, evict = 1 : i32} : !tt.ptr<i32>
    tt.return
  }
}
)";

// TODO(chokobole): Uncomment this. Dependency: triton
TEST(PjrtCAPIGpuExtensionTest, DISABLED_TritonCompile) {
  constexpr std::string_view kArchName = "7.0";
