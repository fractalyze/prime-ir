...
#ifndef ZKX_PJRT_C_PJRT_C_API_HELPERS_H_
#define ZKX_PJRT_C_PJRT_C_API_HELPERS_H_
...
ABSL_CONST_INIT extern const std::string_view kHloFormat;
ABSL_CONST_INIT extern const std::string_view kMlirFormat;
ABSL_CONST_INIT extern const std::string_view kHloWithConfigFormat;
...
// Conversion helper from zkx::PrimitiveType to PJRT_Buffer_Type.
PJRT_Buffer_Type ConvertToPjRtBufferType(zkx::PrimitiveType type);

// Conversion helper from PJRT_Buffer_type to zkx::PrimitiveType.
zkx::PrimitiveType ConvertFromPjRtBufferType(PJRT_Buffer_Type type);

// Conversion helper from zkx::PjRtClient::HostBufferSemantics to
...
// Conversion helper to zkx::PjRtClient::HostBufferSemantics from
// PJRT_HostBufferSemantics.
...
// Returns attributes for plugin that uses ZKX compiler. The attributes have the
...
// Wraps original `zkx::SendCallback` inside `PJRT_Callback` using
...
// Wraps original `zkx::RecvCallback` inside `PJRT_Callback` using
// 1) void* `user_arg` to capture `cpp_send_callback.callback` (std::function)
...
absl::StatusOr<zkx::Shape> BuildZkxShapeFromC(PJRT_Buffer_Type element_type,
                                              const int64_t* dims,
                                              size_t num_dims,
...
#endif  // ZKX_PJRT_C_PJRT_C_API_HELPERS_H_
