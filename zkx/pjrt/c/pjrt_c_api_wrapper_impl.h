...
#ifndef ZKX_PJRT_C_PJRT_C_API_WRAPPER_IMPL_H_
#define ZKX_PJRT_C_PJRT_C_API_WRAPPER_IMPL_H_
...
struct PJRT_TopologyDescription {
  ...
  std::unique_ptr<zkx::PjRtTopologyDescription> owned_topology;
  const zkx::PjRtTopologyDescription* topology;
  std::vector<std::unique_ptr<const zkx::PjRtDeviceDescription>>
      cpp_descriptions;
...
struct PJRT_Client {
  std::unique_ptr<zkx::PjRtClient> client;
  ...
  // `owned_devices`.
  absl::flat_hash_map<zkx::PjRtDevice*, PJRT_Device*> c_device_from_cpp_device;
  ...
  // `owned_memories`.
  absl::flat_hash_map<zkx::PjRtMemorySpace*, PJRT_Memory*>
      c_memory_from_cpp_memory;
  absl::StatusOr<std::unique_ptr<PJRT_TopologyDescription>> topology;

  explicit PJRT_Client(std::unique_ptr<zkx::PjRtClient> cpp_client);
};

struct PJRT_MemoryDescription {
  zkx::PjRtMemorySpaceDescription memory_space_description;
};
...
struct PJRT_AsyncHostToDeviceTransferManager {
  std::unique_ptr<zkx::PjRtClient::AsyncHostToDeviceTransferManager>
      transfer_manager;
  PJRT_Client* client;  // not owned
};

// PJRT_DeviceDescriptions are owned by their corresponding PJRT_Device.
struct PJRT_DeviceDescription {
  // The zkx::PjRtDeviceDescription* is owned transitively by the
  // corresponding zkx::PjRtClient.
  const zkx::PjRtDeviceDescription* device_description;  // not owned
...
struct PJRT_Device {
  // The zkx::PjRtDevice* is owned by the corresponding zkx::PjRtClient.
  zkx::PjRtDevice* device;  // not owned
...
struct PJRT_Memory {
  // The zkx::PjRtMemorySpace* is owned by the corresponding zkx::PjRtClient.
  zkx::PjRtMemorySpace* memory_space;  // not owned
  std::vector<PJRT_Device*> devices;
  PJRT_Client* client;  // not owned
};

struct PJRT_ExecuteContext {
  std::shared_ptr<zkx::ExecuteContext> execute_context;
};
...
struct PJRT_Executable {
  ...
  explicit PJRT_Executable(std::shared_ptr<zkx::PjRtExecutable> executable);

  const zkx::PjRtExecutable* get() const { return executable.get(); }
  zkx::PjRtExecutable* get() { return executable.get(); }
};

struct PJRT_LoadedExecutable {
  // Must be shared_ptr so that we can share with PJRT_Executable.
  std::shared_ptr<zkx::PjRtLoadedExecutable> executable;
  PJRT_Client* client;  // not owned
  ...
  PJRT_LoadedExecutable(std::shared_ptr<zkx::PjRtLoadedExecutable> executable,
                        PJRT_Client* client);

  const zkx::PjRtLoadedExecutable* get() const { return executable.get(); }
  zkx::PjRtLoadedExecutable* get() { return executable.get(); }
};
...
struct PJRT_Buffer {
  ...
  std::vector<std::unique_ptr<zkx::PjRtBuffer::ExternalReference>>
      external_references;
};
...
struct PJRT_TransferMetadata {
  // Decompose zkx::Shape into C API type fields, without any Tuple information.
  // TODO(b/238999986) support other `zkx::Shape` fields when they are fully
  // implemented.
...
// `zkx::PjRtMemorySpace *` (`cpp_memory`). If a match is found, that
...
#endif  // ZKX_PJRT_C_PJRT_C_API_WRAPPER_IMPL_H_
