...
#include "zkx/pjrt/c/pjrt_c_api_gpu_internal.h"
...
#include "zkx/backends/profiler/plugin/plugin_tracer_impl.h"
#include "zkx/backends/profiler/plugin/profiler_error.h"
#include "zkx/client/local_client.h"
#include "zkx/ffi/api/c_api.h"
#include "zkx/ffi/ffi.h"
#include "zkx/ffi/ffi_api.h"
#include "zkx/pjrt/c/pjrt_c_api_custom_partitioner_extension.h"
#include "zkx/pjrt/c/pjrt_c_api_ffi_extension.h"
#include "zkx/pjrt/c/pjrt_c_api_ffi_internal.h"
#include "zkx/pjrt/c/pjrt_c_api_gpu_extension.h"
...
#include "zkx/pjrt/c/pjrt_c_api_triton_internal.h"
#include "zkx/pjrt/c/pjrt_c_api_wrapper_impl.h"
#include "zkx/pjrt/gpu/gpu_helpers.h"
#include "zkx/pjrt/gpu/gpu_topology.h"
...
PJRT_Error* PJRT_Client_Create(PJRT_Client_Create_Args* args) {
  ...
  absl::flat_hash_map<std::string, zkx::PjRtValueType> create_options =
  ...
  zkx::GpuAllocatorConfig allocator_config;
  if (auto it = create_options.find("allocator"); it != create_options.end()) {
    ...
    if (allocator_name == "default") {
      allocator_config.kind = zkx::GpuAllocatorConfig::Kind::kDefault;
    } else if (allocator_name == "platform") {
      allocator_config.kind = zkx::GpuAllocatorConfig::Kind::kPlatform;
    } else if (allocator_name == "bfc") {
      allocator_config.kind = zkx::GpuAllocatorConfig::Kind::kBFC;
    } else if (allocator_name == "cuda_async") {
      allocator_config.kind = zkx::GpuAllocatorConfig::Kind::kCudaAsync;
    } else {
  ...
  PJRT_ASSIGN_OR_RETURN(std::unique_ptr<zkx::PjRtClient> client,
                        zkx::GetStreamExecutorGpuClient(options));
...
absl::StatusOr<TargetConfigAndDevices> GetTargetConfigFromOptions(
    const absl::flat_hash_map<std::string, zkx::PjRtValueType>& options) {
  if (auto target_config_it = options.find("target_config");
    ...
    if (!google::protobuf::TextFormat::ParseFromString(
            target_config_proto_string, &target_config_proto)) {
  ...
  TF_ASSIGN_OR_RETURN(zkx::LocalClient * zkx_client,
                      zkx::GetGpuZkxClient(/*platform_name=*/std::nullopt,
                                           /*allowed_devices=*/std::nullopt));
  stream_executor::StreamExecutor* executor =
      zkx_client->backend().default_stream_executor();
  std::vector<int> device_ids;
  device_ids.reserve(zkx_client->backend().stream_executors().size());
  for (stream_executor::StreamExecutor* executor :
       zkx_client->backend().stream_executors()) {
    device_ids.push_back(executor->device_ordinal());
  }
  auto gpu_target_config = zkx::Compiler::TargetConfig(executor);
...
PJRT_Error* PJRT_GpuDeviceTopology_Create(
  ...
  zkx::PjRtPlatformId platform_id =
      (std::string(PJRT_GPU_PLUGIN_PLATFORM_NAME) == "ROCM") ? zkx::RocmId()
                                                             : zkx::CudaId();
  std::string platform_name =
      (std::string(PJRT_GPU_PLUGIN_PLATFORM_NAME) == "ROCM") ? zkx::RocmName()
                                                             : zkx::CudaName();
  ...
  zkx::TopologySizes sizes{1, 1, static_cast<int>(device_ids.size())};
  ...
  if (auto topology_it = create_options.find("topology");
    ...
    PJRT_ASSIGN_OR_RETURN(sizes,
                          zkx::TopologySizes::FromString(topology_string));
  }
  ...
  auto gpu_topology = std::make_shared<const zkx::GpuTopology>(
  ...
  auto pjrt_topology =
      std::make_unique<zkx::StreamExecutorGpuTopologyDescription>(
          platform_id, platform_name, std::move(gpu_topology),
          absl::flat_hash_map<std::string, zkx::PjRtDeviceAttribute>{
...
PLUGIN_Profiler_Api profiler_api{
    ...
    /*priv=*/nullptr,
    /*error_destroy=*/zkx::profiler::PLUGIN_Profiler_Error_Destroy,
    /*error_message=*/zkx::profiler::PLUGIN_Profiler_Error_Message,
    /*error_get_code=*/zkx::profiler::PLUGIN_Profiler_Error_GetCode,
    /*create=*/zkx::profiler::PLUGIN_Profiler_Create,
    /*destroy=*/zkx::profiler::PLUGIN_Profiler_Destroy,
    /*start=*/zkx::profiler::PLUGIN_Profiler_Start,
    /*stop=*/zkx::profiler::PLUGIN_Profiler_Stop,
    /*collect_data=*/zkx::profiler::PLUGIN_Profiler_CollectData,
};
...
PJRT_Error* PJRT_Register_Custom_Partitioner(
  ...
  PJRT_Error* err = new PJRT_Error;
  err->status = absl::UnimplementedError(
      "CreateCApiCustomCallPartitioner not implemented");
  return err;
  // clang-format off
  // TODO(chokobole): Uncomment this. Dependency: CreateCApiCustomCallPartitioner
  // clang-format on
...
PJRT_Error* PJRT_Register_Batch_Partitionable(
  ...
  return err;
  // clang-format off
  // TODO(chokobole): Uncomment this. Dependency: CustomCallBatchPartitioner
  // clang-format on
  //   std::string name(args->name, args->name_size);
  //   RegisterCustomCallPartitioner(
  //       name, std::make_unique<zkx::CustomCallBatchPartitioner>());
  //   return nullptr;
}
...
PJRT_Error* PJRT_Gpu_Register_Custom_Call(
  ...
  switch (args->api_version) {
    case 0:
      zkx::CustomCallTargetRegistry::Global()->Register(
          function_name, args->handler_execute, PJRT_GPU_PLUGIN_PLATFORM_NAME);
      return nullptr;
    case 1:
      zkx::ffi::Ffi::RegisterStaticHandler(
          zkx::ffi::GetZkxFfiApi(), function_name,
          PJRT_GPU_PLUGIN_PLATFORM_NAME,
          ZKX_FFI_Handler_Bundle{
              reinterpret_cast<ZKX_FFI_Handler*>(args->handler_instantiate),
              reinterpret_cast<ZKX_FFI_Handler*>(args->handler_prepare),
              reinterpret_cast<ZKX_FFI_Handler*>(args->handler_initialize),
              reinterpret_cast<ZKX_FFI_Handler*>(args->handler_execute)});
      return nullptr;
    default:
