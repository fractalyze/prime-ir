...
#include "zkx/pjrt/c_api_client/pjrt_c_api_client.h"
...
#include "zkx/pjrt/c/pjrt_c_api_profiler_extension.h"
#include "zkx/pjrt/c/pjrt_c_api_stream_extension.h"
#include "zkx/pjrt/compile_options.pb.h"
#include "zkx/pjrt/mlir_to_hlo.h"
#include "zkx/pjrt/pjrt_api.h"
...
absl::StatusOr<const PjRtCApiTopologyDescription> InitClientTopoDesc(
...
absl::flat_hash_map<PJRT_Extension_Type, PJRT_Extension_Base*> InitExtensions(
...
// clang-format off
// TODO(jeong0982): Uncomment this. Dependency: host allocator extension
...
void PjRtCApiClient::InitAttributes() {
  ...
  // clang-format off
  // TODO(jeong0982): Uncomment this. Dependency: cross host transfer extension
...
std::string_view PjRtCApiClient::platform_version() const {
...
namespace {
DeviceAssignment CalculateDefaultAssignment(
...
// clang-format off
// TODO(jeong0982): Uncomment this. Dependency: PJRT_ProcessState, PJRT_ProcessInfo,
...
namespace {
absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>> InitializeArgsAndCompile(
...
// TODO: Port GetDefaultStablehloVersion and Serialize from
...
std::string GetPluginStablehloVersionOrDefault(PjRtClient* client) {
  ...
  if (!client) {
    return zkx::GetDefaultStablehloVersion();
  }
...
absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>> PjRtCApiClient::Compile(
  ...
  TF_ASSIGN_OR_RETURN(std::string serialized,
                      zkx::Serialize(module, version_string));
...
PjRtCApiClient::LoadSerializedExecutable(std::string_view serialized,
  ...
  // clang-format off
  // TODO(jeong0982): Uncomment this. Dependency: overridden_serialized_compile_options
...
// clang-format off
// TODO(jeong0982): Uncomment this. Dependency: PJRT_Client_CreateUninitializedBuffer
...
// clang-format off
// TODO(jeong0982): Uncomment this. Dependency: PJRT_Client_FulfillAliasBuffer, PJRT_Client_CreateAliasBuffer
// absl::Status FulfillAliasBuffer(
//     const PJRT_Api* pjrt_c_api,
//     absl::StatusOr<PjRtBuffer*> real_buffer_or,
...
// clang-format off
// TODO(jeong0982): Uncomment this. Dependency: host allocator extension
// absl::StatusOr<PjRtClient::HostAllocator*>
// PjRtCApiClient::GetHostAllocator() const {
...
class PjRtCApiAsyncHostToDeviceTransferManager
 ...
 public:
  ...
  absl::Status TransferLiteralToBuffer(
    ...
    // ZKX: Reimplemented using TransferData (TransferLiteral not in ZKX C API)
  ...
  void AddTransferMetadata(const TransferMetadata& metadata) override {
    ...
    args.extension_start = nullptr;
    args.transfer_manager = c_transfer_manager_.get();
    absl::flat_hash_map<std::string, zkx::PjRtValueType> pjrt_metadata;
...
std::string_view PjRtCApiDeviceDescription::device_kind() const {
  ...
  std::string_view device_kind(args.device_kind, args.device_kind_size);
  return device_kind;
}

std::string_view PjRtCApiDeviceDescription::DebugString() const {
  ...
  pjrt::LogFatalIfPjrtError(c_api_->PJRT_DeviceDescription_DebugString(&args),
                            c_api_);
  std::string_view debug_string(args.debug_string, args.debug_string_size);
  return debug_string;
}

std::string_view PjRtCApiDeviceDescription::ToString() const {
  ...
  pjrt::LogFatalIfPjrtError(c_api_->PJRT_DeviceDescription_ToString(&args),
                            c_api_);
  std::string_view to_string(args.to_string, args.to_string_size);
  return to_string;
}
...
std::string_view PjRtCApiMemorySpace::kind() const {
  ...
  return std::string_view(args.kind, args.kind_size);
}
...
std::string_view PjRtCApiMemorySpace::DebugString() const {
  ...
  pjrt::LogFatalIfPjrtError(pjrt_c_api()->PJRT_Memory_DebugString(&args),
                            pjrt_c_api());
  return std::string_view(args.debug_string, args.debug_string_size);
}

std::string_view PjRtCApiMemorySpace::ToString() const {
  ...
  pjrt::LogFatalIfPjrtError(pjrt_c_api()->PJRT_Memory_ToString(&args),
                            pjrt_c_api());
  return std::string_view(args.to_string, args.to_string_size);
}
...
std::string_view PjRtCApiExecutable::name() const {
  ...
  return std::string_view(args.executable_name, args.executable_name_size);
}
...
namespace {
absl::StatusOr<Shape> GetOutputShapeHelper(
  ...
  for (int i = 0; i < element_types.size(); ++i) {
    TF_ASSIGN_OR_RETURN(zkx::Shape shape, ShapeUtil::MakeValidatedShape(
                                              element_types[i], dimensions[i]));
    *shape.mutable_layout() = layouts[i]->zkx_layout();
...
// clang-format off
// TODO(jeong0982): Uncomment this. Dependency: executable metadata extension
...
//           c_api_,
//           PJRT_Extension_Type::PJRT_Extension_Type_ExecutableMetadata);
...
//       executable_metadata_extension->destroy_serialized_metadata(
//           &free_args);
//     }
//   };
//   if (args.metadata == nullptr) {
...
PjRtCApiExecutable::GetOutputMemoryKinds() const {
  ...
  for (int i = 0; i < args.num_outputs; ++i) {
    out.push_back(
        std::string_view(args.memory_kinds[i], args.memory_kind_sizes[i]));
  }
  return std::vector<std::vector<std::string_view>>{std::move(out)};
}
...
PjRtCApiExecutable::GetHloModules() const {
  ...
  std::string_view program_format(program.format, program.format_size);
...
void PjRtCApiLoadedExecutable::InitDeviceAssignment() {
  // clang-format off
  // TODO(jeong0982): Uncomment this. Dependency: PJRT_LoadedExecutable_GetDeviceAssignment
  // if (pjrt_c_api()->pjrt_api_version.major_version == 0 &&
...
namespace {
std::vector<std::vector<PJRT_Buffer*>> Convert2DCppBuffersToCBuffers(
...
Convert2DCBuffersToCppBuffers(PJRT_Buffer** const* c_lists, size_t outer_size,
                              int inner_size, zkx::PjRtCApiClient* client) {
...
// Wraps original `zkx::SendCallback` inside `PJRT_SendCallbackInfo` using
...
PJRT_SendCallbackInfo CppSendCallbackToC(
    const zkx::SendCallback& cpp_send_callback,
  ...
  *send_callback_function =
    ...
    zkx::Shape dummy_shape;
    absl::Status status = send_callback(zkx::PjRtTransferMetadata{dummy_shape},
...
// Wraps original `zkx::RecvCallback` inside `PJRT_RecvCallbackInfo` using
...
PJRT_RecvCallbackInfo CppRecvCallbackToC(
    const zkx::RecvCallback& cpp_recv_callback, const PJRT_Api* c_api,
  ...
  *recv_callback_function = [&recv_callback = cpp_recv_callback.callback,
    ...
    // use_major_to_minor_data_layout_for_callbacks = false
    zkx::Shape dummy_shape;
    recv_callback(zkx::PjRtTransferMetadata{dummy_shape},
...
namespace {
void CppSendCallbackListsToC(
    absl::Span<const std::vector<zkx::SendCallback>> cpp_lists,
  ...
  int func_count = 0;
  for (const std::vector<zkx::SendCallback>& cpp_list : cpp_lists) {
    std::vector<PJRT_SendCallbackInfo>& c_list = c_lists.emplace_back();
    c_list.reserve(cpp_list.size());
    for (const zkx::SendCallback& cpp_callback : cpp_list) {
...
void CppRecvCallbackListsToC(
    absl::Span<const std::vector<zkx::RecvCallback>> cpp_lists,
    const PJRT_Api* c_api,
...
PjRtCApiLoadedExecutable::GetCommonExecuteArgs(
  ...
  // clang-format off
  // TODO(jeong0982): Uncomment this. Dependency: call_location and incarnations
...
namespace {
absl::StatusOr<PJRT_ExecuteContext*> ForwardExecuteContext(
...
const Shape& PjRtCApiBuffer::on_device_shape() const {
  if (!on_device_shape_.has_value()) {
    Shape shape(element_type(), dimensions(), is_dynamic_dimension(),
                /*tuple_shapes=*/{});
    *shape.mutable_layout() = layout()->zkx_layout();
    absl::MutexLock lock(mu_);
...
absl::StatusOr<Shape> PjRtCApiBuffer::logical_on_device_shape() {
  ...
  Shape result(element_type(), *dims, is_dynamic_dimension(),
               /*tuple_shapes=*/{});
  *result.mutable_layout() = layout()->zkx_layout();
  return result;
}
...
PjRtFuture<> PjRtCApiBuffer::ToLiteral(MutableLiteralBase* literal) {
  ...
  const zkx::Shape& shape = literal->shape();

  if (!shape.IsArray()) {
    return PjRtFuture<>(absl::UnimplementedError(
        "PjRtCApiBuffer::ToLiteral: Shapes other than array are"
        "not supported."));
  }
...
PjRtFuture<> PjRtCApiBuffer::GetReadyFuture() {
  ...
  if (readiness_promise_ == nullptr) {
    // ZKX: Changed - adapted from MakePromise<>() to
    // PjRtFuture<>::CreatePromise()
    auto promise = PjRtFuture<>::CreatePromise();
    readiness_future_ = PjRtFuture<>(promise);
...
//   CHECK_OK(descriptor);
//   args.serialized_descriptor = descriptor->data();
...
void PjRtCApiBuffer::CopyToRemoteDevice(
  ...
  on_done(absl::UnimplementedError(
              "PJRT C API does not support CopyToRemoteDevice"),
          /*sends_were_enqueued=*/false);
}

void PjRtCApiBuffer::CopyToRemoteDeviceScattered(
    PjRtFuture<std::vector<std::string>> serialized_descriptors,
    std::vector<RemoteSendCallback> callbacks,
    const ScatterDetails& scatter_details) {
  for (auto& cb : callbacks) {
    cb(absl::UnimplementedError(
           "PJRT C API does not support CopyToRemoteDeviceScattered"),
       /*sends_were_enqueued=*/false);
  }
}
...
// ZKX: Helper to get platform version from topology description directly,
// since pjrt::GetPlatformVersion only takes PJRT_Client*.
namespace {
std::string GetTopologyPlatformVersion(const PJRT_Api* c_api,
                                       PJRT_TopologyDescription* topo) {
  ...
  args.topology = topo;
  pjrt::LogFatalIfPjrtError(
      c_api->PJRT_TopologyDescription_PlatformVersion(&args), c_api);
...
PjRtCApiTopologyDescription::PjRtCApiTopologyDescription(
    ...
    : compiler_(std::make_unique<PjRtCApiCompiler>(c_api)),
      ...
      platform_version_(absl::StrCat(
          "PJRT C API\n", GetTopologyPlatformVersion(c_api, c_topology))),
...
std::string_view PjRtCApiTopologyDescription::platform_version() const {
  return platform_version_;
}
...
absl::StatusOr<Layout> PjRtCApiTopologyDescription::GetDefaultLayout(
  ...
  // clang-format off
  // TODO(jeong0982): Uncomment this. Dependency: PJRT_Layouts_PJRT_Topology_GetDefaultLayout
  // auto extension = pjrt::FindExtension<PJRT_Layouts_Extension>(
  //     c_api_, PJRT_Extension_Type::PJRT_Extension_Type_Layouts);
  // if (extension == nullptr ||
...
// API PJRT_Compile().
namespace {
absl::StatusOr<std::unique_ptr<PjRtExecutable>> InitializeArgsAndCompileAot(
...
absl::StatusOr<std::unique_ptr<PjRtExecutable>> PjRtCApiCompiler::Compile(
  ...
  TF_ASSIGN_OR_RETURN(std::string serialized,
                      zkx::Serialize(module, target_version));
  std::string format(pjrt::kMlirFormat);
  return InitializeArgsAndCompileAot(c_api_, client, options, topology,
                                     serialized, format);
}
...
// clang-format off
// TODO(jeong0982): Uncomment this. Dependency: PJRT_TopologyDescription_Deserialize
...
absl::StatusOr<std::unique_ptr<PjRtCompiler>> GetCApiCompiler() {
  ...
  if (device_types.size() > 1) {
    return absl::FailedPreconditionError(
        ...
        "GetCApiCompiler(std::string_view device_type) "
        "instead.");
  }
...
// clang-format off
// TODO(jeong0982): Uncomment this. Dependency: PJRT_PhaseCompile_Extension
...
//   }
//   return std::make_unique<PjRtCApiPhaseCompiler>(c_api,
//                                                  phase_compile_extension);
// }
//
// absl::StatusOr<std::unique_ptr<PjRtPhaseCompiler>> GetCApiPhaseCompiler() {
//   TF_ASSIGN_OR_RETURN(std::vector<std::string> device_types,
