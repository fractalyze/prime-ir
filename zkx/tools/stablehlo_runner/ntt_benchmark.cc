...
// NTT benchmark using StableHLO → HLO → HloRunner pipeline.
//
// Benchmarks forward NTT on BabyBearMont field with size 2²⁰ (1,048,576
// elements). The StableHLO module is pre-compiled from whir-zorch's
// TwoAdicSubgroupDft implementation.
...
int main(int argc, char** argv) {
  ...
  if (mlir_path.empty()) {
    LOG(ERROR) << "Usage: ntt_benchmark <mlir_file> [--platform=cpu|gpu] "
  ...
  int ret = zkbench::BenchmarkMain(argc, argv, "zkx-ntt", "0.1.0");
