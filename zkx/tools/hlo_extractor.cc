...
absl::Status Inline(HloModule* module) {
  ...
  // TODO(chokobole): Uncomment this. Dependency: AlgebraicSimplifier
  // When uncomment, TupleSimplifier should be removed. This is added to pass
  // unittests.
...
std::unique_ptr<HloModule> ExtractModule(
  ...
  QCHECK(height == -1 || !cross_computation)
      << "Boundary cannot be calculated across the computations.";
