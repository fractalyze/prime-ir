...
func.func @test_canonicalize_intt_after_ntt(%p0 : !poly_ty) -> !poly_ty {
  ...
  %coeffs1 = poly.ntt %evals {root=#root} inverse=true : !tensor_ty
  %evals2 = poly.ntt %coeffs1 {root=#root} bit_reverse=false : !tensor_ty
  %coeffs2 = poly.ntt %evals2 {root=#root} inverse=true bit_reverse=false : !tensor_ty
...
func.func @test_canonicalize_ntt_after_intt(%t0 : !tensor_ty) -> !tensor_ty {
  ...
  %coeffs = poly.ntt %t0 {root=#root} inverse=true : !tensor_ty
  %evals = poly.ntt %coeffs {root=#root} : !tensor_ty
  %coeffs1 = poly.ntt %evals {root=#root} inverse=true bit_reverse=false : !tensor_ty
  %evals2 = poly.ntt %coeffs1 {root=#root} bit_reverse=false : !tensor_ty
