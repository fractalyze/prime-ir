...
protected:
  ...
  // BF<3> = GF(2^8): a 1-byte storage int.
  BinaryFieldType bf() { return BinaryFieldType::get(&context, /*level=*/3); }
...
TEST_F(FieldDenseElementInterfaceTest, BitSizeIsStorageWidth) {
  EXPECT_EQ(cast<DenseElementType>(pf()).getDenseElementBitSize(), 32u);
  EXPECT_EQ(cast<DenseElementType>(bf()).getDenseElementBitSize(), 8u);
...
//===----------------------------------------------------------------------===//
// Prime / binary field: scalar storage-int round-trip.
//===----------------------------------------------------------------------===//

TEST_F(FieldDenseElementInterfaceTest, PrimeFieldScalarRoundTrip) {
  auto ty = cast<DenseElementType>(pf());
  std::vector<char> bytes = {42, 0, 0, 0}; // 42 in i32 LE
  Attribute attr = ty.convertToAttribute(bytes);
  auto intAttr = dyn_cast<IntegerAttr>(attr);
  ASSERT_TRUE(intAttr);
  EXPECT_EQ(intAttr.getValue().getZExtValue(), 42u);
...
TEST_F(FieldDenseElementInterfaceTest, BinaryFieldScalarRoundTrip) {
  auto ty = cast<DenseElementType>(bf());
  std::vector<char> bytes = {static_cast<char>(0xAB)}; // 1-byte GF(2^8) element
  Attribute attr = ty.convertToAttribute(bytes);
  auto intAttr = dyn_cast<IntegerAttr>(attr);
  ASSERT_TRUE(intAttr);
  EXPECT_EQ(intAttr.getValue().getZExtValue(), 0xABu);

  llvm::SmallVector<char> back;
  ASSERT_TRUE(succeeded(ty.convertFromAttribute(attr, back)));
  ASSERT_EQ(back.size(), 1u);
  EXPECT_EQ(static_cast<unsigned char>(back[0]), 0xABu);
}

TEST_F(FieldDenseElementInterfaceTest, PrimeFieldRejectsWrongSize) {
  auto ty = cast<DenseElementType>(pf());
