...
func.func @test_msm() {
  ...
  %var1 = field.to_mont %c1 : !PF
  %var2 = field.to_mont %c2 : !PF

  %c3 = field.constant 3 : !SF
  %c5 = field.constant 5 : !SF
  %c7 = field.constant 7 : !SF

  %scalar3 = field.to_mont %c3 : !SF
  %scalar5 = field.to_mont %c5 : !SF
  %scalar7 = field.to_mont %c7 : !SF
  ...
  %scalar_mul1 = elliptic_curve.scalar_mul %scalar3, %jacobian1 : !SF, !jacobian -> !jacobian
  %scalar_mul2 = elliptic_curve.scalar_mul %scalar3, %jacobian2 : !SF, !jacobian -> !jacobian
  %scalar_mul3 = elliptic_curve.scalar_mul %scalar5, %jacobian3 : !SF, !jacobian -> !jacobian
  %scalar_mul4 = elliptic_curve.scalar_mul %scalar7, %jacobian4 : !SF, !jacobian -> !jacobian
  ...
  %extract_point_x, %extract_point_y = elliptic_curve.extract %msm_true_affine : !affine -> !PF, !PF
  %extract_point = tensor.from_elements %extract_point_x, %extract_point_y : tensor<2x!PF>
  ...
  // RUNNING MSM
  %scalars = tensor.from_elements %scalar3, %scalar3, %scalar5, %scalar7 : tensor<4x!SF>
  %points = tensor.from_elements %jacobian1, %jacobian2, %jacobian3, %jacobian4 : tensor<4x!jacobian>
  %msm_test = elliptic_curve.msm %scalars, %points: tensor<4x!SF>, tensor<4x!jacobian> -> !jacobian
  %msm_test_affine = elliptic_curve.convert_point_type %msm_test : !jacobian -> !affine

  %extract_point1x, %extract_point1y = elliptic_curve.extract %msm_test_affine : !affine -> !PF, !PF
  %extract_point1 = tensor.from_elements %extract_point1x, %extract_point1y : tensor<2x!PF>
