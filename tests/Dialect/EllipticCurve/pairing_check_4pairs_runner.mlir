...
func.func @test_pairing_check_4pairs() {
  ...
  // P1 = 2·G1 (affine)
  %p1_jac = elliptic_curve.scalar_mul %s2, %g1 : !SFm, !affine -> !jacobian
  %p1 = elliptic_curve.convert_point_type %p1_jac : !jacobian -> !affine

  // P2 = 6·G1 (affine)
  %p2_jac = elliptic_curve.scalar_mul %s6, %g1 : !SFm, !affine -> !jacobian
  %p2 = elliptic_curve.convert_point_type %p2_jac : !jacobian -> !affine

  // P3 = 4·G1 (affine)
  %p3_jac = elliptic_curve.scalar_mul %s4, %g1 : !SFm, !affine -> !jacobian
  %p3 = elliptic_curve.convert_point_type %p3_jac : !jacobian -> !affine

  // P4 = 20·G1 (affine)
  %p4_jac = elliptic_curve.scalar_mul %s20, %g1 : !SFm, !affine -> !jacobian
  %p4 = elliptic_curve.convert_point_type %p4_jac : !jacobian -> !affine

  // Q1 = 3·G2 (affine)
  %q1_jac = elliptic_curve.scalar_mul %s3, %g2 : !SFm, !g2affine -> !g2jacobian
  ...
  // Q3 = 5·G2 (affine)
  %q3_jac = elliptic_curve.scalar_mul %s5, %g2 : !SFm, !g2affine -> !g2jacobian
  %q3 = elliptic_curve.convert_point_type %q3_jac : !g2jacobian -> !g2affine

  // Q4 = -G2 (affine) (same as Q2)
  // Use a fresh negate to ensure independent value
  %neg_g2_2 = elliptic_curve.negate %g2 : !g2affine

  // Build tensors
  %g1_pts = tensor.from_elements %p1, %p2, %p3, %p4 : tensor<4x!affine>
  %g2_pts = tensor.from_elements %q1, %neg_g2, %q3, %neg_g2_2 : tensor<4x!g2affine>

  // Pairing check: should be true (product = 1)
  %result = elliptic_curve.pairing_check %g1_pts, %g2_pts
      : tensor<4x!affine>, tensor<4x!g2affine> -> i1
