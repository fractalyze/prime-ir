...
// Uses mont EC types from bn254_defs.mlir: !affinem, !jacobianm

// scalar = 0 → identity point, no runtime loop
// CHECK-LABEL: @test_scalar_mul_const_zero
// CHECK-NOT: scf.while
// CHECK: return
func.func @test_scalar_mul_const_zero(%point: !affinem) -> !jacobianm {
  %scalar = field.constant 0 : !SFm
  %result = elliptic_curve.scalar_mul %scalar, %point : !SFm, !affinem -> !jacobianm
  return %result : !jacobianm
}
...
// CHECK-NOT: scf.while
// CHECK: return
func.func @test_scalar_mul_const_one(%point: !affinem) -> !jacobianm {
...
// CHECK-NOT: scf.while
// CHECK: return
func.func @test_scalar_mul_const_five(%point: !affinem) -> !jacobianm {
...
// CHECK: scf.while
func.func @test_scalar_mul_dynamic(%point: !affinem, %scalar: !SFm) -> !jacobianm {
  %result = elliptic_curve.scalar_mul %scalar, %point : !SFm, !affinem -> !jacobianm
  return %result : !jacobianm
}
